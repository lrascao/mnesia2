...
%%% of Erlang.  The Mnesia2 test suite is structured in a hierarchy.
...
%%% disk resident schema is created on all nodes and Mnesia2 is
%%% started a on all nodes. This means that all test cases may
%%% assume that Mnesia2 is up and running on all acquired nodes.
%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%
%%% doc(TestCases)
...
%%%    Reads Config from mnesia2_test.config and starts them if neccessary.
%%%    Kills Mnesia2 and wipes out the Mnesia2 directories as a starter.
%%%
...
error(Format, Args, File, Line) ->
    global:send(mnesia2_global_logger, {failed, File, Line}),
    Fail = #'REASON'{file = filename:basename(File),
		     line = Line,
		     desc = Args},
    case global:whereis_name(mnesia2_test_case_sup) of
	undefined ->
	    ignore;
	Pid ->
	    Pid ! Fail
%% 	    global:send(mnesia2_test_case_sup, Fail),
    end,
...
slave_start_link(Host, Name, Retries) ->
    Debug = atom_to_list(mnesia2:system_info(debug)),
    Args = "-mnesia2 debug " ++ Debug ++
	" -pa " ++
	filename:dirname(code:which(?MODULE)) ++
	" -pa " ++
	filename:dirname(code:which(mnesia2)),
...
do_doc(Fd, [H | T], List) ->
	...
	TestCase when is_atom(TestCase), List == [] ->
	    do_doc(Fd, mnesia2_SUITE, TestCase, List);
...
transaction(Coordinator, MaxRetries) ->
    ...
    Coordinator ! {self(), mnesia2:transaction(Fun, MaxRetries)},
    activity_evaluator_loop(Coordinator).
...
sync_trans_tid_serial(Nodes) ->
    Fun = fun() -> mnesia2:write_lock_table(schema) end,
    rpc:multicall(Nodes, mnesia2, transaction, [Fun]).
...
prepare_test_case(Actions, N, Config, File, Line) ->
	...
	true ->
	    ok;
	false ->
	    rpc:multicall(Selected, application, set_env,[mnesia2, schema_location, opt_disc])
    end,
...
do_prepare([{init_test_case, Appls} | Actions], Selected, All, Config, File, Line) ->
    ...
    Alive = mnesia2_lib:intersect(nodes() ++ [node()], All2),
...
do_prepare([delete_schema | Actions], Selected, All, Config, File, Line) ->
    Alive = mnesia2_lib:intersect(nodes() ++ [node()], All),
    case diskless(Config) of
	true ->
	    skip;
	false ->
	    Del = fun(Node) ->
			  case mnesia2:delete_schema([Node]) of
			      ok -> ok;
...
do_prepare([create_schema | Actions], Selected, All, Config, File, Line) ->
	...
	true ->
	    skip;
	_Else ->
	    case mnesia2:create_schema(Selected) of
		ok ->
		    ignore;
		BadNodes ->
		    ?fatal("Cannot create Mnesia2 schema on ~p due to ~p~n",
                [Selected, BadNodes])
	    end
    end,
    do_prepare(Actions, Selected, All, Config, File, Line);
...
set_kill_timer(Config) ->
    case init:get_argument(mnesia2_test_timeout) of
	{ok, _ } -> ok;
	_ ->
	    Time0 =
...
kill_tc(Pid, Time) ->
    ...
    after Time ->
		...
		_ ->
		    ...
		    Files = mnesia2_lib:dist_coredump(),
...
remote_start(mnesia2, Config, Nodes) ->
    case diskless(Config) of
	true ->
	    application_controller:set_env(mnesia2,
					   extra_db_nodes,
					   Nodes -- [node()]),
	    application_controller:set_env(mnesia2,
					   schema_location,
...
%% Start Mnesia2 on all given nodes and wait for specified
%% tables to be accessible on each node. The atom all means
%% that we should wait for all tables to be loaded
%%
%% Returns a list of error tuples {BadNode, mnesia2, Reason}
start_mnesia2(Nodes) ->
    start_appls([mnesia2], Nodes).
start_mnesia2(Nodes, Tabs) when is_list(Nodes) ->
    start_appls([mnesia2], Nodes, [], Tabs).
...
sync_tables(Nodes, Tabs) ->
    ...
    if
	Res == 	[] ->
	    mnesia2:transaction(fun() -> mnesia2:write_lock_table(schema) end),
	    Res;
	true ->
	    Res
    end.
...
start_wait_loop(Tabs) ->
    ...
    after 0 ->
	    case mnesia2:wait_for_tables(Tabs, timer:seconds(30)) of
		ok ->
		    verify_nodes(Tabs);
		{timeout, BadTabs} ->
...
verify_nodes([Tab| Tabs], N) ->
    ?match(X when is_atom(X), mnesia2_lib:val({Tab, where_to_read})),
    Nodes = mnesia2:table_info(Tab, where_to_write),
    Copies =
	mnesia2:table_info(Tab, disc_copies) ++
        mnesia2:table_info(Tab, disc_only_copies) ++
	mnesia2:table_info(Tab, ram_copies),
    Local = mnesia2:table_info(Tab, local_content),
...
%% Nicely stop Mnesia2 on all given nodes
%%
%% Returns a list of error tuples {BadNode, Reason}
stop_mnesia2(Nodes) when is_list(Nodes) ->
    stop_appls([mnesia2], Nodes).
...
%% Abruptly kill Mnesia2 on all given nodes
%% Returns []
...
kill_mnesia2(Nodes) when is_list(Nodes) ->
    kill_appls([mnesia2], Nodes).
...
reload_appls([Appl | Appls], Selected) ->
    ...
    Conv = fun({error,{not_loaded,mnesia2}}) -> ok; (Else) -> Else end,
...
verify_mnesia2(Ups, Downs, File, Line) when is_list(Ups), is_list(Downs) ->
    BadUps =
	[N || N <- Ups, rpc:call(N, mnesia2, system_info, [is_running]) /= yes],
    BadDowns =
	[N || N <- Downs, rpc:call(N, mnesia2, system_info, [is_running]) == yes],
    if
	BadUps == [] ->
	    ignore;
	true ->
	    error("Mnesia2 is not running as expected: ~p~n",
		  [BadUps], File, Line)
    end,
    if
	BadDowns == [] ->
	    ignore;
	true ->
	    error("Mnesia2 is not stopped as expected: ~p~n",
		  [BadDowns], File, Line)
    end,
    ok.
...
verify_replica_location(Tab, [], [], [], _) ->
    ?match({'EXIT', _}, mnesia2:table_info(Tab, ram_copies)),
    ?match({'EXIT', _}, mnesia2:table_info(Tab, disc_copies)),
    ?match({'EXIT', _}, mnesia2:table_info(Tab, disc_only_copies)),
    ?match({'EXIT', _}, mnesia2:table_info(Tab, where_to_write)),
    ?match({'EXIT', _}, mnesia2:table_info(Tab, where_to_read)),
    [];
...
%%    sync_tables(AliveNodes0, [Tab]),
    ...
    S1 = ?match(AliveNodes, lists:sort(mnesia2:system_info(running_db_nodes))),
    S2 = ?match(DiscOnly, lists:sort(mnesia2:table_info(Tab, disc_only_copies))),
    S3 = ?match(Ram, lists:sort(mnesia2:table_info(Tab, ram_copies))),
    S4 = ?match(Disc, lists:sort(mnesia2:table_info(Tab, disc_copies))),
    S5 = ?match(Write, lists:sort(mnesia2:table_info(Tab, where_to_write))),
    S6 = case lists:member(This, Read) of
	     true ->
		 ?match(This, mnesia2:table_info(Tab, where_to_read));
	     false ->
