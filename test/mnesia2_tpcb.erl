...
%% MODULE
%%
%%   mnesia2_tpcb - TPC-B benchmarking of Mnesia2
%%
%% DESCRIPTION
%%
...
%%
%%     spawn(mnesia2_tpcb, start, [[[{n_drivers_per_node, 8}, {stop_after, infinity}]]),
%%     mnesia2_tpcb:stop().
%%
%%  Really advanced;
%%
%%    mnesia2_tpcb:init(([{n_branches, 8}, {replica_type, disc_only_copies}]),
%%    mnesia2_tpcb:run(([{n_drivers_per_node, 8}]),
%%    mnesia2_tpcb:run(([{n_drivers_per_node, 64}]).
%%    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
init(Args) ->
    ...
    stop(),
    if
	TabConfig#tab_config.use_running_mnesia2 =:= true ->
	    ignore;
	true ->
	    rpc:multicall(DbNodes, mnesia2, lkill, []),
	    case mnesia2:delete_schema(DbNodes) of
		ok ->
		    case mnesia2:create_schema(DbNodes) of
			ok ->
			    {Replies, BadNodes} =
				rpc:multicall(DbNodes, mnesia2, start, []),
...
create_tab(Name, Def) ->
    mnesia2:delete_table(Name),
    case mnesia2:create_table(Name, Def) of
	{atomic, ok} ->
	    ok;
	{aborted, Reason} ->
...
gen_branch(TC, BranchId) ->
    ...
    ok = mnesia2:activity(async_dirty, fun gen_accounts/4, ArgsA, mnesia2_frag),

    T = default_teller(TC),
    NT = TC#tab_config.n_tellers_per_branch,
    FirstT = BranchId * NT,
    ArgsT = [FirstT, FirstT + NT - 1, BranchId, T],
    ok = mnesia2:activity(async_dirty, fun gen_tellers/4, ArgsT, mnesia2_frag),

    B = default_branch(TC),
    FunB = fun() -> mnesia2:write(branch, B#branch{id = BranchId}, write) end,
    ok = mnesia2:activity(sync_dirty,  FunB, [], mnesia2_frag).
    
gen_tellers(Id, Last, BranchId, T) when Id =< Last ->
    mnesia2:write(teller, T#teller{id = Id, branch_id=BranchId}, write),
    gen_tellers(Id + 1, Last, BranchId, T);
gen_tellers(_, _, _, _) ->
    ok.
    
gen_accounts(Id, Last, BranchId, A) when Id =< Last ->
    mnesia2:write(account, A#account{id = Id, branch_id=BranchId}, write),
...
reporter_init(Starter, RC) ->
    register(mnesia2_tpcb, self()),
    process_flag(trap_exit, true),
    DbNodes = mnesia2:system_info(db_nodes),
    if
	RC#run_config.use_running_mnesia2 =:= true ->
	    ignore;
	true ->
	    {Replies, BadNodes} =
		rpc:multicall(DbNodes, mnesia2, start, []),
	...
	{ok, Stopper, State2} ->
		...
		RC#run_config.use_running_mnesia2 =:= true -> 
		    ignore;
		true -> 
		    rpc:multicall(DbNodes, mnesia2, stop, [])
	    end,
	    unlink(Starter),
...
table_info(Tab, Item) ->
    Fun = fun() -> mnesia2:table_info(Tab, Item) end,
    mnesia2:activity(sync_dirty, Fun, mnesia2_frag).

%% Returns {Storage, NFragments, ReplicaNodes}
table_storage(Tab) ->
    case mnesia2:table_info(branch, frag_properties) of
	[] ->
	    NFO = 0,
	    NR  = length(mnesia2:table_info(Tab, ram_copies)),
	    ND  = length(mnesia2:table_info(Tab, disc_copies)),
	    NDO = length(mnesia2:table_info(Tab, disc_only_copies)),
	    if
...
real_trans(RC, BranchId, Fun, Args, Mod) ->
	...
	false ->
	    mnesia2:activity(Type, Fun, Args, Mod);
	true ->
	    Node = table_info({branch, BranchId}, where_to_read),
	    case rpc:call(Node, mnesia2, activity, [Type, Fun, Args, Mod]) of
...
trans_type(TC, RC) ->
	...
	RC#run_config.use_sticky_locks =:= false ->
	    {fun add_delta/7, mnesia2};
	TC#tab_config.n_fragments =:= 0,
	RC#run_config.use_sticky_locks =:= true ->
	    {fun sticky_add_delta/7, mnesia2};
	TC#tab_config.n_fragments > 0,
	RC#run_config.use_sticky_locks =:= false ->
	    {fun frag_add_delta/7, mnesia2_frag}
    end.
...
add_delta(DriverId, BranchId, TellerId, _AccountBranchId, AccountId, HistoryId, Delta) ->
    ...
    [B]  = mnesia2:read(branch, BranchId, write),
    NewB = B#branch{balance = B#branch.balance + Delta},
    ok = mnesia2:write(branch, NewB, write),

    %% Add delta to teller balance
    [T]  = mnesia2:read(teller, TellerId, write),
    NewT = T#teller{balance = T#teller.balance + Delta},
    ok = mnesia2:write(teller, NewT, write),

    %% Add delta to account balance
    [A]  = mnesia2:read(account, AccountId, write),
    NewA = A#account{balance = A#account.balance + Delta},
    ok = mnesia2:write(account, NewA, write),
    ...
    ok = mnesia2:write(history, History, write),
...
sticky_add_delta(DriverId, BranchId, TellerId, _AccountBranchId, AccountId, HistoryId, Delta) ->
    ...
    %% Add delta to branch balance
    [B]  = mnesia2:read(branch, BranchId, read),
    NewB = B#branch{balance = B#branch.balance + Delta},
    ok = mnesia2:write(branch, NewB, sticky_write),

    %% Add delta to teller balance
    [T]  = mnesia2:read(teller, TellerId, read),
    NewT = T#teller{balance = T#teller.balance + Delta},
    ok = mnesia2:write(teller, NewT, sticky_write),

    %% Add delta to account balance
    [A]  = mnesia2:read(account, AccountId, read),
    NewA = A#account{balance = A#account.balance + Delta},
    ok = mnesia2:write(account, NewA, sticky_write),
    ...
    ok = mnesia2:write(history, History, sticky_write),
...
frag_add_delta(DriverId, BranchId, TellerId, AccountBranchId, AccountId, HistoryId, Delta) ->
    ...
    [B] = mnesia2:read(branch, BranchId, write),
    NewB = B#branch{balance = B#branch.balance + Delta},
    ok = mnesia2:write(NewB),

    %% Add delta to teller balance
    [T] = mnesia2:read({teller, BranchId}, TellerId, write),
    NewT = T#teller{balance = T#teller.balance + Delta},
    ok = mnesia2:write(NewT),

    %% Add delta to account balance
    %%io:format("frag_add_delta(~p): ~p\n", [node(), {account, BranchId, AccountId}]),
    [A] = mnesia2:read({account, AccountBranchId}, AccountId, write),
    NewA = A#account{balance = A#account.balance + Delta},
    ok = mnesia2:write(NewA),
...
verify_tabs() ->
    Nodes = mnesia2:system_info(running_db_nodes),
    case lists:member(node(), Nodes) of
	true ->
	    Tabs = [branch, teller, account, history],
	    io:format("TPC-B: Verifying tables: ~w~n", [Tabs]),
	    rpc:multicall(Nodes, mnesia2, wait_for_tables, [Tabs, infinity]),
	    
	    Fun = fun() ->
			  mnesia2:write_lock_table(branch),
			  mnesia2:write_lock_table(teller),
			  mnesia2:write_lock_table(account),
			  mnesia2:write_lock_table(history),
			  {Res, BadNodes} =
			      rpc:multicall(Nodes, ?MODULE, count_balance, []),
			  check_balance(Res, BadNodes)
		  end,
	    case mnesia2:transaction(Fun) of
		{atomic, Res} -> Res;
...
count_balance(Tab, [Frag | Frags], Bal, Size, BalPos) ->
    First = mnesia2:dirty_first(Frag),
...
count_frag_balance(Frag, Key, Bal, Size, BalPos) ->
    [Record] = mnesia2:dirty_read({Frag, Key}),
    Bal2 = Bal + element(BalPos, Record),
    Next = mnesia2:dirty_next(Frag, Key),
    count_frag_balance(Frag, Next, Bal2, Size + 1, BalPos).

check_balance([], []) ->
    mnesia2:abort({"No balance"});
check_balance(Summaries, []) ->
	...
	[] ->
	    ok;
	BadSummaries ->
	    mnesia2:abort({"Bad balance", One, BadSummaries})
    end;
check_balance(_, BadNodes) ->
    mnesia2:abort({"Bad nodes", BadNodes}).
