...
%%
-module(mnesia2_dirty_access_test).
...
dirty_write(Config, Storage) ->
    ...
    ?match({'EXIT', _},  mnesia2:dirty_write([])), 
    ?match({'EXIT', _},  mnesia2:dirty_write({Tab, 2})), 
    ?match({'EXIT', _},  mnesia2:dirty_write({foo, 2})), 
    ?match(ok,  mnesia2:dirty_write({Tab, 1, 2})), 

    ?match({atomic, ok},   mnesia2:transaction(fun() -> 
	    mnesia2:dirty_write({Tab, 1, 2}) end)), 
...
dirty_read(Config, Storage) ->
    ...
    ?match({'EXIT', _},  mnesia2:dirty_read([])), 
    ?match({'EXIT', _},  mnesia2:dirty_read({Tab})), 
    ?match({'EXIT', _},   mnesia2:dirty_read({Tab, 1, 2})), 
    ?match([],  mnesia2:dirty_read({Tab, 1})), 
    ?match(ok,  mnesia2:dirty_write({Tab, 1, 2})), 
    ?match([{Tab, 1, 2}],  mnesia2:dirty_read({Tab, 1})), 
    ?match(ok,  mnesia2:dirty_write({Tab, 1, 3})), 
    ?match([{Tab, 1, 2}, {Tab, 1, 3}],  mnesia2:dirty_read({Tab, 1})), 

    ?match({atomic, [{Tab, 1, 2}, {Tab, 1, 3}]},  
	   mnesia2:transaction(fun() -> mnesia2:dirty_read({Tab, 1}) end)), 

    ?match(false, mnesia2:async_dirty(fun() -> mnesia2:is_transaction() end)),
    ?match(false, mnesia2:sync_dirty(fun() -> mnesia2:is_transaction() end)),
    ?match(false, mnesia2:ets(fun() -> mnesia2:is_transaction() end)),
    ?match(false, mnesia2:activity(async_dirty, fun() -> mnesia2:is_transaction() end)),
    ?match(false, mnesia2:activity(sync_dirty,  fun() -> mnesia2:is_transaction() end)),
    ?match(false, mnesia2:activity(ets,         fun() -> mnesia2:is_transaction() end)),
...
dirty_update_counter(Config, Storage) ->
    ...
    ?match({'EXIT', _},  mnesia2:dirty_update_counter({Tab, 1}, [])), 
    ?match({'EXIT', _},  mnesia2:dirty_update_counter({Tab}, 3)), 
    ?match({'EXIT', _},  mnesia2:dirty_update_counter({foo, 1}, 3)), 
    ?match(5,  mnesia2:dirty_update_counter({Tab, 1}, 3)), 
    ?match([{Tab, 1, 5}],  mnesia2:dirty_read({Tab, 1})), 

    ?match({atomic, 8},  mnesia2:transaction(fun() ->
	   mnesia2:dirty_update_counter({Tab, 1}, 3) end)), 
 
    ?match(1,  mnesia2:dirty_update_counter({Tab, foo}, 1)),
    ?match([{Tab, foo,1}], mnesia2:dirty_read({Tab,foo})),

    ?match({ok,_}, mnesia2:subscribe({table, Tab, detailed})),

    ?match(2, mnesia2:dirty_update_counter({Tab, foo}, 1)),
    ?match([{Tab, foo,2}], mnesia2:dirty_read({Tab,foo})),
...
dirty_delete(Config, Storage) ->
    ...
    ?match({'EXIT', _},  mnesia2:dirty_delete([])), 
    ?match({'EXIT', _},  mnesia2:dirty_delete({Tab})), 
    ?match({'EXIT', _},  mnesia2:dirty_delete({Tab, 1, 2})), 
    ...
    ?match(ok,  mnesia2:dirty_write({Tab, 1, 2})), 
    ?match({atomic, ok},  mnesia2:transaction(fun() ->
	   mnesia2:dirty_delete({Tab, 1}) end)), 
    ?verify_mnesia2(Nodes, []).
...
dirty_delete_object(Config, Storage) ->
    ...
    OneRec = {Tab, 1, 2}, 
    ?match({'EXIT', _},  mnesia2:dirty_delete_object([])), 
    ?match({'EXIT', _},  mnesia2:dirty_delete_object({Tab})), 
    ?match({'EXIT', _},  mnesia2:dirty_delete_object({Tab, 1})), 
    ?match(ok,  mnesia2:dirty_delete_object(OneRec)), 
    ?match(ok,  mnesia2:dirty_write(OneRec)), 
    ?match(ok,  mnesia2:dirty_delete_object(OneRec)), 
    ?match(ok,  mnesia2:dirty_write(OneRec)), 
    ?match(ok,  mnesia2:dirty_write(OneRec)), 
    ?match(ok,  mnesia2:dirty_delete_object(OneRec)), 
    ...
    ?match(ok,  mnesia2:dirty_write(OneRec)), 
    ?match({atomic, ok},  mnesia2:transaction(fun() ->
	   mnesia2:dirty_delete_object(OneRec) end)), 

    ?match({'EXIT', {aborted, {bad_type, Tab, _}}}, mnesia2:dirty_delete_object(Tab, {Tab, {['_']}, 21})),
    ?match({'EXIT', {aborted, {bad_type, Tab, _}}}, mnesia2:dirty_delete_object(Tab, {Tab, {['$5']}, 21})),
...
dirty_match_object(Config, Storage) ->
    ...
    OneRec = {Tab, 1, 2}, 
    OnePat = {Tab, '$1', 2}, 
    ?match([], mnesia2:dirty_match_object(OnePat)), 
    ?match(ok,  mnesia2:dirty_write(OneRec)), 
    ?match([OneRec],  mnesia2:dirty_match_object(OnePat)), 
    ?match({atomic, [OneRec]},  mnesia2:transaction(fun() ->
	 mnesia2:dirty_match_object(OnePat) end)), 
    
    ?match({'EXIT', _},  mnesia2:dirty_match_object({foo, '$1', 2})), 
    ?match({'EXIT', _},  mnesia2:dirty_match_object({[], '$1', 2})), 
    ?verify_mnesia2(Nodes, []).
...
dirty_index_match_object(Config, Storage) ->
    ...
    ?match([],  mnesia2:dirty_index_match_object({Tab, '$1', 2}, ValPos)), 
    OneRec = {Tab, 1, 2}, 
    ?match(ok,  mnesia2:dirty_write(OneRec)), 
    
    ?match([OneRec],  mnesia2:dirty_index_match_object({Tab, '$1', 2}, ValPos)), 
    ?match({'EXIT', _},  mnesia2:dirty_index_match_object({Tab, '$1', 2}, BadValPos)), 
    ?match({'EXIT', _},  mnesia2:dirty_index_match_object({foo, '$1', 2}, ValPos)), 
    ?match({'EXIT', _},  mnesia2:dirty_index_match_object({[], '$1', 2}, ValPos)), 
    ?match({atomic, [OneRec]},  mnesia2:transaction(fun() ->
           mnesia2:dirty_index_match_object({Tab, '$1', 2}, ValPos) end)),     
...
dirty_index_read(Config, Storage) ->
    ...
    OneRec = {Tab, 1, 2}, 
    ?match([],  mnesia2:dirty_index_read(Tab, 2, ValPos)), 
    ?match(ok,  mnesia2:dirty_write(OneRec)), 
    ?match([OneRec],  mnesia2:dirty_index_read(Tab, 2, ValPos)), 
    ?match({atomic, [OneRec]},  
	   mnesia2:transaction(fun() -> mnesia2:dirty_index_read(Tab, 2, ValPos) end)),
    ?match(42, mnesia2:dirty_update_counter({Tab, 1}, 40)),
    ?match([{Tab,1,42}], mnesia2:dirty_read({Tab, 1})), 
    ?match([],  mnesia2:dirty_index_read(Tab, 2, ValPos)), 
    ?match([{Tab, 1, 42}],  mnesia2:dirty_index_read(Tab, 42, ValPos)),

    ?match({'EXIT', _},  mnesia2:dirty_index_read(Tab, 2, BadValPos)), 
    ?match({'EXIT', _},  mnesia2:dirty_index_read(foo, 2, ValPos)), 
    ?match({'EXIT', _},  mnesia2:dirty_index_read([], 2, ValPos)), 

    mnesia2:dirty_write({Tab, 5, 1}),
...
index_read_loop(Tab, N) when N =< 1000 ->
    spawn_link(fun() ->
		       mnesia2:transaction(fun() -> mnesia2:write({Tab, 5, 1}) end)
	       end),
    case mnesia2:dirty_match_object({Tab, '_', 1}) of
	[{Tab, 5, 1}] ->
...
dirty_index_update_set(Config, Storage) ->
    ...
    ?match([Rec1], mnesia2:dirty_index_read(Tab, 2, ValPos)),
    
    ?match(ok, mnesia2:dirty_write(Rec2)),
    R1 = mnesia2:dirty_index_read(Tab, 2, ValPos),
    ?match([Rec1, Rec2], lists:sort(R1)),
    
    ?match(ok, mnesia2:dirty_write(Rec3)),
    R2 = mnesia2:dirty_index_read(Tab, 2, ValPos),
    ?match([Rec2], lists:sort(R2)),
    ?match([Rec2], mnesia2:dirty_index_match_object(Pat1, ValPos)),
    
    {atomic, R3} = mnesia2:transaction(fun() -> mnesia2:match_object(Pat2) end), 
    ?match([Rec3, Rec2], lists:sort(R3)),
    
    ?match(ok, mnesia2:dirty_write(Rec4)),
    R4 = mnesia2:dirty_index_read(Tab, 2, ValPos),
    ?match([Rec2, Rec4], lists:sort(R4)),
    
    ?match(ok, mnesia2:dirty_delete({Tab, 4})),
    ?match([Rec2], mnesia2:dirty_index_read(Tab, 2, ValPos)),
    ...
    R5 = mnesia2:dirty_match_object(Pat2),
    ?match([Rec3, Rec2, Rec4], lists:sort(R5)),
    
    R6 = mnesia2:dirty_index_read(Tab, 2, ValPos),
    ?match([Rec2, Rec4], lists:sort(R6)),    
    ?match([], mnesia2:dirty_index_read(Tab, 4, ValPos2)),
    R7 = mnesia2:dirty_index_read(Tab, 14, ValPos2),
    ?match([Rec3, Rec2, Rec4], lists:sort(R7)),

    ?match({atomic, ok}, mnesia2:transaction(fun() -> mnesia2:write(Rec1) end)),
    R8 = mnesia2:dirty_index_read(Tab, 2, ValPos),
    ?match([Rec1, Rec2, Rec4], lists:sort(R8)),
    ?match([Rec1], mnesia2:dirty_index_read(Tab, 4, ValPos2)),
    R9 = mnesia2:dirty_index_read(Tab, 14, ValPos2),
    ?match([Rec2, Rec4], lists:sort(R9)),

    ?match({atomic, ok}, mnesia2:transaction(fun() -> mnesia2:delete_object(Rec2) end)),
    R10 = mnesia2:dirty_index_read(Tab, 2, ValPos),
    ?match([Rec1, Rec4], lists:sort(R10)),
    ?match([Rec1], mnesia2:dirty_index_read(Tab, 4, ValPos2)),
    ?match([Rec4], mnesia2:dirty_index_read(Tab, 14, ValPos2)),

    ?match(ok, mnesia2:dirty_delete({Tab, 4})),
    R11 = mnesia2:dirty_index_read(Tab, 2, ValPos),
    ?match([Rec1], lists:sort(R11)),
    ?match([Rec1], mnesia2:dirty_index_read(Tab, 4, ValPos2)),
    ?match([], mnesia2:dirty_index_read(Tab, 14, ValPos2)),
...
dirty_index_update_bag(Config, Storage) ->
    ...
    ?match(ok, mnesia2:dirty_delete_object(Rec5)),
    ?match([Rec1], mnesia2:dirty_index_read(Tab, 2, ValPos)),

    ?match({atomic, ok}, mnesia2:transaction(fun() -> mnesia2:write(Rec2) end)), 
    R1 = mnesia2:dirty_index_read(Tab, 2, ValPos),
    ?match([Rec1, Rec2], lists:sort(R1)),
    ...
    R3 = mnesia2:dirty_index_match_object(Pat1, ValPos),
    ?match([Rec1, Rec2], lists:sort(R3)),
 
    R4 = mnesia2:dirty_match_object(Pat2),
    ?match([Rec1, Rec3, Rec2], lists:sort(R4)),
 
    ?match({atomic, ok}, mnesia2:transaction(fun() -> mnesia2:write(Rec4) end)), 
    R5 = mnesia2:dirty_index_read(Tab, 2, ValPos),
    ...
    ?match([Rec2], mnesia2:dirty_index_read(Tab, 2, ValPos)),
    R7 = mnesia2:dirty_match_object(Pat2),
    ...
    R9 = mnesia2:dirty_index_read(Tab, 4, ValPos2),
    ?match([Rec1, Rec4], lists:sort(R9)),
    R10 = mnesia2:dirty_index_read(Tab, 14, ValPos2),
    ...
    R12 = mnesia2:dirty_index_read(Tab, 4, ValPos2),
    ?match([Rec1, Rec4], lists:sort(R12)),
    R13 = mnesia2:dirty_index_read(Tab, 14, ValPos2),
    ?match([Rec5, Rec3, Rec2], lists:sort(R13)),

    ?match({atomic, ok}, mnesia2:transaction(fun() -> mnesia2:delete_object(Rec1) end)),
    R14 = mnesia2:dirty_index_read(Tab, 2, ValPos),
    ?match([Rec5, Rec2, Rec4], lists:sort(R14)),
    ?match([Rec4], mnesia2:dirty_index_read(Tab, 4, ValPos2)),
    R15 = mnesia2:dirty_index_read(Tab, 14, ValPos2),
    ?match([Rec5, Rec3, Rec2], lists:sort(R15)),

    ?match(ok, mnesia2:dirty_delete_object(Rec5)),
    R16 = mnesia2:dirty_index_read(Tab, 2, ValPos),
    ?match([Rec2, Rec4], lists:sort(R16)),
    ?match([Rec4], mnesia2:dirty_index_read(Tab, 4, ValPos2)),
    R17 = mnesia2:dirty_index_read(Tab, 14, ValPos2),
    ?match([Rec3, Rec2], lists:sort(R17)),

    ?match(ok, mnesia2:dirty_write(Rec1)),
    ?match(ok, mnesia2:dirty_delete({Tab, 1})),
    R18 = mnesia2:dirty_index_read(Tab, 2, ValPos),
    ?match([Rec2, Rec4], lists:sort(R18)),
    ?match([Rec4], mnesia2:dirty_index_read(Tab, 4, ValPos2)),
    R19 = mnesia2:dirty_index_read(Tab, 14, ValPos2),
...
dirty_iter(Config, Storage) ->
    ...
    ?match({'EXIT', _}, mnesia2:dirty_first(foo)), 
    ?match({'EXIT', _}, mnesia2:dirty_next(foo, foo)), 
    ?match({'EXIT', _}, mnesia2:dirty_slot(foo, 0)), 
    ?match({'EXIT', _}, mnesia2:dirty_slot(foo, [])), 
    ?match({atomic, Keys},
	   mnesia2:transaction(fun() -> lists:sort(all_nexts(Tab)) end)),     
    ?verify_mnesia2(Nodes, []).
...
update_trans(Tab, Key, Acc) ->
    Update = 
	fun() -> 
		Res = (catch mnesia2:read({Tab, Key})),
		case Res of 
		    [{Tab, Key, Extra, Acc}] ->
    ...
    after
	3 -> 
	    case catch mnesia2:sync_dirty(Update) of
		ok -> 	    
...
insert(Tab, N) when N > 0 ->
    ok = mnesia2:sync_dirty(fun() -> false = mnesia2:is_transaction(), mnesia2:write({Tab, N, N, 0}) end),
    insert(Tab, N-1).
