...
good_dump(Config) when is_list(Config) ->
    ...
    T1 = gordon,
    ?match(ok, mnesia2_registry:create_table(T1)),
    One = {T1, 1, 0, integer, 0, 10},
    Two = {T1, "two", 3, integer, 0, 20},
    Three = {T1, 3, 0, string, 6, "thirty"},
    ?match(ok, mnesia2:dirty_write(One)),
    ?match(ok, mnesia2:dirty_write(Two)),
    ?match(ok, mnesia2:dirty_write(Three)),
    ?match([One], mnesia2:dirty_read({T1, 1})),
    ?match([_ | _], dump_registry(Node, T1)),

    NewOne = {T1, 1, 0, integer, 0, 1},
    NewFour = {T1, "4", 1, string, 4, "four"},

    ?match([NewOne], mnesia2:dirty_read({T1, 1})),
    ?match([Two], mnesia2:dirty_read({T1, "two"})),
    ?match([], mnesia2:dirty_read({T1, 3})),
    ?match([NewFour], mnesia2:dirty_read({T1, "4"})),

    T2 = blixt,
    ?match({'EXIT', {aborted, {no_exists, _}}},
	   mnesia2:dirty_read({T2, 1})),
    ?match([_ |_], dump_registry(Node, T2)),

    NewOne2 = setelement(1, NewOne, T2),
    NewFour2 = setelement(1, NewFour, T2),

    ?match([NewOne2], mnesia2:dirty_read({T2, 1})),
    ?match([], mnesia2:dirty_read({T2, "two"})),
    ?match([], mnesia2:dirty_read({T2, 3})),
    ?match([NewFour2], mnesia2:dirty_read({T2, "4"})),
...
bad_dump(Config) when is_list(Config) ->
    ...
    OldTab = ming,
    ?match({'EXIT', {aborted, _}}, mnesia2_registry:start_restore(no_tab, self())),
    ?match({atomic, ok}, mnesia2:create_table(OldTab, [{attributes, [a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p,q]}])),
    ?match({'EXIT',{aborted,{bad_type,_}}}, dump_registry(Node, OldTab)),
    ?match(stopped, mnesia2:stop()),

    ?match({'EXIT', {aborted, _}}, mnesia2_registry:create_table(down_table)),
    ?match({'EXIT', {aborted, _}}, mnesia2_registry:start_restore(no_tab, self())),
