...
%%
-module(mnesia2_consistency_test).
    ...
    [{consistency_after_restart, [],
     ...
     {backup_consistency, [],
      [{group, interupted_install_fallback},
       ...
       {group, mnesia2_down_during_backup_causes_switch},
       {group, mnesia2_down_during_backup_causes_abort},
     ...
     {mnesia2_down_during_backup_causes_switch, [],
      [cause_switch_before, cause_switch_after]},
     {mnesia2_down_during_backup_causes_abort, [],
...
consistency_after_restart(ReplicaType, NodeConfig, Config) ->
    ...
    ?log("waiting 10 seconds for tpcb run to end~n", []),
    timer:sleep(timer:seconds(10)),
    ?log("killing mnesia2~n", []),
    mnesia2_test_lib:kill_mnesia2([Node1]),
    %% Start and wait for tables to be loaded on all nodes
    timer:sleep(timer:seconds(3)),
    ?log("restarting mnesia2~n", []),
    ?match([], mnesia2_test_lib:start_mnesia2(Nodes,[account,branch,teller, history])), 
    mnesia2_tpcb:stop(),
...
consistency_after_dump_tables(ReplicaType, NodeConfig, Config) ->
    ...
    A ! fun() -> mnesia2_tpcb:run(TpcbConfig) end,
    timer:sleep(timer:seconds(10)),
    ?match({atomic, ok}, rpc:call(Node1, mnesia2, dump_tables,
    ...
    ?match([], mnesia2_test_lib:start_mnesia2(Nodes,[account, branch,
						   teller, history])),
    mnesia2_tpcb:stop(),
    ?match(ok, mnesia2_tpcb:verify_tabs()),
    ?verify_mnesia2(Nodes, []).
...
consistency_after_add_replica(ReplicaType, NodeConfig, Config) ->
    ...
    A ! fun () -> mnesia2_tpcb:run(TpcbConfig) end,
    timer:sleep(timer:seconds(10)),
    ?match({atomic, ok}, mnesia2:add_table_copy(account, AddNode, ReplicaType)),
    mnesia2_tpcb:stop(),
    ?match(ok, mnesia2_tpcb:verify_tabs()),
    ?verify_mnesia2(Nodes0, []).
...
consistency_after_del_replica(ReplicaType, NodeConfig, Config) ->
    ...
    A ! fun () -> mnesia2_tpcb:run(TpcbConfig) end,
    timer:sleep(timer:seconds(10)),
    ?match({atomic, ok}, mnesia2:del_table_copy(account, Node2)),
    mnesia2_tpcb:stop(),
    ?match(ok, mnesia2_tpcb:verify_tabs()),
    ?verify_mnesia2(Nodes, []).
...
consistency_after_move_replica(ReplicaType, NodeConfig, Config) ->
    ...
    A ! fun () -> mnesia2_tpcb:run(TpcbConfig) end,
    timer:sleep(timer:seconds(10)),
    ?match({atomic, ok}, mnesia2:move_table_copy(account, Node1, Node2)),    
    ?log("First move completed from node ~p to ~p ~n", [Node1, Node2]),
    ?match({atomic, ok}, mnesia2:move_table_copy(account, Node2, Node1)),
    mnesia2_tpcb:stop(),
    ?match(ok, mnesia2_tpcb:verify_tabs()),
    ?verify_mnesia2(Nodes, []).
...
consistency_after_transform_table(Type, Config) ->
    ...
    ?match({atomic, ok}, mnesia2:create_table(tab1, [{index, [3]}, {Type, [N1]}])),
    ?match({atomic, ok}, mnesia2:create_table(tab2, [{index, [3]}, {Type, [N1,N2]}])),
    ?match({atomic, ok}, mnesia2:create_table(tab3, [{index, [3]}, {Type, Nodes}])),
    ?match({atomic, ok}, mnesia2:create_table(empty, [{index, [3]},{Type, Nodes}])),
    
    Tabs = lists:sort([tab1, tab2, tab3, empty]),
    
    [[mnesia2:dirty_write({Tab, N, N}) || N <- lists:seq(1,10)] || 
	Tab <- Tabs -- [empty, tab4]],
    mnesia2:dump_log(),
    
    Ok = lists:duplicate(4, {atomic, ok}),
    ?match(Ok, [mnesia2:transform_table(Tab, fun({T, N, N}) ->  {T, N, N, ok} end,
	[k,a,n]) || Tab <- Tabs]),
    [?match([k,a,n], mnesia2:table_info(Tab, attributes)) || Tab <- Tabs],

    Filter = fun(Tab) -> mnesia2:foldl(fun(A, Acc) when size(A) == 3 -> [A|Acc];
					 (A, Acc) when size(A) == 4 -> Acc
				      end, [], Tab)
	     end,    
	
    ?match([[],[],[],[]], [element(2,mnesia2:transaction(Filter, [Tab])) || Tab <- Tabs]),
    
    mnesia2_test_lib:kill_mnesia2(Nodes),
    mnesia2_test_lib:start_mnesia2(Nodes, Tabs),
    
    ?match([Tabs, Tabs, Tabs], 
	[lists:sort(rpc:call(Node, mnesia2,system_info, [tables]) -- [schema]) || Node <- Nodes]),
...
consistency_after_fallback(ReplicaType, NodeConfig, Config) ->
    ...
    Node1 = hd(Nodes),
    %%?verbose("mnesia2 info: ~p~n", [mnesia2:info()]),
    ...
    ?match(ok, mnesia2:backup(consistency_after_fallback2)),
    
    %% Install the backup as a fallback
    ?verbose("Doing fallback~n", []),
    ?match(ok, mnesia2:install_fallback(consistency_after_fallback2)),
    timer:sleep(timer:seconds(Delay)),
    ...
    %% Stop and then start mnesia2 and check table consistency
    %%?verbose("Restarting mnesia2~n", []),
    mnesia2_test_lib:kill_mnesia2(Nodes),
    mnesia2_test_lib:start_mnesia2(Nodes,[account,branch,teller,history]),
    
    ?match(ok, mnesia2_tpcb:verify_tabs()),
    if 
	ReplicaType == ram_copies ->
	    %% Test that change_table_copy work i.e. no account.dcd file exists.
	    ?match({atomic, ok}, mnesia2:change_table_copy_type(account, node(), disc_copies));
	true -> 
	    ignore
    end,
...
consistency_after_restore(ReplicaType, Op, Config) ->
    ...
    ?match({atomic, ok}, mnesia2:create_table(carA, [{ReplicaType, Nodes}])),
    ?match({atomic, ok}, mnesia2:create_table(carB, [{ReplicaType, Nodes -- [Node1]}])),
    ?match({atomic, ok}, mnesia2:create_table(carC, [{ReplicaType, Nodes -- [Node2]}])),
    ?match({atomic, ok}, mnesia2:create_table(carD, [{ReplicaType, [Node2]}])),

    NList = lists:seq(0, 20),
    [lists:foreach(fun(E) -> ok = mnesia2:dirty_write({Tab, E, 1}) end, NList) ||
	Tab <- Tabs],
    
    {ok, Name, _} = mnesia2:activate_checkpoint([{max, [schema | Tabs]}, 
						{ram_overrides_dump, true}]),
    ?verbose("Doing backup~n", []),
    ?match(ok, mnesia2:backup_checkpoint(Name, File)),
    ?match(ok, mnesia2:deactivate_checkpoint(Name)),
    
    [lists:foreach(fun(E) -> ok = mnesia2:dirty_write({Tab, E, 2}) end, NList) ||
	Tab <- Tabs],
    ...
    Restore = fun(F, Args) ->
		      case mnesia2:restore(F, Args) of
...
change_tab(Father, Tab, Test) ->
    ...
    Update = fun() ->
		     case mnesia2:read({Tab, Key}) of
			 [{Tab, Key, 1}] -> 
			     quit;
			 [{Tab, Key, _N}] ->				       
			     mnesia2:write({Tab, Key, 3})
		     end
	     end,
...
restore_verify_tabs([Tab | R]) ->
    ?match({atomic, ok}, 
	   mnesia2:transaction(fun() -> mnesia2:foldl(fun({_, _, 1}, ok) -> 
							    ok;
						       (Else, Acc) ->
							    [Else|Acc]
						    end, ok, Tab) 
...
updates_during_checkpoint_activation(ReplicaType,NodeConfig,Config) ->
    ...
    {ok, CPName, _NodeList} =
        mnesia2:activate_checkpoint([{max, mnesia2:system_info(tables)},
				    {ram_overrides_dump, true}]),
    timer:sleep(timer:seconds(Delay)),
    ...
    ?match(ok, mnesia2:backup_checkpoint(CPName, 
					updates_during_checkpoint_activation2)),
    timer:sleep(timer:seconds(Delay)),

    ?match(ok, mnesia2:install_fallback(updates_during_checkpoint_activation2)),
...
updates_during_checkpoint_iteration(ReplicaType,NodeConfig,Config) ->
   ...
   %?verbose("mnesia2 info: ~p~n", [mnesia2:info()]),
    ...
    {ok, CPName, _NodeList} =
        mnesia2:activate_checkpoint([{max, mnesia2:system_info(tables)},
                                    {ram_overrides_dump,true}]),  
    A ! fun () -> mnesia2:backup_checkpoint(CPName, File) end,
    ...
    ?match({error,{"Bad balance",_,_}}, mnesia2_tpcb:verify_tabs()),
...
load_table_with_activated_checkpoint(Type, Config) ->
    ...
    mnesia2_test_lib:stop_mnesia2([Node1]),    
    ...
    A ! fun () ->
		mnesia2:table_info(Tab,checkpoints)
	end,
    ...
    A ! fun () ->
		mnesia2:backup_checkpoint(CPName, load_table_a)
	end,
    ?match_receive({A,ok}),
    B ! fun () ->
		mnesia2:backup_checkpoint(CPName, load_table_b)
	end,
    ?match_receive({B,ok}),
    
    Mod = mnesia2_backup, %% Assume local files
...
view(Source, Mod) ->
    ...
    {ok,TabList} =  
	mnesia2:traverse_backup(Source, Mod, dummy, read_only, View, []),
    lists:sort(TabList).
...
add_table_copy_to_table_with_activated_checkpoint(Type,Config) -> 
    ...
    ?match({atomic,ok},mnesia2:add_table_copy(Tab,Node2,ram_copies)),
    ...
    ?match(ok, mnesia2:dirty_write({Tab, 1, 815})),
    ?match(ok, mnesia2:dirty_write({Tab, 2, 815})),

    A ! fun () ->
		mnesia2:backup_checkpoint(CPName, add_table_a)
	end,
    ?match_receive({A,ok}),
    B ! fun () ->
		mnesia2:backup_checkpoint(CPName, add_table_b)
	end,
    ?match_receive({B,ok}),
...
inst_fallback_process_dies(Config) when is_list(Config) ->
    ...
    TestPid = self(),
    DebugId = {mnesia2_bup, fallback_receiver_loop, pre_swap},
    DebugFun =
    ...
    ?match(ok, mnesia2:backup_checkpoint(CPName, install_backup)),
    
    A ! fun() -> mnesia2:install_fallback(install_backup) end,    
    ...
    mnesia2_test_lib:kill_mnesia2(Nodes),    
    ?verbose("~n---->Mnesia2 is stopped everywhere<-----~n", []),
...
check_data([N1 | R], Tab) ->
    ?match([{Tab, 1, 4711}], rpc:call(N1, mnesia2, dirty_read, [{Tab, 1}])),
    ?match([{Tab, 2, 42}],   rpc:call(N1, mnesia2, dirty_read, [{Tab, 2}])),
    ?match([{Tab, 3, 256}],  rpc:call(N1, mnesia2, dirty_read, [{Tab, 3}])),
    check_data(R, Tab);
check_data([], _Tab) -> 
    ok.
...
fatal_when_inconsistency(Config) when is_list(Config) ->
    ...
    ?match(ok, mnesia2:backup_checkpoint(CPName, install_backup)),
    ?match(ok, mnesia2:dirty_write({Tab, 2, 42424242})),
    
    A ! fun() ->
                mnesia2:install_fallback(install_backup)
        end,    
    ...
    ?match([{Node2, mnesia2, _}], mnesia2_test_lib:start_mnesia2([Node2],[])),
    is_running(Node2, no),
    ?match([{Node3, mnesia2, _}], mnesia2_test_lib:start_mnesia2([Node3],[])),
    is_running(Node3, no),
    mnesia2_test_lib:kill_mnesia2(Nodes),
    
    ?match(ok, mnesia2:install_fallback(install_backup)),    
    mnesia2_test_lib:start_mnesia2(Nodes,[Tab]),
...
is_running(Node, Shouldbe) ->
    timer:sleep(1000),
    Running = rpc:call(Node, mnesia2, system_info, [is_running]),
    case Running of
	Shouldbe -> ok;
...
do_uninstall(Config,DebugPoint) ->
    ...
    DebugId = {mnesia2_bup, uninstall_fallback2, DebugPoint},
    ...
    A ! fun () ->
		mnesia2:uninstall_fallback()
	end,
    %%
    %%  catch the debug entry in mnesia and kill one Mnesia2 node
    %%
...
do_something_during_backup(Action,DebugPoint,Config) ->
    ...
    DebugId = {mnesia2_log, tab_copier, DebugPoint},
    ...
    A ! fun () ->    %% A shall perform the backup, so the test proc is
		...
		mnesia2:backup_checkpoint(CPName, Bak)
	end,
    
    %% catch the debug function of mnesia2, stop the backup process 
    ...
    case Action of
        cause_switch ->
	    mnesia2_test_lib:kill_mnesia2([ReadNode_a]),
	    timer:sleep(timer:seconds(1));
        cause_abort ->
	    mnesia2_test_lib:kill_mnesia2([Node2,Node3]),
	    timer:sleep(timer:seconds(1));
        change_schema ->
    ...
    case Action of
        ...
        _ ->      %% cause_switch, change_schema
            ...
            WritePid ! fun () ->
			       ?match(ok, mnesia2:dirty_write({Tab, 1, 815})),
			       ?match(ok, mnesia2:dirty_write({Tab, 2, 816})),
			       ok
                       end,
            ?match_receive({ WritePid, ok }),  
            ?match(ok, mnesia2:install_fallback(Bak))
    end,        
