...
%% 3a Run mnesia2_meter:go()
%% 3b Run mnesia2_meter:go(ReplicaType)
%% 3c Run mnesia2_meter:go(ReplicaType, Nodes)

-module(mnesia2_meter).
...
update_fun(Name) ->
    fun() ->
            case mnesia2:read({person, Name}) of
                [] -> 
                    mnesia2:abort(no_such_person);
                [Pers] ->
                    [Partner] = mnesia2:read({person, Pers#person.married_to}), 
                    mnesia2:write(Pers#person{married_to = undefined}), 
                    mnesia2:write(Partner#person{married_to = undefined})
            end
    end.
    
update_records(Name) ->
  mnesia2:transaction(update_fun(Name)).

sync_dirty_update_records(Name) ->
  {atomic, mnesia2:sync_dirty(update_fun(Name))}.

async_dirty_update_records(Name) ->
  {atomic, mnesia2:async_dirty(update_fun(Name))}.

ets_update_records(Name) ->
  {atomic, mnesia2:ets(update_fun(Name))}.

w_update_records(Name) ->
    F = fun() ->
                case mnesia2:wread({person, Name}) of
                    [] -> 
                        mnesia2:abort(no_such_person);
                    [Pers] ->
                        [Partner] = mnesia2:wread({person, Pers#person.married_to}), 
                        mnesia2:write(Pers#person{married_to = undefined}), 
                        mnesia2:write(Partner#person{married_to = undefined})
                end
        end, 
    mnesia2:transaction(F).
...
s_update_records(Name) ->
    F = fun() ->
                case mnesia2:read({person, Name}) of
                    ...
                    [Pers] ->
                        [Partner] = mnesia2:read({person, Pers#person.married_to}), 
                        mnesia2:s_write(Pers#person{married_to = undefined}), 
                        mnesia2:s_write(Partner#person{married_to = undefined})
                end
        end, 
    mnesia2:transaction(F).

dirty_update_records(Name) ->
    case mnesia2:dirty_read({person, Name}) of
        [] -> 
            mnesia2:abort(no_such_person);
        [Pers] ->
            [Partner] = mnesia2:dirty_read({person, Pers#person.married_to}), 
            mnesia2:dirty_write(Pers#person{married_to = undefined}), 
            mnesia2:dirty_write(Partner#person{married_to = undefined})
    end,
    {atomic, ok}.
...
write_records_fun(Pers, Partner) ->
    fun() ->
            ...
            mnesia2:write(P#person{name = Pers, married_to = Partner}), 
            mnesia2:write(P#person{name = Partner, married_to = Pers})
    end.

write_records(Pers, Partner) ->
    mnesia2:transaction(write_records_fun(Pers, Partner)).

sync_dirty_write_records(Pers, Partner) ->
    {atomic, mnesia2:sync_dirty(write_records_fun(Pers, Partner))}.

async_dirty_write_records(Pers, Partner) ->
    {atomic, mnesia2:async_dirty(write_records_fun(Pers, Partner))}.

ets_write_records(Pers, Partner) ->
    {atomic, mnesia2:ets(write_records_fun(Pers, Partner))}.

s_write_records(Pers, Partner) ->
    F = fun() ->
                P = #person{children = [ulla, bella]}, 
                mnesia2:s_write(P#person{name = Pers, married_to = Partner}), 
                mnesia2:s_write(P#person{name = Partner, married_to = Pers})
        end,
    mnesia2:transaction(F).

dirty_write_records(Pers, Partner) ->
    P = #person{children = [ulla, bella]}, 
    mnesia2:dirty_write(P#person{name = Pers, married_to = Partner}), 
    mnesia2:dirty_write(P#person{name = Partner, married_to = Pers}),
    {atomic, ok}.

ets_opt_write_records(Pers, Partner) ->
    case mnesia2:table_info(person, where_to_commit) of
...
read_records_fun(Pers, Partner) ->
    fun() ->
            case {mnesia2:read({person, Pers}), 
                  mnesia2:read({person, Partner})} of
                {[_], [_]} ->
                    ok;
                _ ->
                    mnesia2:abort(no_such_person)
...
read_records(Pers, Partner) ->
    mnesia2:transaction(read_records_fun(Pers, Partner)).

sync_dirty_read_records(Pers, Partner) ->
    {atomic, mnesia2:sync_dirty(read_records_fun(Pers, Partner))}.

async_dirty_read_records(Pers, Partner) ->
    {atomic, mnesia2:async_dirty(read_records_fun(Pers, Partner))}.

ets_read_records(Pers, Partner) ->
    {atomic, mnesia2:ets(read_records_fun(Pers, Partner))}.

dirty_read_records(Pers, Partner) ->
    case {mnesia2:dirty_read({person, Pers}), 
          mnesia2:dirty_read({person, Partner})} of
        {[_], [_]} ->
            {atomic, ok};
        _ ->
            mnesia2:abort(no_such_person)
...
run_meter(M, Nodes, FunOverhead) when is_record(M, meter) ->
    ...
    case catch init_records(M#meter.init, ?TIMES) of
        {atomic, ok} ->
            rpc:multicall(Nodes, mnesia2, dump_log, []),
