...
%% This code exercises the mnesia2 system and produces a bunch
...
go(Nodes) when hd(Nodes) == node() ->
    {ok, Out} = file:open("MNESIA2_COST", write),
    put(out, Out),
    ...
    {atomic, ok} = mnesia2:create_table(item, TabDef),

    round("single ram copy", "no index"),
    {atomic, ok} = mnesia2:add_table_index(item, #item.e),
    round("single ram copy", "One index"),

    {atomic, ok} = mnesia2:add_table_index(item, #item.c),
    round("single ram copy", "Two indexes"),

    {atomic, ok} = mnesia2:del_table_index(item, #item.e),
    {atomic, ok} = mnesia2:del_table_index(item, #item.c),

    {atomic, ok} = mnesia2:change_table_copy_type(item, node(), disc_copies),
    round("single disc copy", "no index"),

    {atomic, ok} = mnesia2:change_table_copy_type(item, node(), ram_copies),
	...
	_Len ->
	    ...
	    {atomic, ok} = mnesia2:add_table_copy(item, N2, ram_copies),
...
round(Replication, Index) ->
	...
	fun() -> mnesia2:write(#item{}) end),


    run(Replication, Index, [read],
	fun() -> mnesia2:read({item, 1234}) end),

    run(Replication, Index, [read, write],
	fun() -> mnesia2:read({item, 1234}),
		 mnesia2:write(#item{}) end),

    run(Replication, Index, [wread, write],
	fun() -> mnesia2:wread({item, 1234}),
		 mnesia2:write(#item{}) end),


    run(Replication, Index, [match, write, write, write],
	fun() -> mnesia2:match_object({item, 1, '_', '_', '_', true}),
		 mnesia2:write(#item{a =1}),
		 mnesia2:write(#item{a =2}),
		 mnesia2:write(#item{a =3}) end).
...
do_trans_times(I, F) when I /= 0 ->
    {atomic, _} = mnesia2:transaction(F),
...
table_load([N1,N2| _ ] = Ns) ->    
    Nodes = [N1,N2],
    rpc:multicall(Ns, mnesia2, lkill, []),
    ok = mnesia2:delete_schema(Ns),
    ok = mnesia2:create_schema(Nodes),
    rpc:multicall(Nodes, mnesia2, start, []),
    ...
    [mnesia2:create_table(Tab,TabDef) || Tab <- Tabs],

%%     InitTab = fun(Tab) ->
%% 		      mnesia2:write_lock_table(Tab),
%% 		      InitRec = fun(Key) -> mnesia2:write(Tab,#item{a=Key},write) end,
%% 		      lists:foreach(InitRec, lists:seq(1,100))
%% 	      end,
%%     
%%    {Time,{atomic,ok}} = timer:tc(mnesia2,transaction, [fun() ->lists:foreach(InitTab, Tabs) end]),
    mnesia2:dump_log(),
%%    io:format("Init took ~p msec ~n", [Time/1000]),
    rpc:call(N2, mnesia2, stop, []),    timer:sleep(1000),
    mnesia2:stop(), timer:sleep(500),
    %% Warmup
    ok = mnesia2:start([{no_table_loaders, 1}]),    
    timer:tc(mnesia2, wait_for_tables, [Tabs, infinity]),
    mnesia2:dump_log(),
    rpc:call(N2, mnesia2, dump_log, []),
    io:format("Initialized ~n",[]),

    mnesia2:stop(), timer:sleep(1000),
    ok = mnesia2:start([{no_table_loaders, 1}]),
    {T1, ok} = timer:tc(mnesia2, wait_for_tables, [Tabs, infinity]),
    io:format("Loading from disc with 1 loader ~p msec~n",[T1/1000]),
    mnesia2:stop(), timer:sleep(1000),
    ok = mnesia2:start([{no_table_loaders, 4}]),
    {T2, ok} = timer:tc(mnesia2, wait_for_tables, [Tabs, infinity]),
...
remote_load(Tabs,Loaders) ->
    ok = mnesia2:start([{no_table_loaders, Loaders}]),
%%    io:format("~p ~n", [mnesia2_controller:get_info(500)]),
    {Time, ok} = timer:tc(mnesia2, wait_for_tables, [Tabs, infinity]),
    timer:sleep(1000), mnesia2:stop(), timer:sleep(1000),
    Time.
