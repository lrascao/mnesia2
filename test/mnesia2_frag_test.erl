...
nice_single(Config) when is_list(Config) ->
    ...
    ?match({atomic, ok}, mnesia2:create_table(Tab, [{frag_properties, Props}])),
    ...
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab, {add_node, Node2})),
    Dist =  frag_dist(Tab), 
    ...
    %% Add new fragment hopefully on the new node
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab, {add_frag, Dist2})),
    Dist3 =  frag_dist(Tab), 
    ?match([{Node1, 2}, {Node2, 2}], Dist3), 
    ?match([3, 2, 5, 2], frag_rec_dist(Tab)),

    %% Add new fragment hopefully on the new node
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab, {add_frag, Dist3})),
    Dist4 =  frag_dist(Tab), 
    ?match([{Node2, 2}, {Node1, 3}], Dist4), 
    ?match([_, _, _, _, _], frag_rec_dist(Tab)),

    %% Dropping a node in pool should not affect distribution
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab, {del_node, Node1})),
    ?match([{Node2, 2}, {Node1, 3}], frag_dist(Tab)),
    ?match([_, _, _, _, _], frag_rec_dist(Tab)),

    %% Dropping a fragment
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab, del_frag)),
    Dist5 =  frag_dist(Tab), 
    ?match([{Node2, 2}, {Node1, 2}], Dist5), 
    ?match([3, 2, 5, 2], frag_rec_dist(Tab)),
    
    %% Add new fragment hopefully on the new node
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab, {add_frag, Dist5})),
    Dist6 =  frag_dist(Tab), 
    ...
    ?match(Len when Len > 0,
		    length(mnesia2:table_info(Tab, frag_properties))),
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab, deactivate)),
    ?match(0, length(mnesia2:table_info(Tab, frag_properties))),

    %% Making the table fragmented again
    Props2 = [{n_fragments, 1}],
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab, {activate, Props2})),
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab, {add_frag, frag_dist(Tab)})),
    Dist7 = frag_dist(Tab),
...
nice_multi(Config) when is_list(Config) ->
    ...
    Tab = frag_master,
    Name = frag_rec,
    Type = case mnesia2_test_lib:diskless(Config) of 
	       true -> n_ram_copies;
	       false -> n_disc_copies
	   end,
    Props = [{n_fragments, 2},
    ...
    ?match(Len when Len > 0,
		    length(mnesia2:table_info(TabF, frag_properties))),
    ?match({atomic, ok}, mnesia2:change_table_frag(TabF, deactivate)),
    ?match(0, length(mnesia2:table_info(TabF, frag_properties))),
    ?match(Len when Len > 0,
		    length(mnesia2:table_info(Tab, frag_properties))),
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab, deactivate)),
    ?match(0, length(mnesia2:table_info(Tab, frag_properties))),
    ...
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab, {activate, Props2})),
    ?match({atomic, ok}, mnesia2:delete_table(TabF)),
    ?match({atomic, ok}, mnesia2:create_table(TabF, DefF)),
    [frag_write(TabF, {TabF, {Id}, Id})  || Id <- lists:seq(1, 16)],
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab, {add_frag, frag_dist(Tab)})),
    ...
    ?match({atomic, ok}, mnesia2:delete_table(TabF)),
    ?match(false, lists:member(TabF, mnesia2:system_info(tables))),
    ?match({atomic, ok}, mnesia2:delete_table(Tab)),
    ?match(false, lists:member(Tab, mnesia2:system_info(tables))),
...
nice_access(Config) when is_list(Config) ->
    ...
    ?match(done, mnesia2:activity(transaction, fun do_access/3, [Tab, Tab, Pool], mnesia2_frag)),
    ?match(done, mnesia2:activity(transaction, fun do_access/3, [TabF, Tab, Pool], mnesia2_frag)),

    ?verify_mnesia2(Nodes, []).

do_access(Tab, Master, Pool) ->
    ?match(20, mnesia2:table_info(Tab, n_fragments)),
    ?match(Pool, mnesia2:table_info(Tab, node_pool)),
    ?match(2, mnesia2:table_info(Tab, n_ram_copies)),
    ?match(0, mnesia2:table_info(Tab, n_disc_copies)),
    ?match(0, mnesia2:table_info(Tab, n_disc_only_copies)),
    ?match(20, length(mnesia2:table_info(Tab, frag_names))),
    ?match(20, length(mnesia2:table_info(Tab, frag_size))),
    ?match(20, length(mnesia2:table_info(Tab, frag_memory))),
    PoolSize = length(Pool),
    ?match(PoolSize, length(mnesia2:table_info(Tab, frag_dist))),
    ?match(400, mnesia2:table_info(Tab, size)),
    ?match(I when is_integer(I), mnesia2:table_info(Tab, memory)),
    ?match(Tab, mnesia2:table_info(Tab, base_table)),

    Foreign = 
	if
	    Master == Tab ->
		?match(undefined, mnesia2:table_info(Tab, foreign_key)),
		?match([_], mnesia2:table_info(Tab, foreigners)),
                ?match({'EXIT', {aborted, {combine_error, Tab, frag_properties, {foreign_key, undefined}}}},
                    mnesia2:read({Tab, 5}, 5, read)),
                fun({T, _K}) -> T end;
	    true ->
		?match({Master, 3}, mnesia2:table_info(Tab, foreign_key)),
		?match([], mnesia2:table_info(Tab, foreigners)),
                fun({T, K}) -> {T, K} end
	end,
    ...
    ?match(400, mnesia2:foldl(Count, 0, Tab)),
    ?match(400, mnesia2:foldr(Count, 0, Tab)),
    ?match(ok, mnesia2:write({Tab, [-1], 1})),
    ?match(401, length(mnesia2:match_object(Tab, {Tab, '_', '_'}, read))),
    ?match(401, length(mnesia2:select(Tab, [{{Tab, '_', '$1'}, [], ['$1']}], read))),

    First = mnesia2:select(Tab, [{{Tab, '_', '$1'}, [], ['$1']}], 10, read),
    TestCont = fun('$end_of_table', Total, _This) -> 
		       Total;
		  ({Res,Cont1}, Total, This) ->
		       Cont = mnesia2:select(Cont1),
    ...
    Frag2key = mnesia2:dirty_first(Frag2),
    ?match({[Frag2key],_},mnesia2:select(Tab,[{{Tab,Frag2key,'$1'},[],['$1']}],100,read)),

    ?match([{Tab, [-1], 1}], mnesia2:read(Foreign({Tab, 1}), [-1], read)),
    ?match(401, mnesia2:foldl(Count, 0, Tab)),
    ?match(401, mnesia2:foldr(Count, 0, Tab)),
    ?match(ok, mnesia2:delete(Foreign({Tab, 2}), 2, write)),
    ?match([], mnesia2:read(Foreign({Tab, 2}), 2, read)),
    ?match([{Tab, 3, 3}], mnesia2:read(Foreign({Tab, 3}), 3, read)),
    ?match(400, mnesia2:foldl(Count, 0, Tab)),
    ?match(400, mnesia2:foldr(Count, 0, Tab)),
    ?match(ok, mnesia2:delete_object({Tab, 3, 3})),
    ?match([], mnesia2:read(Foreign({Tab, 3}), 3, read)),
    One = lists:sort([{Tab, 1, 1}, {Tab, [-1], 1}]),
    Pat = {Tab, '$1', 1},
    ?match(One, lists:sort(mnesia2:match_object(Tab, Pat, read))),
    ?match([1,[-1]], lists:sort(mnesia2:select(Tab, [{Pat, [], ['$1']}], read))),
    ?match([[[-1]]], lists:sort(mnesia2:select(Tab, [{Pat, [{is_list, '$1'}], [['$1']]}], read))),
    ?match([[1, 100]], lists:sort(mnesia2:select(Tab, [{Pat, [{is_integer, '$1'}], [['$1',100]]}], read))),
    ?match([1,[-1]], lists:sort(mnesia2:select(Tab, [{Pat, [{is_list, '$1'}], ['$1']},{Pat, [{is_integer, '$1'}], ['$1']}], read))),
    ?match(One, lists:sort(mnesia2:index_match_object(Tab, Pat, Attr, read) )),
    ?match(One, lists:sort(mnesia2:index_read(Tab, 1, Attr))),
    Keys = mnesia2:all_keys(Tab),
    ?match([-1], lists:max(Keys)),  %% OTP-3779
    ?match(399, length(Keys)),
    ?match(399, mnesia2:foldl(Count, 0, Tab)),
    ?match(399, mnesia2:foldr(Count, 0, Tab)),

    ?match(Pool, lists:sort(mnesia2:lock({table, Tab}, write))),
...
iter_access(Config) when is_list(Config) ->
    ...
    RawRead = 
	fun(Frag) -> 
		Node = mnesia2:table_info(Frag, where_to_read),
		{Frag, rpc:call(Node, ets, tab2list, [Frag])}
	end,
    
    ?match(done, mnesia2:activity(transaction, fun nice_iter_access/3, [Tab, FragNames, RawRead], mnesia2_frag)),

    FragNames = frag_names(Tab),
    [First, Second | _] = FragNames,
    [Last, LastButOne | _] = lists:reverse(FragNames),

    ?match({atomic, ok}, mnesia2:clear_table(First)),
    ?match({atomic, ok}, mnesia2:clear_table(Second)),
    ?match({atomic, ok}, mnesia2:clear_table(lists:nth(8, FragNames))),
    ?match({atomic, ok}, mnesia2:clear_table(lists:nth(9, FragNames))),
    ?match({atomic, ok}, mnesia2:clear_table(lists:nth(10, FragNames))),
    ?match({atomic, ok}, mnesia2:clear_table(lists:nth(11, FragNames))),
    ?match({atomic, ok}, mnesia2:clear_table(LastButOne)),
    ?match({atomic, ok}, mnesia2:clear_table(Last)),

    ?match(done, mnesia2:activity(transaction, fun evil_iter_access/3, [Tab, FragNames, RawRead], mnesia2_frag)),
    Size = fun(Table) -> mnesia2:table_info(Table, size) end,
    ?match(true, 0 < mnesia2:activity(transaction, Size, [Tab], mnesia2_frag)),
    ?match({atomic, ok}, mnesia2:activity(ets, fun() -> mnesia2:clear_table(Tab) end, mnesia2_frag)),
    ?match(0, mnesia2:activity(transaction, Size, [Tab], mnesia2_frag)),
...
nice_iter_access(Tab, FragNames, RawRead) ->
    ...
    ?match(ExpectedFirst, mnesia2:first(Tab)),
    ExpectedLast = lists:last(Keys),
    ?match(ExpectedLast, mnesia2:last(Tab)),
    
    ExpectedAllPrev = ['$end_of_table' | lists:droplast(Keys)],
    ?match(ExpectedAllPrev, lists:map(fun(K) -> mnesia2:prev(Tab, K) end, Keys)),
    
    ExpectedAllNext = tl(Keys) ++ ['$end_of_table'],
    ?match(ExpectedAllNext, lists:map(fun(K) -> mnesia2:next(Tab, K) end, Keys)),
...
evil_create(Config) when is_list(Config) ->
    ...
    Create = fun(T, D, P) -> mnesia2:create_table(T, [{frag_properties, P}| D]) end,
...
evil_change(Config) when is_list(Config) ->
    ...
    ?match({atomic,ok}, mnesia2:change_table_frag(Tab1, {add_frag, Nodes})),
    Dist10 =  frag_dist(Tab1), 
    ?match([{N1,3}], Dist10), 
    ?match({atomic, ok}, mnesia2:change_table_frag(Tab1, {add_node, N2})),
    Dist11 =  frag_dist(Tab1),
    ?match([{N2,0},{N1,3}], Dist11),
    mnesia2_test_lib:kill_mnesia2([N2]),
    ?match({aborted,_}, mnesia2:change_table_frag(Tab1, {add_frag, [N2,N1]})),
    ?verbose("~p~n",[frag_dist(Tab1)]),
    mnesia2_test_lib:start_mnesia2([N2]),

    Tab2 = evil_change_disc,
    ?match({atomic,ok}, Create(Tab2,[],[{n_disc_copies,1},{n_fragments,1},{node_pool,[N1,N2]}])),
    ?verbose("~p~n", [frag_dist(Tab2)]),
    ?match({atomic,ok}, mnesia2:change_table_frag(Tab2, {add_frag, [N1,N2]})),
    _Dist20 =  frag_dist(Tab2), 
    mnesia2_test_lib:kill_mnesia2([N2]),
    ?match({atomic,ok}, mnesia2:change_table_frag(Tab2, {add_frag, [N1,N2]})),
    ?match({aborted,_}, mnesia2:change_table_frag(Tab2, {add_frag, [N2,N1]})),
...
evil_combine(doc) -> ["Bug in mnesia2_4.1.5. and earlier"];
evil_combine(suite) -> [];
evil_combine(Config) when is_list(Config) ->
    [Node1] = Nodes = ?acquire_nodes(1, Config),
    ?match({atomic, ok},mnesia2:create_table(tab1, [{disc_copies, [Node1]},
						   {frag_properties, [{n_fragments, 2},
								      {node_pool, [Node1]},
								      {n_disc_copies, 1}]}])),
    ?match({atomic, ok},mnesia2:create_table(tab2, [{disc_copies, [Node1]}])),
    mnesia2:wait_for_tables([tab1, tab2], infinity),

    Add2 = fun() -> 
		   mnesia2:transaction(fun() ->
					      mnesia2:write({tab2,1,1})
				      end)
	   end,
    Fun = fun() ->
		  Add2(),
		  mnesia2:write({tab1,9,10})
	  end,
    ?match(ok, mnesia2:activity({transaction, 1}, Fun, [], mnesia2_frag)),
    
    Read = fun(T, K) ->
		   mnesia2:read(T, K, read)
	   end,
    
    ?match([{tab1,9,10}],mnesia2:activity(async_dirty, Read, [tab1, 9], mnesia2_frag)),
    ?match([{tab2,1,1}],mnesia2:activity(async_dirty, Read, [tab2, 1], mnesia2_frag)),
...
evil_loop(Config) when is_list(Config) ->
    ...
    Trans  = fun(Fun, Args) -> mnesia2:activity(transaction, Fun, Args, mnesia2_frag) end,
    ...
    [mnesia2:dirty_delete_object(Frag, R) || R <- [R1, R2], 
					   Tab <- Tabs,
					   Frag <- frag_names(Tab)],
...
evil_delete_db_node(Config) when is_list(Config) ->
    ...
    Tab = frag_master,
    ?match({atomic, ok}, mnesia2:create_table(Tab, [{frag_properties, [{Type, 2}, {node_pool, Nodes}]}])),
    ExtraTab = frag_foreigner,
    ?match({atomic, ok}, mnesia2:create_table(ExtraTab, [{frag_properties, [{foreign_key, {Tab, key}}, {node_pool, Nodes}]}])),
    
    GetPool = fun(T) -> 
		      case lists:keysearch(node_pool, 1, mnesia2:table_info (T, frag_properties)) of
    ...
    ?match(stopped, rpc:call(Remote, mnesia2, stop, [])),
    ?match({atomic, ok}, mnesia2:del_table_copy(schema, Remote)),
	   
    ?match([Local], GetPool(Tab)),
    ?match([Local], GetPool(ExtraTab)),
     
    ?verify_mnesia2([Local], []).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Misc convenient helpers

frag_write(Tab, Rec) ->
    Fun = fun() -> mnesia2:write(Tab, Rec, write) end,
    mnesia2:activity(sync_dirty, Fun, mnesia2_frag).

frag_dist(Tab) ->
    Fun = fun() -> mnesia2:table_info(Tab, frag_dist) end,
    mnesia2:activity(sync_dirty, Fun, mnesia2_frag).

frag_names(Tab) ->
    Fun = fun() -> mnesia2:table_info(Tab, frag_names) end,
    mnesia2:activity(sync_dirty, Fun, mnesia2_frag).

frag_rec_dist(Tab) -> 
    Fun = fun() -> mnesia2:table_info(Tab, frag_size) end,
    [Size || {_, Size} <- mnesia2:activity(sync_dirty, Fun, mnesia2_frag)].

table_size(Tab) ->
    Node = mnesia2:table_info(Tab, where_to_read),
    rpc:call(Node, mnesia2, table_info, [Tab, size]).
