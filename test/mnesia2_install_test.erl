...
silly() ->
    ...
    Config = [{nodes, Nodes}],
    mnesia2_test_lib:eval_test_case(?MODULE, silly2, Config).

silly2(Config) when is_list(Config) ->
    [Node1 | _] = Nodes = ?acquire_nodes(3, Config),
    mnesia2_test_lib:kill_mnesia2(Nodes),
    ?ignore([mnesia2:delete_schema([N]) || N <- Nodes]),
    ?match(ok, mnesia2:create_schema([Node1])),
    ?match(ok, rpc:call(Node1, mnesia2, start, [])),
    ?match(ok, rpc:call(Node1, mnesia2, wait_for_tables,
			[[schema], infinity])),
	...
	case length(Nodes) of
		...
		?match({atomic, ok},
		       rpc:call(Node1, mnesia2, add_table_copy, AddDb)),
		Args = [[{extra_db_nodes, [Node1]}]],
		?match(ok, rpc:call(Node2, mnesia2, start, Args)),
		ChangeDb = [schema, Node2, disc_copies],
		?match({atomic, ok},
		       rpc:call(Node1, mnesia2, change_table_copy_type,
				ChangeDb)),
		?match([], mnesia2_test_lib:sync_tables([Node1, Node2],
						       [schema])),
...
silly_durability(Config) when is_list(Config) ->
    ...
    Read = fun() -> mnesia2:read({Tab, a}) end,
    Write = fun() -> mnesia2:write({Tab, a, b}) end,

    ?match({atomic, []},
	   rpc:call(Node1, mnesia2, transaction, [Read])),
    ?match({atomic, ok},
	   rpc:call(Node1, mnesia2, transaction, [Write])),
	...
	true -> 
	    skip;
	false ->
	    ?match(ok, rpc:call(Node1, mnesia2, wait_for_tables, [[Tab], infinity])),	    
	    ?match({atomic, [{Tab, a, b}]},
		   rpc:call(Node1, mnesia2, transaction, [Read]))
    end,
    ?verify_mnesia2([Node1], []).
...
silly_move(Config) when is_list(Config) ->
    ...
    Tab = silly_move,
    ?match({atomic, ok},
	   rpc:call(Node1, mnesia2,
		    create_table, [Tab, [{ram_copies, [Node2]}]])),
    ?match([], mnesia2_test_lib:sync_tables([Node1, Node2], [Tab])),
    ...
    ?match({atomic, [{Tab, a, b}]}, rpc:call(Node1, mnesia2, transaction, [Read])),
...
silly_upgrade(Config) when is_list(Config) ->
    ...
    ?match({atomic, ok}, mnesia2:create_table(Tab2, [{disc_only_copies, Nodes}])),

    CpState = add_some_records(Tab1, Tab2, []),
    ?match(match, verify_state(Tab1, Tab2, CpState)),
    file:delete(Bup),
    ?match(ok, mnesia2:backup(Bup)),
    Args = [{name, Name}, {ram_overrides_dump, true},
	    {min, [Tab1, schema]}, {max, [Tab2]}],
    ?match({ok, Name, _}, mnesia2:activate_checkpoint(Args)),
    ...
    file:delete(Bup2),
    ?match(ok, mnesia2:backup_checkpoint(Name, Bup2)),
    ...
    ?match(ok, mnesia2:install_fallback(Bup2)),
    file:delete(Bup2),
    %% Will generate intentional crash, fatal error
    ?match([], mnesia2_test_lib:stop_mnesia2([Node2])),
    wait_till_dead([Node1, Node2]),
    ?match([], mnesia2_test_lib:start_mnesia2([Node1, Node2], [Tab1, Tab2])),
    ?match(match, verify_state(Tab1, Tab2, CpState)),

    ?match(ok, mnesia2:install_fallback(Bup)),
    file:delete(Bup),
    %% Will generate intentional crash, fatal error
...
wait_till_dead(Repeat = [N|Ns]) ->
    ...
    case lists:keymember(mnesia2, 1, Apps) of
	true ->
	    timer:sleep(10),
	    wait_till_dead(Repeat);
	false ->
	    case rpc:call(N, erlang, whereis, [mnesia2_monitor]) of
		undefined ->
		    wait_till_dead(Ns);
		_ ->
		    timer:sleep(10),
...
add_some_records(Tab1, Tab2, Old) ->
    ...
    ?match({atomic, _}, mnesia2:transaction(Trans)),
...
add_more_records(Tab1, Tab2, Old) ->
    ...
    lists:foreach(fun(R) -> mnesia2:dirty_write(R) end, Change1),
    lists:foreach(fun(R) -> mnesia2:dirty_delete(R) end, Del),
    Fun = fun(R) -> mnesia2:write(R) end,
    Trans = fun() -> lists:foreach(Fun, Change2 ++ New) end,
    ?match({atomic, ok}, mnesia2:transaction(Trans)),
...
verify_state(Tab1, Tab2, Exp) ->
    Fun = fun() ->
		  Act1 = [mnesia2:read({Tab1, K}) || K <- mnesia2:all_keys(Tab1)],
		  Act2 = [mnesia2:read({Tab2, K}) || K <- mnesia2:all_keys(Tab2)],
...
transform_some_records(Tab1, _Tab2, Old) ->
    ...
    ?match({atomic, ok},
	   mnesia2:transform_table(Tab1, Fun, [key, val, extra])),
...
dist(Config) when is_list(Config) ->
    ...
    Storage = mnesia2_test_lib:storage_type(disc_copies, Config),
    ?match({ok, _}, mnesia2_tpcb:start(dist_args(Nodes, Storage))).
...
conflict(Config) when is_list(Config) ->
    Nodes = ?acquire_nodes(3, Config ++ [{tc_timeout, 10 * 60000}]),
    Storage = mnesia2_test_lib:storage_type(disc_copies, Config),
    ?match({ok, _}, mnesia2_tpcb:start(conflict_args(Nodes, Storage))).
