...
read(Tid, Store, Tab, IxKey, Pos) ->
    ResList = mnesia2_locker:ixrlock(Tid, Store, Tab, IxKey, Pos),
    %% Remove all tuples which don't include Ixkey, happens when Tab is a bag
    case val({Tab, setorbag}) of
	bag -> 
	    mnesia2_lib:key_search_all(IxKey, Pos, ResList);
	_ -> 
	    ResList
    end.
...
add_index2([{Pos, Ixt} |Tail], Type, Tab, K, Obj, OldRecs0) ->
    ...
    OldRecs1 = case OldRecs0 of
		   undefined -> mnesia2_lib:db_get(Tab, K);
		   _ -> OldRecs0
	       end,
...
delete_index2([{Pos, Ixt} | Tail], Tab, K) ->
    DelObjs = mnesia2_lib:db_get(Tab, K), 
...
del_object_bag(Tab, Key, Obj, Pos, Ixt, undefined) -> 
    ...
    Old = [X || X <-  mnesia2_lib:db_get(Tab, Key), element(Pos, X) =:= IxKey],
...
merge([{_IxKey, RealKey} | Tail], Tab, Pat, Ack) ->
    ...
    Recs = mnesia2_lib:db_match_object(Tab, Pat2),
...
dirty_select(Tab, Spec, Pos) ->
    ...
    lists:append([mnesia2_lib:db_get(StorageType, Tab, Key) || {_,Key} <- RealKeys]).

dirty_read(Tab, IxKey, Pos) ->
    ResList = mnesia2:dirty_rpc(Tab, ?MODULE, dirty_read2,
			       [Tab, IxKey, Pos]),
    case val({Tab, setorbag}) of
	bag ->
...
tab2filename(Tab, Pos) ->
    mnesia2_lib:dir(Tab) ++ "_" ++ integer_to_list(Pos) ++ ".DAT".

tab2tmp_filename(Tab, Pos) ->
    mnesia2_lib:dir(Tab) ++ "_" ++ integer_to_list(Pos) ++ ".TMP".
...
delete_transient_index(Tab, Pos, disc_only_copies) ->
    Tag = {Tab, index, Pos},
    mnesia2_monitor:unsafe_close_dets(Tag),
    _ = file:delete(tab2filename(Tab, Pos)),
    del_index_info(Tab, Pos), %% Uses val(..)
    mnesia2_lib:unset({Tab, {index, Pos}});
...
init_disc_index(Tab, [Pos | Tail]) when is_integer(Pos) ->
    ...
    mnesia2_monitor:open_dets(IxTag, Args),
    ...
    mnesia2_lib:set({Tab, {index, Pos}}, IxTag),
...
create_fun(Cont, Tab, Pos) ->
    fun(read) ->
		...
		case Cont of
		    {start, KeysPerChunk} ->
			mnesia2_lib:db_init_chunk(disc_only_copies, Tab, KeysPerChunk);
		    '$end_of_table' -> 
			'$end_of_table';
		    _Else ->
			mnesia2_lib:db_chunk(disc_only_copies, Cont)
		end,
	    case Data of
		'$end_of_table' ->
		    end_of_input;
...
add_ram_index(Tab, Pos) when is_integer(Pos) ->
    ...
    Index = mnesia2_monitor:mktab(mnesia2_index, [IndexType, public]),
    Insert = fun(Rec, _Acc) ->
    ...
    mnesia2_lib:set({Tab, {index, Pos}}, Index),
