...
tm_mnesia2_down(Node) ->
    lists:foreach(fun(Name) -> cast(Name, {mnesia2_down, Node}) end,
		  checkpoints()).
...
tm_retain(Tid, Tab, Key, Op, Checkpoints) ->
    case Op of
	clear_table ->
	    OldRecs = mnesia2_lib:db_match_object(Tab, '_'),
	    send_group_retain(OldRecs, Checkpoints, Tid, Tab, []),
	    OldRecs;
	_ ->
	    OldRecs = mnesia2_lib:db_get(Tab, Key),
...
tm_del_copy(Tab, Node) when Node == node() ->
    mnesia2_subscr:unsubscribe_table(Tab),
    case val({Tab, commit_work}) of
	[{checkpoints, Checkpoints} | _ ] ->	    
...
writers(Retainers) ->
    ...
    mnesia2_lib:uniq(Writers).

do_activate(Cp) ->
    Name = Cp#checkpoint_args.name,
    Nodes = Cp#checkpoint_args.nodes,
    case mnesia2_tm:prepare_checkpoint(Nodes, Cp) of
	{Replies, []} ->
...
collect_pending(Name, Nodes, IgnoreNew) ->
	...
	{Replies, []} ->
	    try
		UnionTab = ?ets_new_table(mnesia2_union, [bag]),
...
start_retainer(Cp) ->
    ...
    Name = Cp#checkpoint_args.name,
    case supervisor:start_child(mnesia2_checkpoint_sup, [Cp]) of
	{ok, _Pid} ->
...
start(Cp) ->
    ...
    mnesia2_monitor:start_proc({?MODULE, Name}, ?MODULE, init, Args).
...
init(Cp) ->
    ...
    try ?ets_new_table(mnesia2_pending_checkpoint, Props) of
	PendingTab ->
...
retainer_create(Cp, R, Tab, Name, Storage) ->
    T = ?ets_new_table(mnesia2_retainer, [set, public, {keypos, 2}]),
    Overriders = Cp#checkpoint_args.ram_overrides_dump,
...
prepare_ram_tab(Tab, T, ram_copies, true, false) ->
	...
	true -> 
	    Log = mnesia2_log:open_log(prepare_ram_tab, 
	    ...
	    traverse_dcd(mnesia2_log:chunk_log(Log, start), Log, Add),
...
traverse_dcd({Cont, Recs}, Log, Fun) ->     %% trashed data?? 
    ...
    traverse_dcd(mnesia2_log:chunk_log(Log, Cont), Log, Fun);
...
retainer_fixtable(Tab, Bool) when is_atom(Tab) ->
    mnesia2_lib:db_fixtable(val({Tab, storage_type}), Tab, Bool);
retainer_fixtable({ets, Tab}, Bool) ->
    mnesia2_lib:db_fixtable(ram_copies, Tab, Bool);
retainer_fixtable({dets, Tab}, Bool) ->
    mnesia2_lib:db_fixtable(disc_only_copies, Tab, Bool).

retainer_delete({ets, Store}) ->
    ?ets_delete_table(Store);
retainer_delete({dets, Store}) ->
    mnesia2_lib:dets_sync_close(Store),
...
retainer_loop(Cp = #checkpoint_args{is_activated=false, name=Name}) ->
	...
	{From, {activate, Pending}} ->
            StillPending = mnesia2_recover:still_pending(Pending),
...
deactivate_tab(R) ->
	...
	del({Tab, checkpoints}, Name),   %% Keep checkpoint info for table_info & mnesia2_session 
...
del_chkp_info(Tab, Name) ->   
	...
	[{checkpoints, ChkList} | Rest] -> 
		...
		[] -> 
		    ...
		    mnesia2_lib:set({Tab, commit_work}, Rest);
		NewList ->
		    mnesia2_lib:set({Tab, commit_work}, 
...
do_del_retainer2(Cp, R, Node) ->
    ...
    if
	Writers == [] ->
	    Event = {mnesia2_checkpoint_deactivated, Cp#checkpoint_args.name},
...
do_change_copy(Cp, Tab, FromType, ToType) ->
	...
	FromType == disc_only_copies ->
	    mnesia2_lib:dets_sync_close(Old),
	    loaded = mnesia2_lib:dets_to_ets(Old, New, Fname, set, no, yes),
...
%%	             {ram_file, true},
		     ...
		     {repair, false}],
	    {ok, _} = mnesia2_lib:dets_sync_open(New, Props),
	    ok = mnesia2_dumper:raw_dump_table(New, Old),
