...
do_get_disc_copy2(Tab, Reason, Storage, Type) when Storage == disc_copies ->
	...
	_ ->
	    ...
	    _Count = mnesia2_log:dcd2ets(Tab, Repair),
	    case mnesia2_monitor:get_env(dump_disc_copies_at_startup)
		andalso mnesia2_dumper:needs_dump_ets(Tab) of
		true ->
		    ok = mnesia2_log:ets2dcd(Tab);
		_ ->
		    ignore
	    end
    end,
    mnesia2_index:init_index(Tab, Storage),
    snmpify(Tab, Storage),
...
do_get_disc_copy2(Tab, Reason, Storage, Type) when Storage == ram_copies ->
	...
	_ ->
	    mnesia2_monitor:mktab(Tab, Args),
	    Fname = mnesia2_lib:tab2dcd(Tab),
	    Datname = mnesia2_lib:tab2dat(Tab),
	    Repair = mnesia2_monitor:get_env(auto_repair),
	    case mnesia2_monitor:use_dir() of
		true ->
		    case mnesia2_lib:exists(Fname) of
			true -> mnesia2_log:dcd2ets(Tab, Repair);
			false ->
			    case mnesia2_lib:exists(Datname) of
				true ->
				    mnesia2_lib:dets_to_ets(Tab, Tab, Datname,
							   Type, Repair, no);
				false ->
				    false
			    end
...
do_get_network_copy(Tab, Reason, Ns, Storage, Cs) ->
	...
	true ->
		...
		ok ->
		    set({Tab, load_node}, Node),
		    set({Tab, load_reason}, Reason),
		    mnesia2_controller:i_have_tab(Tab),
...
do_snmpify(Tab, Us, Storage) ->
    Snmp = mnesia2_snmp_hook:create_table(Us, Tab, Storage),
...
init_receiver(Node, Tab,Storage,Cs,Reason) ->
	...
	fun() ->
		...
		mnesia2_locker:rlock(Tid, Ts#tidstore.store, {schema, Tab}),
    ...
    unlink(whereis(mnesia2_tm)),  %% Avoid late unlink from tm
    Res.

start_remote_sender(Node,Tab,Storage) ->
    mnesia2_controller:start_remote_sender(Node, Tab, self(), Storage),
    put(mnesia2_table_sender_node, {Tab, Node}),
    receive
...
spawned_receiver(ReplyTo,Tab,Storage,Cs, SenderPid,TabSize,DetsData, Init) ->
    ...
    ReplyTo ! {self(),Done},
    unlink(ReplyTo),
    unlink(whereis(mnesia2_controller)),
    exit(normal).
...
do_init_table(Tab,Storage,Cs,SenderPid,
	...
	{Storage,Tab} ->
	    %% Debug info
	    Node = node(SenderPid),
	    put(mnesia2_table_receiver, {Tab, Node, SenderPid}),
	    mnesia2_tm:block_tab(Tab),
...
%%		    {ram_file, true},
		    ...
		    | DetsOpts],
	    file:delete(Tmp),
	    case mnesia2_lib:dets_sync_open(Tab, Args) of
		{ok, _} ->
...
handle_last({disc_copies, Tab}, _Type, nobin) ->
    Ret = mnesia2_log:ets2dcd(Tab),
    Fname = mnesia2_lib:tab2dat(Tab),
    case mnesia2_lib:exists(Fname) of
...
handle_last({ram_copies, Tab}, _Type, DatBin) ->
	...
	true ->
	    ...
	    ok = file:rename(Tmp, mnesia2_lib:tab2dcd(Tab)),
	    mnesia2_lib:unlock_table(Tab),
	    ok;
	false ->
	    ok
...
down(Tab, Storage) ->
	...
	disc_copies ->
	    ?SAFE(?ets_delete_table(Tab));
	disc_only_copies ->
	    TmpFile = mnesia2_lib:tab2tmp(Tab),
	    mnesia2_lib:dets_sync_close(Tab),
	    file:delete(TmpFile)
    end,
    mnesia2_checkpoint:tm_del_copy(Tab, node()),
    mnesia2_controller:sync_del_table_copy_whereabouts(Tab, node()),
    mnesia2_tm:unblock_tab(Tab),
    flush_subcrs(),
    down.
...
calc_nokeys(Storage, Tab) ->
    ...
    Key = mnesia2_lib:db_first(Storage, Tab),
    Recs = mnesia2_lib:db_get(Storage, Tab, Key),
...
send_table(Pid, Tab, RemoteS) ->
	...
	Storage ->
	    %% Send first
	    TabSize = mnesia2:table_info(Tab, size),
	    ...
	    %% Debug info
	    put(mnesia2_table_sender, {Tab, node(Pid), Pid}),
...
prepare_copy(Pid, Tab, Storage) ->
    Trans =
	fun() ->
		mnesia2:lock_table(Tab, load),
		mnesia2_subscr:subscribe(Pid, {table, Tab}),
		update_where_to_write(Tab, node(Pid)),
		mnesia2_lib:db_fixtable(Storage, Tab, true),
		ok
	end,
    mnesia2:transaction(Trans).
...
update_where_to_write([H|T], Tab, AddNode) ->
    rpc:call(H,  mnesia2_controller, call,
...
reader_funcs(UseDetsChunk, Tab, Storage, KeysPerTransfer) ->
	...
	false ->
	    {fun() -> mnesia2_lib:db_init_chunk(Storage, Tab, KeysPerTransfer) end,
	     fun(Cont) -> mnesia2_lib:db_chunk(Storage, Cont) end};
	true ->
...
compression_level() ->
	...
	{'EXIT', _} ->
	    mnesia2_lib:set(send_compressed, NoCompression),
	    NoCompression;
	Val -> Val
    end.
...
finish_copy(Pid, Tab, Storage, RemoteS) ->
    ...
    Trans =
	fun() ->
		mnesia2:read_lock_table(Tab),
		A = val({Tab, access_mode}),
		mnesia2_controller:sync_and_block_table_whereabouts(Tab, RecNode, RemoteS, A),
		cleanup_tab_copier(Pid, Storage, Tab),
		mnesia2_checkpoint:tm_add_copy(Tab, RecNode),
...
cleanup_tab_copier(Pid, Storage, Tab) ->
    ...
    mnesia2_subscr:unsubscribe(Pid, {table, Tab}).

dat2bin(Tab, ram_copies, ram_copies) ->
    mnesia2_lib:lock_table(Tab),
    Res = file:read_file(mnesia2_lib:tab2dcd(Tab)),
    mnesia2_lib:unlock_table(Tab),
    case Res of
	{ok, DatBin} -> DatBin;
	_ -> nobin
