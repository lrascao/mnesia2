...
init(Parent) ->
    ...
    ?ets_new_table(mnesia2_held_locks, [ordered_set, private, named_table]),
    ?ets_new_table(mnesia2_tid_locks, [ordered_set, private, named_table]),
    ?ets_new_table(mnesia2_sticky_locks, [set, private, named_table]),
    ?ets_new_table(mnesia2_lock_queue, [bag, private, named_table, {keypos, 2}]),
...
loop(State) ->
	...
	{From, {test_set_sticky, Tid, {Tab, _} = Oid, Lock}} ->
	    case ?ets_lookup(mnesia2_sticky_locks, Tab) of
		[] ->
...
set_lock(Tid, Oid, Op, []) ->
    ?ets_insert(mnesia2_tid_locks, {{Tid, Oid, Op}}),
    ?ets_insert(mnesia2_held_locks, {Oid, Op, [{Op, Tid}]});
set_lock(Tid, Oid, read, [{Oid, Prev, Items}]) ->
    ?ets_insert(mnesia2_tid_locks, {{Tid, Oid, read}}),
    ?ets_insert(mnesia2_held_locks, {Oid, Prev, [{read, Tid}|Items]});
set_lock(Tid, Oid, write, [{Oid, _Prev, Items}]) ->
    ?ets_insert(mnesia2_tid_locks, {{Tid, Oid, write}}),
    ?ets_insert(mnesia2_held_locks, {Oid, write, [{write, Tid}|Items]});
set_lock(Tid, Oid, Op, undefined) ->
    set_lock(Tid, Oid, Op, ?ets_lookup(mnesia2_held_locks, Oid)).
...
try_lock(Tid, Op, SimpleOp, Lock, Pid, Oid) ->
	...
	{{queue, Lucky},_} ->
	    ...
	    ?ets_insert(mnesia2_lock_queue,
			#queue{oid = Oid, tid = Tid, op = Op,
			       pid = Pid, lucky = Lucky}),
	    ?ets_insert(mnesia2_tid_locks, {{Tid, Oid, {queued, Op}}})
    end.
...
grant_lock(Tid, read, Lock, Oid = {Tab, Key}, Default)
  when Key /= ?ALL, Tab /= ?GLOBAL ->
	...
	false ->
	    try
		Val = mnesia2_lib:db_get(Tab, Key), %% lookup as well
		set_lock(Tid, Oid, Lock, Default),
		{granted, Val}
	    catch _:_Reason ->
...
can_lock(Tid, read, Oid = {Tab, Key}, AlreadyQ) when Key /= ?ALL ->
    ObjLocks = ?ets_lookup(mnesia2_held_locks, Oid),
    TabLocks = ?ets_lookup(mnesia2_held_locks, {Tab, ?ALL}),
    {check_lock(Tid, Oid,
...
can_lock(Tid, read, Oid, AlreadyQ) -> % Whole tab
    Tab = element(1, Oid),
    ObjLocks = ?ets_match_object(mnesia2_held_locks, {{Tab, '_'}, write, '_'}),
...
can_lock(Tid, write, Oid, AlreadyQ) -> % Whole tab
    Tab = element(1, Oid),
    ObjLocks = ?ets_match_object(mnesia2_held_locks, ?match_oid_held_locks({Tab, '_'})),
...
check_lock(Tid, Oid = {Tab, Key}, [], [], X, AlreadyQ, Type) ->
	...
	true ->
	    ...
	    ObjLocks = ets:lookup(mnesia2_lock_queue, Oid),
...
can_queue([{Op, WaitForTid}|Locks], Tid, Oid = {Tab, _}, _) ->
	...
	true when Tid#tid.pid == WaitForTid#tid.pid ->
	    ...
	    HaveQ = (ets:lookup(mnesia2_lock_queue, Oid) /= [])
		orelse (ets:lookup(mnesia2_lock_queue,{Tab,?ALL}) /= []),
	    case HaveQ of
		true -> {no, WaitForTid};
...
check_queue(Tid, Tab, X, AlreadyQ) ->
    TabLocks = ets:lookup(mnesia2_lock_queue, {Tab,?ALL}),
...
release_remote_non_pending(Node, Pending) ->
    ...
    ?ets_match_delete(mnesia2_sticky_locks, {'_' , Node}),
    ...
    AllTids0 = ?ets_match(mnesia2_tid_locks, {{'$1', '_', '_'}}),
...
do_release_tid(Tid) ->
    Objects = ets:select(mnesia2_tid_locks, [{{{Tid, '_', '_'}}, [], ['$_']}]),
    Locks = lists:map(fun({L}) -> L end, Objects),
    ?dbg("Release ~p ~p ~n", [Tid, Locks]),
    [?ets_delete(mnesia2_tid_locks, L) || L <- Locks],
    release_locks(Locks),
...
release_lock({Tid, Oid, {queued, _}}) ->
    ?ets_match_delete(mnesia2_lock_queue, #queue{oid=Oid, tid = Tid, op = '_',
						pid = '_', lucky = '_'});
release_lock({_Tid, Oid, write}) ->
    ?ets_delete(mnesia2_held_locks, Oid);
release_lock({Tid, Oid, read}) ->
    case ?ets_lookup(mnesia2_held_locks, Oid) of
	[{Oid, Prev, Locks0}] ->
	    case remove_tid(Locks0, Tid, []) of
		[] -> ?ets_delete(mnesia2_held_locks, Oid);
		Locks -> ?ets_insert(mnesia2_held_locks, {Oid, Prev, Locks})
	    end;
	[] -> ok
    end.
...
rearrange_queue([{_Tid, {Tab, Key}, _} | Locks]) ->
	...
	true ->
	    ...
	    Queue = ?ets_match_object(mnesia2_lock_queue, Pat),
	    Sorted = sort_queue(Queue),
...
try_waiter(Oid, Op, SimpleOp, Lock, ReplyTo, Tid) ->
	...
	{yes, Default} ->
	    ...
	    ?ets_match_delete(mnesia2_lock_queue,
			      #queue{oid=Oid, tid = Tid, op = Op,
				     pid = ReplyTo, lucky = '_'}),
	    Reply = grant_lock(Tid, SimpleOp, Lock, Oid, Default),
	    reply(ReplyTo,Reply),
...
ix_read_res(Tab,IxKey,Pos) ->
    Index = mnesia2_index:get_index_table(Tab, Pos),
    Rks = mnesia2_lib:elems(2,mnesia2_index:db_get(Index, IxKey)),
    lists:append(lists:map(fun(Real) -> mnesia2_lib:db_get(Tab, Real) end, Rks)).
...
check_majority(Tab, HaveNs) ->
	...
	true ->
	    case mnesia2_lib:have_majority(Tab, HaveNs) of
		true ->
		    ok;
		false ->
		    mnesia2:abort({no_majority, Tab})
	    end;
	_ ->
	    ok
    end.
...
sticky_check_majority(W, Tab, true, WNodes) when W==write; W==read_write ->
    case mnesia2_lib:have_majority(Tab, WNodes) of
	true ->
	    ok;
	false ->
	    mnesia2:abort({no_majority, Tab})
...
add_debug(Nodes) ->  % Use process dictionary for debug info
    put(mnesia2_wlock_nodes, Nodes).

del_debug() ->
    erase(mnesia2_wlock_nodes).
...
opt_lookup_in_client(lookup_in_client, Oid, Lock) ->
    {Tab, Key} = Oid,
    try mnesia2_lib:db_get(Tab, Key)
    catch error:_ ->
	    %% Table has been deleted from this node,
	    %% restart the transaction.
...
dirty_rpc(nowhere, Tab, Key, _Lock) ->
    mnesia2:abort({no_exists, {Tab, Key}});
...
dirty_rpc(Node, Tab, Key, Lock) ->
    Args = [Tab, Key],
    case rpc:call(Node, mnesia2_lib, db_get, Args) of
	{badrpc, Reason} ->
	    case val({Tab, where_to_read}) of
		Node ->
...
get_held_locks() ->
    ?MODULE ! {get_table, self(), mnesia2_held_locks},
    Locks = receive {mnesia2_held_locks, Ls} -> Ls after 5000 -> [] end,
    rewrite_locks(Locks, []).
...
get_lock_queue() ->
    ?MODULE ! {get_table, self(), mnesia2_lock_queue},
    Q = receive {mnesia2_lock_queue, Locks} -> Locks after 5000 -> [] end,
