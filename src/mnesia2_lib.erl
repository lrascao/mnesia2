...
all_nodes() ->
    Ns = mnesia2:system_info(db_nodes) ++
...
is_running() ->
    case ?catch_val(mnesia2_status) of
	{'EXIT', _} -> no;
	running -> yes;
	starting -> starting;
	stopping -> stopping
...
dir() -> mnesia2_monitor:get_env(dir).
...
set(Var, Val) ->
    ?ets_insert(mnesia2_gvar, {Var, Val}).

unset(Var) ->
    ?ets_delete(mnesia2_gvar, Var).
...
pr_other(Var) ->
    ...
    verbose("~p (~p) val(mnesia2_gvar, ~w) -> ~p ~n",
...
set_remote_where_to_read(Tab, Ignore) ->
	...
	case mnesia2_recover:get_master_nodes(Tab) of
	    [] ->  Active;
	    Masters -> mnesia2_lib:intersect(Masters, Active)
	end,    
    Available = mnesia2_lib:intersect(val({current, db_nodes}), Valid -- Ignore),    
...
set_counter(Name, Val) ->
    ?ets_insert(mnesia2_stats, {Name, Val}).

incr_counter(Name) ->
    ?ets_update_counter(mnesia2_stats, Name, 1).

incr_counter(Name, I) ->
    ?ets_update_counter(mnesia2_stats, Name, I).

%% update_counter(Name, Val) ->
%%     ?ets_update_counter(mnesia2_stats, Name, Val).

read_counter(Name) ->
    ?ets_lookup_element(mnesia2_stats, Name, 2).
...
coredump(CrashInfo) ->
    ...
    Out = core_file(),
    important("Writing Mnesia2 core to file: ~p...~p~n", [Out, CrashInfo]),
...
core_file() ->
    ...
    case mnesia2_monitor:get_env(core_dir) of
	Dir when is_list(Dir) ->
	    filename:absname(lists:concat(["Mnesia2Core.", node()] ++ List), Dir);
	_ ->
	    filename:absname(lists:concat(["Mnesia2Core.", node()] ++ List))
    end.
...
%   dbg_out("Making a Mnesia2 core dump...~p~n", [CrashInfo]),
    ...
    HeldLocks = (catch mnesia2:system_info(held_locks)),
    Core = [
	    CrashInfo,
	    {time, {date(), time()}},
...
read_log_files() ->
    [{F, catch file:read_file(F)} || F <- mnesia2_log:log_files()].

dir_info() ->
    {ok, Cwd} = file:get_cwd(),
    Dir = dir(),
    [{cwd, Cwd, file:read_file_info(Cwd)},
     {mnesia2_dir, Dir, file:read_file_info(Dir)}] ++
    case file:list_dir(Dir) of
	{ok, Files} ->
	    [{mnesia2_file, F, catch file:read_file_info(dir(F))} || F <- Files];
	Other ->
	    [Other]
    end.
...
view(File) ->
	...
	false ->
		...
		true ->
		    view(File, log);
		false ->
		    case lists:prefix("Mnesia2Core.", File) of
			true ->
			    view(File, core);
			false ->
...
vcore_elem({logfile, {ok, BinList}}) ->
    Fun = fun({F, Info}) ->
		  ...
		  case Info of
		      {ok, B} ->
			  Fname = "/tmp/mnesia2_vcore_elem.TMP",
			  file:write_file(Fname, B),
			  mnesia2_log:view(Fname),
			  file:delete(Fname);
		      _ ->
			  show("~p~n", [Info])
		  end
...
vcore_elem({transactions, Info}) ->
    mnesia2_tm:display_info(user, Info);
...
error_desc(mnesia2_down) -> "A transaction involving objects at some remote "
...
fatal(Format, Args) ->
    ?SAFE(catch set(mnesia2_status, stopping)),
...
report_fatal(Format, Args, Core) ->
    report_system_event({mnesia2_fatal, Format, Args, Core}),
    ?SAFE(exit(whereis(mnesia2_monitor), fatal)).
...
report_system_event(Event0) ->
    Event = {mnesia2_system_event, Event0},
...
catch_notify(Event) ->
    case whereis(mnesia2_event) of
	undefined ->
	    {'EXIT', {badarg, {mnesia2_event, Event}}};
	Pid ->
	    gen_event:notify(Pid, Event)
    end.

report_system_event({'EXIT', Reason}, Event) ->
    Mod = mnesia2_monitor:get_env(event_module),
    case mnesia2_sup:start_event() of
	{ok, Pid} ->
	    link(Pid),
	    gen_event:call(mnesia2_event, Mod, Event, infinity),
	    unlink(Pid),

            %% We get an exit signal if server dies
            receive {'EXIT', Pid, _Reason} -> ok
            after 0 -> gen_event:stop(mnesia2_event)
            end;

	Error ->
	    Msg = "Mnesia2(~p): Cannot report event ~p: ~p (~p)~n",
...
important(Format, Args) ->
    save({Format, Args}),
    report_system_event({mnesia2_info, Format, Args}).

%% Warning messages are reported regardless of debug level
warning(Format, Args) ->
    save({Format, Args}),
    report_system_event({mnesia2_warning, Format, Args}).

%% error messages are reported regardless of debug level
error(Format, Args) ->
    save({Format, Args}),
    report_system_event({mnesia2_error, Format, Args}).

%% verbose messages are reported if debug level == debug or verbose
verbose(Format, Args) ->
    case mnesia2_monitor:get_env(debug) of
...
dbg_out(Format, Args) ->
	...
	_ ->  report_system_event({mnesia2_info, Format, Args})
    end.
...
lock_table(Tab) ->
    global:set_lock({{mnesia2_table_lock, Tab}, self()}, [node()], infinity).
%    dbg_out("dets_sync_open: ~p ~p~n", [T, self()]),

unlock_table(Tab) ->
    global:del_lock({{mnesia2_table_lock, Tab}, self()}, [node()]).
