...
verify_header([H | RawSchema]) when is_record(H, log_header) ->
    Current = mnesia2_log:backup_log_header(),
    if
...
refresh_cookie(Schema, NewCookie) ->
    case lists:keysearch(schema, 2, Schema) of
        {value, {schema, schema, List}} ->
            Cs = mnesia2_schema:list2cs(List),
            Cs2 = Cs#cstruct{cookie = NewCookie},
            Item = {schema, schema, mnesia2_schema:cs2list(Cs2)},
...
convert_schema(Latest, Schema) ->
    H = mnesia2_log:backup_log_header(),
    if
...
convert_0_1(Schema) ->
    case lists:keysearch(schema, 2, Schema) of
        ...
        false ->
            List = mnesia2_schema:get_initial_schema(disc_copies, [node()]),
            Cs = mnesia2_schema:list2cs(List),
...
schema2bup({schema, Tab, TableDef}) ->
    {schema, Tab, mnesia2_schema:cs2list(TableDef)}.
...
create_schema(Ns) when is_list(Ns) ->
    case is_set(Ns) of
        true ->
            create_schema(Ns, mnesia2_schema:ensure_no_schema(Ns));
        false ->
    ...
    case mnesia2_lib:ensure_loaded(?APPLICATION) of
        ok ->
            case mnesia2_monitor:get_env(schema_location) of
                ...
                _ ->
                    case mnesia2_schema:opt_create_dir(true, mnesia2_lib:dir()) of
                        {error, What} ->
                            {error, What};
                        ok ->
                            Mod = mnesia2_backup,
                            Str = mk_str(),
                            File = mnesia2_lib:dir(Str),
                            file:delete(File),
...
make_initial_backup(Ns, Opaque, Mod) ->
    Orig = mnesia2_schema:get_initial_schema(disc_copies, Ns),
    ...
    O3 = do_apply(Mod, write, [O2, [mnesia2_log:backup_log_header()]], O2),
...
check_fallback_arg_type(Arg, FA) ->
    case Arg of
        ...
        {module, Mod} ->
            Mod2 = mnesia2_monitor:do_check_type(backup_module, Mod),
            FA#fallback_args{module = Mod2};
        {mnesia2_dir, Dir} ->
            FA#fallback_args{mnesia2_dir = Dir,
...
restore_recs(Recs, Header, Schema, {start, FA}) ->
    ...
    try mnesia2_schema:list2cs(CreateList) of
        Cs ->
            Ns = get_fallback_nodes(FA, Cs#cstruct.disc_copies),
            global:set_lock({{mnesia2_table_lock, schema}, self()}, Ns, infinity),
            Args = [self(), FA],
            ...
            global:del_lock({{mnesia2_table_lock, schema}, self()}, Ns),
            Res
    catch _:Reason ->
...
rec_answers([], Acc) ->
    case {lists:keysearch(error, 1, Acc), mnesia2_lib:uniq(Acc)} of
...
fallback_bup() -> mnesia2_lib:dir(fallback_name()).
...
check_fallback_dir_arg(FA) ->
    case FA#fallback_args.use_default_dir of
        ...
        false when FA#fallback_args.scope =:= local ->
            Dir = FA#fallback_args.mnesia2_dir,
            try mnesia2_monitor:do_check_type(dir, Dir)
	    catch _:_ ->
...
fallback_receiver_loop(Master, R, FA, State) ->
    receive
        {Master, {start, Header, Schema}} when State =:= schema ->
            Dir = FA#fallback_args.mnesia2_dir,
            throw_bad_res(ok, mnesia2_schema:opt_create_dir(true, Dir)),
        ...
        {Master, swap} when State =/= schema ->
            ...
            ?SAFE(mnesia2_lib:set(active_fallback, true)),
...
tm_fallback_start(IgnoreFallback) ->
    mnesia2_schema:lock_schema(),
    Res = do_fallback_start(fallback_exists(), IgnoreFallback),
    mnesia2_schema:unlock_schema(),
    case Res of
        ok -> ok;
...
do_fallback_start(true, true) ->
    ...
    mnesia2_lib:set(active_fallback, true),
    ok;
do_fallback_start(true, false) ->
    ...
    Mod = mnesia2_backup,
    LocalTabs = ?ets_new_table(mnesia2_local_tables, [set, public, {keypos, 2}]),
    case iterate(Mod, fun restore_tables/4, BupFile, {start, LocalTabs}) of
        {ok, _Res} ->
            ?SAFE(dets:close(schema)),
            TmpSchema = mnesia2_lib:tab2tmp(schema),
            DatSchema = mnesia2_lib:tab2dat(schema),
...
restore_tables(Recs, Header, Schema, {start, LocalTabs}) ->
    ...
    mnesia2_schema:purge_dir(OldDir, []),
    mnesia2_schema:purge_dir(Dir, [fallback_name()]),
...
init_dat_files(Schema, LocalTabs) ->
    TmpFile = mnesia2_lib:tab2tmp(schema),
...
create_dat_files([{schema, Tab, TabDef} | Tail], LocalTabs) ->
    ...
    DatFile = mnesia2_lib:tab2dat(Tab),
	...
	Storage =:= unknown ->
        ...
        Storage =:= disc_only_copies ->
            ...
            Open = fun(T, LT) when T =:= LT#local_tab.name ->
                           case mnesia2_lib:dets_sync_open(T, Args) of
                               {ok, _} ->
                                   ok;
                               {error, Reason} ->
	    ...
	    Swap = fun(T, LT) when T =:= LT#local_tab.name ->
			   ...
			   case file:rename(TmpFile, DatFile) of
				   ...
				   ok;
			       {error, Reason} ->
				   mnesia2_lib:fatal("Cannot rename file ~p -> ~p: ~p~n",
	    ...
	    Open = fun(T, LT) when T =:= LT#local_tab.name ->
            ...
            Add = fun(T, Key, Rec, LT) when T =:= LT#local_tab.name ->
			  ...
			  case Rec of
			      {_T, Key} ->
				  mnesia2_log:append(Log, {{T, Key}, {T, Key}, delete});
			      (Rec) when T =:= RecName ->
				  mnesia2_log:append(Log, {{T, Key}, Rec, write});
			      (Rec) ->
				  Rec2 = setelement(1, Rec, RecName),
				  mnesia2_log:append(Log, {{T, Key}, Rec2, write})
			  end
		  end,
            Close = fun(T, LT) when T =:= LT#local_tab.name ->
			    mnesia2_log:close_log({?MODULE, T})
                    end,
	    Swap = fun(T, LT) when T =:= LT#local_tab.name ->
			   Expunge(),
			   if
				   ...
				   ok;
			       true ->
				   Log = mnesia2_log:open_log(fallback_tab,
							     mnesia2_log:dcd_log_header(),
							     DcdFile,
							     false),
				   mnesia2_log:close_log(Log),
				   case LT#local_tab.opened of
				       true ->
					   Close(T,LT);
				       false ->
...
local_uninstall_fallback(Master, FA) ->
    ...
    register(mnesia2_fallback, self()),        % May exit
    ...
    receive
        {Master, do_uninstall} ->
            ...
            ?SAFE(mnesia2_lib:set(active_fallback, false)),
            Tmp = FA2#fallback_args.fallback_tmp,
            Bup = FA2#fallback_args.fallback_bup,
            file:delete(Tmp),
            Res = file:delete(Bup),
...
traverse_backup(Source, Target, Fun, Acc) ->
    Mod = mnesia2_monitor:get_env(backup_module),
