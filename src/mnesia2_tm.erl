...
init(Parent) ->
    ...
    IgnoreFallback = mnesia2_monitor:get_env(ignore_fallback_at_startup),
    mnesia2_bup:tm_fallback_start(IgnoreFallback),
    mnesia2_schema:init(IgnoreFallback),

    %% Handshake and initialize transaction recovery
    mnesia2_recover:init(),
    Early = mnesia2_monitor:init(),
    AllOthers = mnesia2_lib:uniq(Early ++ mnesia2_lib:all_nodes()) -- [node()],
    set(original_nodes, AllOthers),
    mnesia2_recover:connect_nodes(AllOthers),

    %% Recover transactions, may wait for decision
    case mnesia2_monitor:use_dir() of
	true ->
	    P = mnesia2_dumper:opt_dump_log(startup), % previous log
	    L = mnesia2_dumper:opt_dump_log(startup), % latest log
	    Msg = "Initial dump of log during startup: ~p~n",
	    mnesia2_lib:verbose(Msg, [[P, L]]),
	    mnesia2_log:init();
	false ->
	    ignore
    end,

    mnesia2_schema:purge_tmp_files(),
    mnesia2_recover:next_garb(),
    mnesia2_recover:next_check_overload(),
	...
	Debug when Debug /= debug, Debug /= trace ->
	    ignore;
	_ ->
	    mnesia2_subscr:subscribe(whereis(mnesia2_event), {table, schema})
    end,
...
doit_loop(#state{coordinators=Coordinators,participants=Participants,supervisor=Sup}=State0) ->
	...
	{From, start_outer} -> %% Create and associate ets_tab with Tid
	    try ?ets_new_table(mnesia2_trans_store, [bag, public]) of
		Etab ->
		    tmlink(From),
		    C = mnesia2_recover:incr_trans_tid_serial(),
	...
	{From, {ask_commit, Protocol, Tid, Commit, DiscNs, RamNs}} ->
	    ...
	    mnesia2_checkpoint:tm_enter_pending(Tid, DiscNs, RamNs),
	    Pid =
		case Protocol of
	...
	{sync_trans_serial, Tid} ->
	    ...
	    mnesia2_recover:sync_trans_tid_serial(Tid),
	    doit_loop(State);
	...
	{mnesia2_up, Node} ->
	    verbose("Got mnesia2_up from ~p, starting async_dirty senders ...~n", [Node]),
	    lists:foreach(fun (I) ->
				   Sender = node_num_to_async_dirty_tm_sender_name(Node, I),
				   mnesia2_monitor:start_proc(Sender, ?MODULE,
				   							 init_async_dirty_tm_sender,
				   							 [Node, I, self()])
			  end,
	...
	{mnesia2_down, N} ->
	    verbose("Got mnesia2_down from ~p, reconfiguring...~n", [N]),
	    lists:foreach(fun (I) ->
				   Sender = node_num_to_async_dirty_tm_sender_name(N, I),
 				   case whereis(Sender) of
 				       Pid when is_pid(Pid) ->
 				        %% original patch had a prepend option
					   	%%		erlang:send(Pid, {mnesia2_down, N}, [prepend]);
					   	%% this was a modification made to the erlang:send/3
					   	%% method, 646c210d on erts/emulator/beam/bif.c
					   	%% this is a rather nice optimization which allows a
					   	%% sender to put a message at the top of the process's
					   	%% private queue instead of the end
					   	%% regrettably here we we'll to do with out it
					   	erlang:send(Pid, {mnesia2_down, N}, []);
 				       _ -> skip
 				   end
			  end,
			  lists:seq(1, ?NUM_ASYNC_DIRTY_TM_SENDER)),
	    ...
	    mnesia2_locker:mnesia2_down(N, Tids),
	    mnesia2_monitor:mnesia2_down(?MODULE, N),
	    doit_loop(NewState);
	...
	{From, {prepare_checkpoint, Cp}} ->
	    Res = mnesia2_checkpoint:tm_prepare(Cp),
	    case Res of
	...
	{From, {fixtable, [Tab,Lock,Requester]}} ->
		...
		Storage ->
		    mnesia2_lib:db_fixtable(Storage,Tab,Lock),
...
prepare_pending_coordinators([{Tid, [Store | _Etabs]} | Coords], IgnoreNew) ->
	...
	[Pending] ->
		...
		false ->
		    mnesia2_checkpoint:tm_enter_pending(Pending);
		true ->
		    ignore
	    end,
	    prepare_pending_coordinators(Coords, IgnoreNew)
...
prepare_pending_participants([Part | Parts], IgnoreNew) ->
    ...
    case lists:member(Tid, IgnoreNew) of
	false ->
	    mnesia2_checkpoint:tm_enter_pending(Tid, D, R);
	true ->
	    ignore
    end,
...
handle_exit(Pid, Reason, State) ->
	...
	{none, _} ->
	    ...
	    case mnesia2_lib:key_search_delete(Pid,#participant.pid,Ps) of
		{none, _} ->
...
recover_coordinator(Tid, asym_trans, committed, Local, DiscNs, RamNs) ->
    ...
    mnesia2_recover:log_decision(D),
    do_commit(Tid, Local);
...
clear_fixtable(Node, State=#state{fixed_tabs = FT0}) ->
    case mnesia2_lib:key_search_delete(Node, 1, FT0) of
	{none, _Ft} ->
	    State;
	{{Node,Tabs},FT} ->
	    lists:foreach(
...
transaction_terminated(Tid)  ->
    mnesia2_checkpoint:tm_exit_pending(Tid),
    Pid = Tid#tid.pid,
    if
	node(Pid) == node() ->
	    unlink(Pid);
...
non_transaction(OldState, Fun, Args, ActivityKind, Mod) ->
    ...
    put(mnesia2_activity_state, NewState),
    try apply(Fun, Args) of
	{'EXIT', Reason} -> exit(Reason);
	{aborted, Reason} -> mnesia2:abort(Reason);
	Res -> Res
    catch
    ...
    after
	case OldState of
	    undefined -> erase(mnesia2_activity_state);
	    _ -> put(mnesia2_activity_state, OldState)
	end
    end.
...
transaction(OldTidTs, Fun, Args, Retries, Mod, Type) ->
	...
	{_, _, non_transaction} -> % Transaction inside ?sync_dirty
	    ...
	    put(mnesia2_activity_state, OldTidTs),
	    Res;
...
execute_outer(Mod, Fun, Args, Factor, Retries, Type) ->
	...
	{new_tid, Tid, Store} ->
	    ...
	    NewTidTs = {Mod, Tid, Ts},
	    put(mnesia2_activity_state, NewTidTs),
...
execute_transaction(Fun, Args, Factor, Retries, Type) ->
	...
	{atomic, Value} ->
	    mnesia2_lib:incr_counter(trans_commits),
...
restart(Mod, Tid, Ts, Fun, Args, Factor0, Retries0, Type, Why) ->
    mnesia2_lib:incr_counter(trans_restarts),
    Retries = decr(Retries0),
    case Why of
	{bad_commit, _N} ->
	    return_abort(Fun, Args, Why),
	    Factor = 1,
	    SleepTime = mnesia2_lib:random_time(Factor, Tid#tid.counter),
	...
	{node_not_running, _N} ->   %% Avoids hanging in receive_release_tid_ack
	    ...
	    execute_outer(Mod, Fun, Args, Factor, Retries, Type);
	_ ->
	    SleepTime = mnesia2_lib:random_time(Factor0, Tid#tid.counter),
	    dbg_out("Restarting transaction ~w: in ~wms ~w~n", [Tid, SleepTime, Why]),
...
return_abort(Fun, Args, Reason)  ->
    ...
    intercept_friends(Tid, Ts),
    ?SAFE(mnesia2_lib:incr_counter(trans_failures)),
	...
	Level == 1 ->
	    ...
	    flush_downs(),
	    ?SAFE(unlink(whereis(?MODULE))),
	    {aborted, mnesia2_lib:fix_error(Reason)};
	true ->
	    %% Nested transaction
...
put_activity_id(SimpleState,_) ->
    put(mnesia2_activity_state, SimpleState).
...
do_arrange(Tid, Store, RestoreKey, Prep, N) when RestoreKey == restore_op ->
    ...
    Recs2 = mnesia2_schema:arrange_restore(R, Fun, Prep#prep.records),
...
have_majority([{Tab, AllNodes} | Rest], Nodes) ->
    case mnesia2_lib:have_majority(Tab, AllNodes, Nodes) of
	true ->
...
prepare_snmp(Tab, Key, Items) ->
	...
	Ustruct when Key /= '_' ->
	    ...
	    SnmpOid = mnesia2_snmp_hook:key_to_oid(Tab, Key, Ustruct), % May exit
	    [{Op, Tab, Key, SnmpOid}];
	_ ->
	    [{clear_table, Tab}]
    end.
...
prepare_snmp(Tid, Tab, Key, Types, Us, Items, Recs) ->
    if Key /= '_' ->
	    {_Oid, _Val, Op} = hd(Items),
	    SnmpOid = mnesia2_snmp_hook:key_to_oid(Tab, Key, Us), % May exit
...
pick_node(_Tid, Node, [], _Done) ->
    mnesia2:abort({bad_commit, {missing_lock, Node}}).
...
multi_commit(sym_trans, _Maj = [], Tid, CR, Store) ->
    ...
    {DiscNs, RamNs} = commit_nodes(CR, [], []),
    Pending = mnesia2_checkpoint:tm_enter_pending(Tid, DiscNs, RamNs),
...
multi_commit(asym_trans, Majority, Tid, CR, Store) ->
	...
	ok  -> ok;
	{error, Tab} -> mnesia2:abort({no_majority, Tab})
    end,
    Pending = mnesia2_checkpoint:tm_enter_pending(Tid, DiscNs, RamNs),
    ?ets_insert(Store, Pending),
    {WaitFor, Local} = ask_commit(asym_trans, Tid, CR2, DiscNs, RamNs),
    SchemaPrep = ?CATCH(mnesia2_schema:prepare_commit(Tid, Local, {coord, WaitFor})),
...
rec_acc_pre_commit([], Tid, Store, {Commit,OrigC}, Res, DumperMode, GoodPids, SchemaAckPids) ->
	...
	do_commit ->
	    ...
	    mnesia2_recover:log_decision(D2),
...
commit_participant(Coord, Tid, Bin, C0, DiscNs, _RamNs) ->
    ...
    try mnesia2_schema:prepare_commit(Tid, C0, {part, Coord}) of
	{Modified, C = #commit{}, DumperMode} ->
		...
		true ->
		    case Modified of
			false -> mnesia2_log:log(Bin);
			true  -> mnesia2_log:log(C)
		    end
	    end,
		...
		{Tid, pre_commit} ->
		    D = C#commit.decision,
		    mnesia2_recover:log_decision(D#decision{outcome = unclear}),
		    ...
		    receive
			{Tid, committed} ->
			    mnesia2_recover:log_decision(D#decision{outcome = committed}),
			...
			{Tid, {do_abort, _Reason}} ->
			    mnesia2_recover:log_decision(D#decision{outcome = aborted}),
...
do_abort(Tid, Commit) ->
    mnesia2_schema:undo_prepare_commit(Tid, Commit),
    Commit.
...
do_commit(Tid, C, DumperMode) ->
    ...
    mnesia2_subscr:report_activity(Tid),
    R4.
...
do_update_op(Tid, Storage, {{Tab, Key}, Obj, delete_object}) ->
    ...
    mnesia2_lib:db_match_erase(Storage, Tab, Obj);
...
commit_write([{checkpoints, CpList}|R], Tid, Tab, K, Obj, Old) ->
    mnesia2_checkpoint:tm_retain(Tid, Tab, K, write, CpList),
    commit_write(R, Tid, Tab, K, Obj, Old);
commit_write([H|R], Tid, Tab, K, Obj, Old)
  when element(1, H) == subscribers ->
    mnesia2_subscr:report_table_event(H, Tab, Tid, Obj, write, Old),
    commit_write(R, Tid, Tab, K, Obj, Old);
commit_write([H|R], Tid, Tab, K, Obj, Old)
  when element(1, H) == index ->
    mnesia2_index:add_index(H, Tab, K, Obj, Old),
    commit_write(R, Tid, Tab, K, Obj, Old).

commit_update([], _, _, _, _, _) -> ok;
commit_update([{checkpoints, CpList}|R], Tid, Tab, K, Obj, _) ->
    Old = mnesia2_checkpoint:tm_retain(Tid, Tab, K, write, CpList),
...
commit_delete([{checkpoints, CpList}|R], Tid, Tab, K, Obj, _) ->
    Old = mnesia2_checkpoint:tm_retain(Tid, Tab, K, delete, CpList),
    commit_delete(R, Tid, Tab, K, Obj, Old);
commit_delete([H|R], Tid, Tab, K, Obj, Old)
  when element(1, H) == subscribers ->
    mnesia2_subscr:report_table_event(H, Tab, Tid, Obj, delete, Old),
    commit_delete(R, Tid, Tab, K, Obj, Old);
commit_delete([H|R], Tid, Tab, K, Obj, Old)
  when element(1, H) == index ->
    mnesia2_index:delete_index(H, Tab, K),
    commit_delete(R, Tid, Tab, K, Obj, Old).

commit_del_object([], _, _, _, _, _) -> ok;
commit_del_object([{checkpoints, CpList}|R], Tid, Tab, K, Obj, _) ->
    Old = mnesia2_checkpoint:tm_retain(Tid, Tab, K, delete_object, CpList),
    commit_del_object(R, Tid, Tab, K, Obj, Old);
commit_del_object([H|R], Tid, Tab, K, Obj, Old)
  when element(1, H) == subscribers ->
    mnesia2_subscr:report_table_event(H, Tab, Tid, Obj, delete_object, Old),
    commit_del_object(R, Tid, Tab, K, Obj, Old);
commit_del_object([H|R], Tid, Tab, K, Obj, Old)
  when element(1, H) == index ->
    mnesia2_index:del_object_index(H, Tab, K, Obj, Old),
    commit_del_object(R, Tid, Tab, K, Obj, Old).

commit_clear([], _, _, _, _) ->  ok;
commit_clear([{checkpoints, CpList}|R], Tid, Tab, K, Obj) ->
    mnesia2_checkpoint:tm_retain(Tid, Tab, K, clear_table, CpList),
    commit_clear(R, Tid, Tab, K, Obj);
commit_clear([H|R], Tid, Tab, K, Obj)
  when element(1, H) == subscribers ->
    mnesia2_subscr:report_table_event(H, Tab, Tid, Obj, clear_table, undefined),
    commit_clear(R, Tid, Tab, K, Obj);
commit_clear([H|R], Tid, Tab, K, Obj)
  when element(1, H) == index ->
    mnesia2_index:clear_index(H, Tab, K, Obj),
    commit_clear(R, Tid, Tab, K, Obj).

do_snmp(_, []) ->   ok;
do_snmp(Tid, [Head | Tail]) ->
    try mnesia2_snmp_hook:update(Head)
    catch _:Reason ->
...
init_async_dirty_tm_sender_loop(Node, Num, Parent) ->
	...
	ok ->
		...
		true ->
		    verbose("~p: mnesia2_tm async_dirty sender registered (~p)~n", [Sender, self()]),
	    	    async_dirty_sender_loop(Node, Num, Parent, []);
		{'EXIT', Reason} ->
		    case whereis(Sender) of
			Pid when is_pid(Pid) ->
			    verbose("~p: mnesia2_tm async_dirty sender already running (~p)~n", [Sender, Pid]),
			    unlink(Parent),
			    exit(shutdown);
			_ ->
			    verbose("~p: mnesia2_tm async_dirty sender registration failure: ~p~n", [Sender, Reason]),
...
check_remote_tm(Node) ->
	...
	_:Err ->
	    verbose("** ERROR ** Possible OTP version mismatch: remote mnesia2_tm not responding (node=~p): ~1000p~n", [Node, Err]),
	    {error, Err}
    end.
...
reconfigure_coordinators(N, [{Tid, [Store | _]} | Coordinators]) ->
    case mnesia2_recover:outcome(Tid, unknown) of
	committed ->
	    WaitingNodes = ?ets_lookup(Store, waiting_for_commit_ack),
	    case lists:keymember(N, 2, WaitingNodes) of
		false ->
		    ignore; % avoid spurious mnesia2_down messages
		true ->
		    send_mnesia2_down(Tid, Store, N)
	    end;
	aborted ->
	    ignore; % avoid spurious mnesia2_down messages
	_ ->
...
%% about the outcome.
tell_outcome(Tid, Protocol, Node, CheckNodes, TellNodes) ->
    Outcome = mnesia2_recover:what_happened(Tid, Protocol, CheckNodes),
    case Outcome of
	aborted ->
	    rpc:abcast(TellNodes, ?MODULE, {Tid,{do_abort, {mnesia2_down, Node}}});
	committed ->
