...
start() ->
    mnesia2_monitor:start_proc(?SERVER_NAME, ?MODULE, init, [self()]).

init(Parent) ->
    %% Trap exit omitted intentionally
    register(?SERVER_NAME, self()),
    link(whereis(mnesia2_controller)),  %% We may not hang
    mnesia2_controller:merge_schema(),
    unlink(whereis(mnesia2_controller)),
    mnesia2_lib:set(mnesia2_status, running),
...
loop(State) ->
	...
	{_From, {async_late_disc_load, Tabs, Reason}} ->
	    mnesia2_controller:schedule_late_disc_load(Tabs, Reason),
	    loop(State);
	...
	{_From, {maybe_async_late_disc_load, Tabs, Reason}} ->
	    ...
	    mnesia2_controller:schedule_late_disc_load(GoodTabs, Reason),
	    loop(State);

	{system, From, Msg} ->
	    mnesia2_lib:dbg_out("~p got {system, ~p, ~p}~n",
			       [?SERVER_NAME, From, Msg]),
	    Parent = State#state.supervisor,
	    sys:handle_system_msg(Msg, From, Parent, ?MODULE, [], State);
	    
	Msg ->
	    mnesia2_lib:error("~p got unexpected message: ~p~n",
			     [?SERVER_NAME, Msg]),
	    loop(State)
    end.
