...
handle_system_event({mnesia2_down, Node}, State) ->
    case mnesia2:system_info(fallback_activated) andalso Node =/= node() of
	true ->
	    case mnesia2_monitor:get_env(fallback_error_function) of
		{mnesia2, lkill} ->
		    Msg = "A fallback is installed and Mnesia2 "
			"must be restarted. Forcing shutdown "
			"after mnesia2_down from ~p...~n",
		    report_fatal(Msg, [Node], nocore, State#state.dumped_core),
		    catch exit(whereis(mnesia2_monitor), fatal),
		    {ok, State};
		{UserMod, UserFunc} ->
		    Msg = "Warning: A fallback is installed and Mnesia2 got mnesia2_down "
			"from ~p. ~n",
...
handle_system_event({inconsistent_database, Reason, Node}, State) ->
    report_error("mnesia2_event got {inconsistent_database, ~w, ~w}~n",
		 [Reason, Node]),
    {ok, State}; 
...
handle_system_event(Msg, State) ->
    report_error("mnesia2_event got unexpected system event: ~p~n", [Msg]),
    {ok, State}.
...
report_warning(Format0, Args0) ->
    Format = "Mnesia2(~p): ** WARNING ** " ++ Format0,
    Args = [node() | Args0],
...
report_error(Format0, Args0) ->
    Format = "Mnesia2(~p): ** ERROR ** " ++ Format0,
    Args = [node() | Args0],
    error_logger:format(Format, Args),
    case global:whereis_name(mnesia2_global_logger) of
	undefined ->
...
report_fatal(Format, Args, BinaryCore, CoreDumped) ->
    ...
    CoreDir = mnesia2_monitor:get_env(core_dir),
    if
