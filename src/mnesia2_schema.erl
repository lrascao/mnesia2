...
init(IgnoreFallback) ->
    ...
    mnesia2_controller:add_active_replica(schema, node()).
...
val(Var) ->
	...
	{'EXIT', _} -> mnesia2_lib:other_val(Var);
	Value -> Value
    end.
...
do_set_schema(Tab, Cs) ->
    ...
    set({Tab, all_nodes}, mnesia2_lib:cs_to_nodes(Cs)),
    ...
    mnesia2_frag:set_frag_hash(Tab, Cs#cstruct.frag_properties),
    ...
    set({Tab, cstruct}, Cs),
    Storage = mnesia2_lib:schema_cs_to_storage_type(node(), Cs),
    set({Tab, storage_type}, Storage),
    mnesia2_lib:add({schema, tables}, Tab),
    Ns = mnesia2_lib:cs_to_nodes(Cs),
    case lists:member(node(), Ns) of
        true ->
            mnesia2_lib:add({schema, local_tables}, Tab);
...
read_nodes() ->
    ...
    case mnesia2_lib:ensure_loaded(?APPLICATION) of
	ok ->
...
version() ->
    case read_schema(false) of
	    ...
	    Cs = list2cs(CreateList),
            ...
            Version;
        _ ->
            case dir_exists(mnesia2_lib:dir()) of
                true -> {1,0};
                false -> {0,0}
            end
    end.
...
incr_version(Cs) ->
    ...
    Nodes = mnesia2_lib:intersect(val({schema, disc_copies}),
...
insert_cstruct(Tid, Cs, KeepWhereabouts) ->
    ...
    mnesia2_checkpoint:tm_retain(Tid, schema, Tab, write),
    mnesia2_subscr:report_table_event(schema, Tid, Val, write),
...
delete_cstruct(Tid, Cs) ->
    ...
    TabDef = cs2list(Cs),
    Val = {schema, Tab, TabDef},
    mnesia2_checkpoint:tm_retain(Tid, schema, Tab, delete),
    mnesia2_subscr:report_table_event(schema, Tid, Val, delete),
    mnesia2_controller:update(
      fun() ->
	      ?ets_match_delete(mnesia2_gvar, {{Tab, '_'}, '_'}),
	      ?ets_match_delete(mnesia2_gvar, {{Tab, '_', '_'}, '_'}),
...
delete_schema(Ns) when is_list(Ns), Ns /= [] ->
    RunningNs = mnesia2_lib:running_nodes(Ns),
    ...
    if
        ...
        true ->
            ...
            {error, {"Mnesia2 is not stopped everywhere", RunningNs}}
    end;
delete_schema(Ns) ->
    {error, {badarg, Ns}}.
...
delete_schema2() ->
    ...
    case mnesia2_lib:ensure_loaded(?APPLICATION) of
	ok ->
	    case mnesia2_lib:is_running() of
		no ->
		    Dir = mnesia2_lib:dir(),
		    purge_dir(Dir, []),
		    ok;
		_ ->
		    {error, {"Mnesia2 still running", node()}}
	    end;
	{error, Reason} ->
	    {error, Reason}
    end.
...
remote_read_schema() ->
    ...
    case mnesia2_lib:ensure_loaded(?APPLICATION) of
	ok ->
	    case mnesia2_monitor:get_env(schema_location) of
		opt_disc ->
		    read_schema(false);
		_ ->
		    read_schema(false)
...
dir_exists(Dir) ->
    dir_exists(Dir, mnesia2_monitor:use_dir()).
dir_exists(Dir, true) ->
        ...
        false ->
            case file:make_dir(Dir) of
                ok ->
                    ...
                    ok;
                {error, Reason} ->
                    verbose("Cannot create mnesia2 dir ~p~n", [Reason]),
                    {error, {"Cannot create Mnesia2 dir", Dir, Reason}}
            end
    end;
...
check_can_write(Dir) ->
    case file:read_file_info(Dir) of
        {ok, FI} when FI#file_info.type == directory,
            ...
            ok;
        {ok, _} ->
            {error, "Not allowed to write in Mnesia2 dir", Dir};
        _ ->
            {error, "Non existent Mnesia2 dir", Dir}
    end.

lock_schema() ->
    mnesia2_lib:lock_table(schema).

unlock_schema() ->
    mnesia2_lib:unlock_table(schema).
...
read_disc_schema(Keep, IgnoreFallback) ->
    Running = mnesia2:system_info(is_running),
    case mnesia2_bup:fallback_exists() of
        true when IgnoreFallback == false, Running /= yes ->
             mnesia2_bup:fallback_to_schema();
        _ ->
            %% If we're running, we read the schema file even
            %% if fallback exists
            Dat = mnesia2_lib:tab2dat(schema),
            case mnesia2_lib:exists(Dat) of
                true ->
                    do_read_disc_schema(Dat, Keep);
                false ->
		    Dmp = mnesia2_lib:tab2dmp(schema),
		    case mnesia2_lib:exists(Dmp) of
			true ->
...
read_cstructs_from_disc() ->
    ...
    Fname = mnesia2_lib:tab2dat(schema),
    case mnesia2_lib:exists(Fname) of
	true ->
	    Args = [{file, Fname},
		    {keypos, 2},
		    {repair, mnesia2_monitor:get_env(auto_repair)},
...
get_tid_ts_and_lock(Tab, Intent) ->
    TidTs = get(mnesia2_activity_state),
    case TidTs of
	{_Mod, Tid, Ts} when is_record(Ts, tidstore)->
	    Store = Ts#tidstore.store,
	    case Intent of
		read -> mnesia2_locker:rlock_table(Tid, Store, Tab);
		write -> mnesia2_locker:wlock_table(Tid, Store, Tab);
		none -> ignore
	    end,
	    TidTs;
	_ ->
	    mnesia2:abort(no_transaction)
    end.

schema_transaction(Fun) ->
    case get(mnesia2_activity_state) of
	undefined ->
	    Args = [self(), Fun, whereis(mnesia2_controller)],
...
schema_coordinator(Client, Fun, Controller) when is_pid(Controller) ->
    ...
    Res = mnesia2:transaction(Fun),
    Client ! {transaction_done, Res, self()},
    unlink(Controller),         % Avoids spurious exit message
    unlink(whereis(mnesia2_tm)), % Avoids spurious exit message
    exit(normal).
...
list2cs(List) when is_list(List) ->
    ...
    verify({alt, [nil, list]}, mnesia2_lib:etype(UserProps),
    ...
    CheckProp = fun(Opt, Opts) when is_atom(Opt) ->
			lists:member(Opt, Opts)
			    andalso mnesia2:abort({badarg, Name, Opt});
		   (Tuple, Opts) when is_tuple(Tuple) ->
			lists:member(element(1,Tuple), Opts)
			    andalso mnesia2:abort({badarg, Name, Tuple});
		   (What,_) ->
			mnesia2:abort({badarg, Name, What})
		end,
    BadEtsOpts = [set, ordered_set, bag, duplicate_bag,
		  public, private, protected,
		  keypos, named_table],
    EtsOpts = proplists:get_value(ets, BEProps, []),
    is_list(EtsOpts) orelse mnesia2:abort({badarg, Name, {ets, EtsOpts}}),
    ...
    is_list(DetsOpts) orelse mnesia2:abort({badarg, Name, {dets, DetsOpts}}),
...
pick(Tab, Key, List, Default) ->
    case lists:keysearch(Key, 1, List) of
        false  when Default == must ->
            mnesia2:abort({badarg, Tab, "Missing key", Key, List});
        false ->
            Default;
        {value, {Key, Value}} ->
            Value;
	{value, BadArg} ->
	    mnesia2:abort({bad_type, Tab, BadArg})
    end.
...
check_keys(Tab, [{Key, _Val} | Tail], Items) ->
    case lists:member(Key, Items) of
        ...
        false -> mnesia2:abort({badarg, Tab, Key})
    end;
check_keys(_, [], _) ->
    [];
check_keys(Tab, Arg, _) ->
    mnesia2:abort({badarg, Tab, Arg}).

check_duplicates(Tab, Keys) ->
    case has_duplicates(Keys) of
        false -> ok;
        true -> mnesia2:abort({badarg, Tab, "Duplicate keys", Keys})
    end.
...
verify_cstruct(Cs) when is_record(Cs, cstruct) ->
    ...
    Tab = Cs#cstruct.name,
    verify(atom, mnesia2_lib:etype(Tab), {bad_type, Tab}),
    Type = Cs#cstruct.type,
    ...
    if
 	Type == ordered_set, Cs#cstruct.disc_only_copies /= [] ->
	    mnesia2:abort({bad_type, Tab, {not_supported, Type, disc_only_copies}});
	true ->
	    ok
    end,

    RecName = Cs#cstruct.record_name,
    verify(atom, mnesia2_lib:etype(RecName),
	   {bad_type, Tab, {record_name, RecName}}),

    Attrs = Cs#cstruct.attributes,
    verify(list, mnesia2_lib:etype(Attrs),
    ...
    lists:foldl(fun(Attr,_Other) when Attr == snmp ->
                        mnesia2:abort({bad_type, Tab, {attributes, [Attr]}});
                   (Attr,Other) ->
                        verify(atom, mnesia2_lib:etype(Attr),
    ...
    verify({alt, [nil, list]}, mnesia2_lib:etype(Index),
    ...
    verify(true, mnesia2_snmp_hook:check_ustruct(Snmp),
	   {badarg, Tab, {snmp, Snmp}}),

    CheckProp = fun(Prop) when is_tuple(Prop), size(Prop) >= 1 -> ok;
		   (Prop) -> mnesia2:abort({bad_type, Tab, {user_properties, [Prop]}})
		end,
	...
	when is_integer(MegaSecs), is_integer(Secs),
	     is_integer(MicroSecs), is_atom(node) ->
            ok;
        Cookie ->
            mnesia2:abort({bad_type, Tab, {cookie, Cookie}})
    end,
    case Cs#cstruct.version of
        {{Major, Minor}, _Detail}
            ...
            ok;
        Version ->
            mnesia2:abort({bad_type, Tab, {version, Version}})
    end.
...
verify_nodes(Cs) ->
    ...
    verify({alt, [nil, list]}, mnesia2_lib:etype(Ram),
	   {bad_type, Tab, {ram_copies, Ram}}),
    verify({alt, [nil, list]}, mnesia2_lib:etype(Disc),
    ...
    verify(integer, mnesia2_lib:etype(LoadOrder),
	   {bad_type, Tab, {load_order, LoadOrder}}),

    Nodes = Ram ++ Disc ++ DiscOnly,
    verify(list, mnesia2_lib:etype(Nodes),
	   {combine_error, Tab,
	    [{ram_copies, []}, {disc_copies, []}, {disc_only_copies, []}]}),
    verify(false, has_duplicates(Nodes), {combine_error, Tab, Nodes}),
    AtomCheck = fun(N) -> verify(atom, mnesia2_lib:etype(N), {bad_type, Tab, N}) end,
...
ensure_active(Cs, What) ->
    ...
    case Nodes -- val(W) of
        ...
        Ns ->
			...
			{_Replies, BadNs} ->
			    mnesia2:abort({not_active, Expl, Tab, BadNs})
                    end;
                false ->
                    mnesia2:abort({not_active, Expl, Tab, Ns})
            end
    end.
...
ensure_not_active(Tab = schema, Node) ->
	...
	false ->
	    mnesia2:abort({no_exists, Tab});
	true ->
	    Expl = "Mnesia2 is running",
	    mnesia2:abort({active, Expl, Node})
    end.
...
check_active([{false, Node} | _Replies], Expl, Tab) ->
    mnesia2:abort({not_active, Expl, Tab, [Node]});
check_active([{badrpc, Reason} | _Replies], Expl, Tab) ->
    mnesia2:abort({not_active, Expl, Tab, Reason});
...
do_multi_create_table(TabDef) ->
	...
	[] ->
	    do_create_table(Cs);
	_Props ->
	    CsList = mnesia2_frag:expand_cstruct(Cs),
...
unsafe_make_create_table(Cs) ->
    ...
    Nodes = mnesia2_lib:intersect(mnesia2_lib:cs_to_nodes(Cs), RunningNodes),
    Store = Ts#tidstore.store,
    mnesia2_locker:wlock_no_exist(Tid, Store, Tab, Nodes),
	...
	true ->
		...
		whole_table ->
			...
			_Props ->
			    ...
			    mnesia2_frag:lookup_frag_hash(Tab),

			    %% Check for foreigners
			    F = mnesia2_frag:lookup_foreigners(Tab),
			    verify([], F, {combine_error,
					   Tab, "Too many foreigners", F}),
			    [make_delete_table2(T) ||
				T <- mnesia2_frag:frag_names(Tab)]
		    end;
		single_frag ->
		    [make_delete_table2(Tab)]
	    end
...
do_change_table_frag(Tab, Change) when is_atom(Tab), Tab /= schema ->
    TidTs = get_tid_ts_and_lock(schema, write),
    Ops = mnesia2_frag:change_table_frag(Tab, Change),
    [insert_schema_ops(TidTs, Op) || Op <- Ops],
    ok;
do_change_table_frag(Tab, _Change) ->
    mnesia2:abort({bad_type, Tab}).
...
make_add_table_copy(Tab, Node, Storage) ->
	...
	Tab == schema ->
	    if
		Storage /= ram_copies ->
		    mnesia2:abort({badarg, Tab, Storage});
		IsRunning == true ->
		    mnesia2:abort({already_exists, Tab, Node});
		true ->
		    ignore
	    end;
	Storage == ram_copies ->
	...
	IsRunning == true ->
	    ignore;
	IsRunning == false ->
	    mnesia2:abort({not_active, schema, Node})
    end,
...
make_del_table_copy(Tab, Node) ->
    ...
    case mnesia2_lib:cs_to_nodes(Cs2) of
        [] when Tab == schema ->
            mnesia2:abort({combine_error, Tab, "Last replica"});
        [] ->
	    ensure_active(Cs),
...
remove_node_from_tabs([Tab|Rest], Node) ->
    ...
    case mnesia2_lib:schema_cs_to_storage_type(Node, Cs)  of
	unknown ->
...
new_cs(Cs, _Node, Storage, _Op) ->
    mnesia2:abort({badarg, Cs#cstruct.name, Storage}).
...
make_move_table(Tab, FromNode, ToNode) ->
    ...
    Storage = mnesia2_lib:schema_cs_to_storage_type(FromNode, Cs),
...
make_change_table_copy_type(Tab, Node, ToS) ->
    ensure_writable(schema),
    Cs = incr_version(val({Tab, cstruct})),
    FromS = mnesia2_lib:storage_type_at_node(Node, Tab),

    case compare_storage_type(false, FromS, ToS) of
	{same, _} ->
	    mnesia2:abort({already_exists, Tab, Node, ToS});
	{diff, _} ->
	    ignore;
	incompatible ->
	    ensure_active(Cs)
...
make_add_snmp(Tab, Ustruct) ->
    ...
    verify(true, mnesia2_snmp_hook:check_ustruct(Ustruct), Error),
...
make_transform(Tab, Fun, NewAttrs, NewRecName) ->
    ...
    ensure_active(Cs),
    ensure_writable(Tab),
    case mnesia2_lib:val({Tab, index}) of
	[] ->
...
do_change_table_access_mode(Tab, Mode) ->
    ...
    Store = Ts#tidstore.store,
    mnesia2_locker:wlock_no_exist(Tid, Store, schema, val({schema, active_replicas})),
    mnesia2_locker:wlock_no_exist(Tid, Store, Tab, val({Tab, active_replicas})),
...
make_change_table_majority(Tab, Majority) ->
    ...
    FragOps = case lists:keyfind(base_table, 1, Cs#cstruct.frag_properties) of
		  {_, Tab} ->
		      FragNames = mnesia2_frag:frag_names(Tab) -- [Tab],
		      lists:map(
			fun(T) ->
				get_tid_ts_and_lock(Tab, none),
		  ...
		  false    -> [];
		  {_, _}   -> mnesia2:abort({bad_type, Tab})
	      end,
    verify_cstruct(Cs2),
	...
	OrigOps ->
		...
		optional ->
		    dbg_out("Transaction log dump skipped (~p): ~w~n",
			    [DumperMode, InitBy]);
		mandatory ->
		    case mnesia2_controller:sync_dump_log(InitBy) of
			dumped ->
			    GoodRes;
			{error, Reason} ->
		...
		_ ->
		    ...
		    mnesia2_controller:wait_for_schema_commit_lock()
	    end,
	    GoodRes
    end.
...
prepare_op(_Tid, {op, rec, unknown, Rec}, _WaitFor) ->
    ...
    case val({Tab, storage_type}) of
        unknown ->
            {false, optional};
        Storage ->
            mnesia2_tm:prepare_snmp(Tab, Key, Items), % May exit
...
prepare_op(_Tid, {op, announce_im_running, Node, SchemaDef, Running, RemoteRunning}, _WaitFor) ->
	...
	true ->
	    ...
	    Current  = mnesia2_lib:intersect(val({current,db_nodes}), [node()|val(recover_nodes)]),
	    NewNodes = mnesia2_lib:uniq(Running++RemoteRunning) -- Current,
	    mnesia2_lib:set(prepare_op, {announce_im_running,NewNodes}),
...
prepare_op(_Tid, {op, sync_trans}, {part, CoordPid}) ->
	...
	{mnesia2_down, _Node} = Else ->
	    mnesia2_lib:verbose("sync_op terminated due to ~p~n", [Else]),
	    mnesia2:abort(Else);
	{'EXIT', _, _} = Else ->
	    mnesia2_lib:verbose("sync_op terminated due to ~p~n", [Else]),
	    mnesia2:abort(Else)
...
prepare_op(_Tid, {op, sync_trans}, {coord, Nodes}) ->
	...
	{abort, Reason} ->
	    mnesia2_lib:verbose("sync_op terminated due to ~p~n", [Reason]),
	    mnesia2:abort(Reason);
	Pids ->
...
prepare_op(Tid, {op, create_table, TabDef}, _WaitFor) ->
    Cs = list2cs(TabDef),
    Storage = mnesia2_lib:cs_to_storage_type(node(), Cs),
    UseDir = mnesia2_monitor:use_dir(),
    Tab = Cs#cstruct.name,
    case Storage of
        disc_copies when UseDir == false ->
	    UseDirReason = {bad_type, Tab, Storage, node()},
            mnesia2:abort(UseDirReason);
        disc_only_copies when UseDir == false ->
	    UseDirReason = {bad_type, Tab, Storage, node()},
            mnesia2:abort(UseDirReason);
	ram_copies ->
	    mnesia2_lib:set({Tab, create_table},true),
...
prepare_op(Tid, {op, add_table_copy, Storage, Node, TabDef}, _WaitFor) ->
	...
	Node == node() ->
	    case mnesia2_lib:val({schema, storage_type}) of
	    ...
	    case mnesia2_controller:get_network_copy(Tab, Cs) of
		{loaded, ok} ->
		    {true, optional};
	...
	Node /= node() ->
		...
		Storage /= ram_copies ->
		    case mnesia2_lib:schema_cs_to_storage_type(node(), Cs) of
			ram_copies ->
			    Dat = mnesia2_lib:tab2dcd(Tab),
			    case mnesia2_lib:exists(Dat) of
				true ->
				    mnesia2:abort({combine_error, Tab, Storage,
...
prepare_op(_Tid, {op, change_table_copy_type,  N, FromS, ToS, TabDef}, _WaitFor)
  when N == node() ->
    ...
    NotActive = mnesia2_lib:not_active_here(Tab),

    if
	NotActive == true ->
	    mnesia2:abort({not_active, Tab, node()});

	Tab == schema ->
	    case {FromS, ToS} of
		{ram_copies, disc_copies} ->
		    case mnesia2:system_info(schema_location) of
			opt_disc ->
			    ignore;
			_ ->
			    mnesia2:abort({combine_error,  Tab, node(),
			...
			ok ->
			    purge_dir(Dir, []),
			    mnesia2_log:purge_all_logs(),
			    set(use_dir, true),
			    mnesia2_log:init(),
			    Ns = val({current, db_nodes}), %mnesia2_lib:running_nodes(),
			    F = fun(U) -> mnesia2_recover:log_mnesia2_up(U) end,
			    lists:foreach(F, Ns),

			    mnesia2_dumper:raw_named_dump_table(Tab, dmp),
			    mnesia2_checkpoint:tm_change_table_copy_type(Tab, FromS, ToS);
			{error, Reason} ->
			    mnesia2:abort(Reason)
		    end;
		...
		_ ->
		    mnesia2:abort({combine_error, Tab, ToS})
	    end;
	...
	FromS == ram_copies ->
		...
		false ->
		    mnesia2:abort({has_no_disc, node()})
	    end;
	...
	FromS == disc_only_copies ->
	    Type = Cs#cstruct.type,
	    create_ram_table(Tab, Cs),
	    Datname = mnesia2_lib:tab2dat(Tab),
	    Repair = mnesia2_monitor:get_env(auto_repair),
	    case mnesia2_lib:dets_to_ets(Tab, Tab, Datname, Type, Repair, no) of
		loaded -> ok;
		Reason ->
		    Err = "Failed to copy disc data to ram",
		    mnesia2:abort({system_limit, Tab, {Err,Reason}})
	    end;
	true ->
	    ignore
    end,
...
prepare_op(_Tid, {op, dump_table, unknown, TabDef}, _WaitFor) ->
    ...
    case lists:member(node(), Cs#cstruct.ram_copies) of
		...
		true ->
		    mnesia2_log:ets2dcd(Tab, dmp),
		    Size = mnesia2:table_info(Tab, size),
...
prepare_op(_Tid, {op, add_snmp, Ustruct, TabDef}, _WaitFor) ->
    Cs = list2cs(TabDef),
    case mnesia2_lib:cs_to_storage_type(node(), Cs) of
        unknown ->
            {true, optional};
        Storage ->
            Tab = Cs#cstruct.name,
            Stab = mnesia2_snmp_hook:create_table(Ustruct, Tab, Storage),
            mnesia2_lib:set({Tab, {index, snmp}}, Stab),
            {true, optional}
    end;
...
prepare_op(_Tid, {op, transform, Fun, TabDef}, _WaitFor) ->
    ...
    case mnesia2_lib:cs_to_storage_type(node(), Cs) of
	    ...
	    mnesia2_lib:db_fixtable(Storage, Tab, true),
            Key = mnesia2_lib:db_first(Tab),
	    Op = {op, transform, Fun, TabDef},
            try transform_objs(Fun, Tab, RecName, Key,
			       NewArity, Storage, Type, [Op]) of
                Objs ->
		    mnesia2_lib:db_fixtable(Storage, Tab, false),
                    {true, Objs, mandatory}
	    catch _:Reason ->
		    mnesia2_lib:db_fixtable(Storage, Tab, false),
...
prepare_op(_Tid, {op, merge_schema, TabDef}, _WaitFor) ->
	...
	Error ->
	    ...
	    mnesia2:abort({bad_commit, Error})
    end;
...
create_ram_table(Tab, #cstruct{type=Type, storage_properties=Props}) ->
    ...
    case mnesia2_monitor:unsafe_mktab(Tab, Args) of
	Tab ->
	    ok;
	{error,Reason} ->
...
create_disc_table(Tab) ->
    File = mnesia2_lib:tab2dcd(Tab),
    file:delete(File),
    FArg = [{file, File}, {name, {mnesia2,create}},
	    {repair, false}, {mode, read_write}],
    case mnesia2_monitor:open_log(FArg) of
	{ok,Log} ->
	    mnesia2_monitor:unsafe_close_log(Log),
	    ok;
	{error,Reason} ->
...
create_disc_only_table(Tab, #cstruct{type=Type, storage_properties=Props}) ->
    File = mnesia2_lib:tab2dat(Tab),
    file:delete(File),
    DetsOpts = proplists:get_value(dets, Props, []),
    Args = [{file, mnesia2_lib:tab2dat(Tab)},
	    {type, mnesia2_lib:disk_type(Tab, Type)},
	    {keypos, 2},
	    {repair, mnesia2_monitor:get_env(auto_repair)}
	    | DetsOpts],
    case mnesia2_monitor:unsafe_open_dets(Tab, Args) of
	{ok, _} ->
	    ok;
	{error,Reason} ->
	    Err = "Failed to create disc table",
...
set_where_to_read(Tab, Node, Cs) ->
	...
	Node ->
		...
		true ->
		    ok;
		false ->
		    mnesia2_lib:set_remote_where_to_read(Tab, [Node]),
		    ok
	    end;
	_ ->
	    ok
...
transform_objs(Fun, Tab, RecName, Key, A, Storage, Type, Acc) ->
    Objs = mnesia2_lib:db_get(Tab, Key),
    NextKey = mnesia2_lib:db_next_key(Tab, Key),
    Oid = {Tab, Key},
...
undo_prepare_commit(Tid, Commit) ->
	...
	Ops ->
	    ...
	    ?SAFE(mnesia2_controller:release_schema_commit_lock()),
...
undo_prepare_op(Tid, {op, create_table, TabDef}) ->
    Cs = list2cs(TabDef),
    Tab = Cs#cstruct.name,
    mnesia2_lib:unset({Tab, create_table}),
    delete_cstruct(Tid, Cs),
    case mnesia2_lib:cs_to_storage_type(node(), Cs) of
	unknown ->
	    ok;
	...
	disc_copies ->
	    ...
	    DcdFile = mnesia2_lib:tab2dcd(Tab),
	    %%	    disc_delete_table(Tab, Storage),
	    file:delete(DcdFile);
	disc_only_copies ->
	    mnesia2_monitor:unsafe_close_dets(Tab),
	    Dat = mnesia2_lib:tab2dat(Tab),
	    %%	    disc_delete_table(Tab, Storage),
	    file:delete(Dat)
    end;
...
undo_prepare_op(Tid, {op, add_table_copy, Storage, Node, TabDef}) ->
	...
	Node == node() ->
	    mnesia2_checkpoint:tm_del_copy(Tab, Node),
	    mnesia2_controller:unannounce_add_table_copy(Tab, Node),
	    if
		Storage == disc_only_copies; Tab == schema ->
		    mnesia2_monitor:close_dets(Tab),
		    file:delete(mnesia2_lib:tab2dat(Tab));
		true ->
		    file:delete(mnesia2_lib:tab2dcd(Tab))
	    end,
...
undo_prepare_op(_Tid, {op, change_table_copy_type, N, FromS, ToS, TabDef})
    ...
    Cs = list2cs(TabDef),
    Tab = Cs#cstruct.name,
    mnesia2_checkpoint:tm_change_table_copy_type(Tab, ToS, FromS),
    Dmp = mnesia2_lib:tab2dmp(Tab),

    case {FromS, ToS} of
        {ram_copies, disc_copies} when Tab == schema ->
            file:delete(Dmp),
            mnesia2_log:purge_some_logs(),
	    set(use_dir, false);
	{ram_copies, disc_copies} ->
	    file:delete(Dmp);
...
undo_prepare_op(_Tid, {op, add_snmp, _Ustruct, TabDef}) ->
	...
	_Storage ->
		...
		{'EXIT',_} ->
		    ignore;
		Stab ->
		    mnesia2_snmp_hook:delete_table(Tab, Stab),
		    mnesia2_lib:unset({Tab, {index, snmp}})
	    end
    end;
...
ram_delete_table(Tab, Storage) ->
	...
	_Else ->
	    ...
	    ?CATCH(mnesia2_index:del_transient(Tab, Storage)),
	    case ?catch_val({Tab, {index, snmp}}) of
		{'EXIT', _} ->
		    ignore;
		Etab ->
		    ?SAFE(mnesia2_snmp_hook:delete_table(Tab, Etab))
	    end,
...
purge_tmp_files() ->
	...
	true ->
	    Dir = mnesia2_lib:dir(),
	    KeepFiles = [],
	    Exists = mnesia2_lib:exists(mnesia2_lib:tab2dat(schema)),
	    case Exists of
		true ->
...
get_table_properties(Tab) ->
    try mnesia2_lib:db_match_object(ram_copies, mnesia2_gvar, {{Tab, '_'}, '_'}) of
	RawGvar ->
	    case [{Item, Val} || {{_Tab, Item}, Val} <- RawGvar] of
		[] ->
		    [];
		Gvar ->
		    Size = {size, mnesia2:table_info(Tab, size)},
		    Memory = {memory, mnesia2:table_info(Tab, memory)},
		    Master = {master_nodes, mnesia2:table_info(Tab, master_nodes)},
		    lists:sort([Size, Memory, Master | Gvar])
	    end
    catch error:_ ->
	    mnesia2:abort({no_exists, Tab, all})
    end.
...
restore(Opaque) ->
    restore(Opaque, [], mnesia2_monitor:get_env(backup_module)).
restore(Opaque, Args) when is_list(Args) ->
    restore(Opaque, Args, mnesia2_monitor:get_env(backup_module));
...
restore(Opaque, Args, Module) when is_list(Args), is_atom(Module) ->
	...
	R when is_record(R, r) ->
	    case mnesia2_bup:read_schema(R#r.module, Opaque) of
		{error, Reason} ->
		    {aborted, Reason};
		BupSchema ->
...
arrange_restore(R, Fun, Recs) ->
    ...
    case mnesia2_bup:iterate(R#r.module, fun restore_items/4, R#r.opaque, R2) of
	{ok, R3} -> R3#r.recs;
	{error, Reason} -> mnesia2:abort(Reason)
    end.
...
restore_schema([{schema, Tab, List} | Schema], R) ->
	...
	recreate_tables ->
	    TidTs = case ?catch_val({Tab, cstruct}) of
			{'EXIT', _} ->
			    TTs = {_Mod, Tid, Ts} = get(mnesia2_activity_state),
			    RunningNodes = val({current, db_nodes}),
			    Nodes = mnesia2_lib:intersect(mnesia2_lib:cs_to_nodes(list2cs(List)),
							 RunningNodes),
			    mnesia2_locker:wlock_no_exist(Tid, Ts#tidstore.store, Tab, Nodes),
			    TTs;
			_ ->
			    get_tid_ts_and_lock(Tab, write)
		    end,
...
do_merge_schema(LockTabs0) ->
	...
	Miss -> %% Abort! We don't want the sideeffects below to be executed
	    mnesia2:abort({bad_commit, {missing_lock, Miss}})
    end,
    case Connected -- Running of
	[Node | _] = OtherNodes ->
	    %% Time for a schema merging party!
	    mnesia2_locker:wlock_no_exist(Tid, Store, schema, [Node]),
            [mnesia2_locker:wlock_no_exist(
		...
		{cstructs, Cstructs, RemoteRunning1} ->
		    ...
		    {New, Old} = mnesia2_recover:connect_nodes(RemoteRunning1),
		    RemoteRunning = mnesia2_lib:intersect(New ++ Old, RemoteRunning1),
		    if
			RemoteRunning /= RemoteRunning1 ->
			    mnesia2_lib:error("mnesia2 on ~p could not connect to node(s) ~p~n",
					     [node(), RemoteRunning1 -- RemoteRunning]),
			    mnesia2:abort({node_not_running, RemoteRunning1 -- RemoteRunning});
			true -> ok
		    end,
		    NeedsLock = RemoteRunning -- LockedAlready,
		    mnesia2_locker:wlock_no_exist(Tid, Store, schema, NeedsLock),
                    [mnesia2_locker:wlock_no_exist(Tid, Store, T,
...
fetch_cstructs(Node) ->
    case need_old_cstructs([Node]) of
	false ->
	    rpc:call(Node, mnesia2_controller, get_remote_cstructs, []);
	_Ver ->
	    case rpc:call(Node, mnesia2_controller, get_cstructs, []) of
		{cstructs, Cs0, RR} ->
...
need_old_cstructs(Nodes) ->
    Filter = fun(Node) -> not mnesia2_monitor:needs_protocol_conversion(Node) end,
    case lists:dropwhile(Filter, Nodes) of
	[] -> false;
	[Node|_] ->
	    case rpc:call(Node, mnesia2_lib, val, [{schema,cstruct}]) of
		#cstruct{} ->
...
do_make_merge_schema(Node, NeedsConv, RemoteCs = #cstruct{name = schema}) ->
    ...
    Masters = mnesia2_recover:get_master_nodes(schema),
    ...
    StCsLocal   = mnesia2_lib:cs_to_storage_type(node(), Cs),
    StRcsLocal  = mnesia2_lib:cs_to_storage_type(node(), RemoteCs),
    StCsRemote  = mnesia2_lib:cs_to_storage_type(Node, Cs),
    StRcsRemote = mnesia2_lib:cs_to_storage_type(Node, RemoteCs),
...
do_merge_cstructs(Cs, RemoteCs, Force) ->
    ...
    Ns = mnesia2_lib:uniq(mnesia2_lib:cs_to_nodes(Cs) ++
...
merge_storage_type([N | Ns], AnythingNew, Cs, RemoteCs, Force) ->
    Local = mnesia2_lib:cs_to_storage_type(N, Cs),
    Remote = mnesia2_lib:cs_to_storage_type(N, RemoteCs),
...
announce_im_running([N | Ns], SchemaCs) ->
    {L1, L2} = mnesia2_recover:connect_nodes([N]),
    case lists:member(N, L1) or lists:member(N, L2) of
	true ->
	    mnesia2_lib:add({current, db_nodes}, N),
	    mnesia2_controller:add_active_replica(schema, N, SchemaCs);
	false ->
	    mnesia2_lib:error("Mnesia2 on ~p could not connect to node ~p~n",
			     [node(), N]),
	    mnesia2:abort({node_not_running, N})
    end,
...
unannounce_im_running([N | Ns]) ->
    mnesia2_lib:del({current, db_nodes}, N),
    mnesia2_controller:del_active_replica(schema, N),
