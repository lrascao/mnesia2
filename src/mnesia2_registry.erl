...
create_table(Tab) ->
    Storage = mnesia2:table_info(schema, storage_type),
    create_table(Tab, [{Storage, [node()]}]).

create_table(Tab, TabDef) ->
    Attrs = record_info(fields, registry_entry),
    case mnesia2:create_table(Tab, [{attributes, Attrs} | TabDef]) of
	{atomic, ok} ->
	    ok;
...
dump_loop(S) ->
	...
	{commit, ReplyTo} ->
	    create_table(Tab),
	    RecName = mnesia2:table_info(Tab, record_name),
	    %% The Ops are in reverse order, but there is no need
	    %% for reversing the list of accumulated operations
	    case mnesia2:transaction(fun handle_ops/3, [Tab, RecName, Ops]) of
                {atomic, ok} ->
                    ReplyTo ! {ok, self()},
...
do_handle_ops(Tab, RecName, [{write, RegEntry} | Ops]) ->
    ...
    mnesia2:write(Tab, Record, write),
    do_handle_ops(Tab, RecName, Ops);
do_handle_ops(Tab, RecName, [{delete, Key} | Ops]) ->
    mnesia2:delete(Tab, Key, write),
    do_handle_ops(Tab, RecName, Ops);
...
restore_table(Tab, Starter, LinkTo) ->
    Pat = mnesia2:table_info(Tab, wild_pattern),
    Fun = fun() -> mnesia2:match_object(Tab, Pat, read) end,
