...
log(C) ->
    case mnesia2_monitor:use_dir() of
	    ...
	    mnesia2_dumper:incr_log_writes();
	false ->
	    ignore
    end.
...
%%    io:format("~p:open_log: ~p ~p~n", [?MODULE, Name, Fname]),
    case mnesia2_monitor:open_log(Args) of
	...
	{repaired, Log, _Recover, BadBytes} ->
	    mnesia2_lib:important("Data may be missing, log ~p repaired: Lost ~p bytes~n",
				 [Fname, BadBytes]),
	    Log;
	...
	{error, Reason} when Repair == true ->
	    file:delete(Fname),
	    mnesia2_lib:important("Data may be missing, Corrupt logfile deleted: ~p, ~p ~n",
				 [Fname, Reason]),
	    %% Create a new
...
%%    io:format("mnesia2_log:close_log ~p~n", [Log]),
	...
	{error, {read_only_mode, Log}} ->
	    ok;
	{error, Reason} ->
	    mnesia2_lib:important("Failed syncing ~p to_disk reason ~p ~n",
				 [Log, Reason])
    end,
    mnesia2_monitor:close_log(Log).
...
purge_some_logs() ->
    mnesia2_monitor:unsafe_close_log(latest_log),
...
prepare_log_dump(InitBy) ->
    Diff = mnesia2_dumper:get_log_writes() -
...
prepare_prev(Diff, _InitBy, Prev, false) ->
    ...
    case mnesia2_monitor:reopen_log(latest_log, Prev, Head) of
	ok ->
	    {needs_dump, Diff};
	{error, Reason} ->
	    Latest = latest_log_file(),
...
chunk_log(Log, Cont) ->
	...
	{C2, Chunk, _BadBytes} ->
	    ...
	    mnesia2_lib:important("~p repaired, lost ~p bad bytes~n", [Log, _BadBytes]),
	    {C2, Chunk};
	Other ->
	    Other
    end.
...
confirm_log_dump(Updates) ->
    case mnesia2_monitor:close_log(previous_log) of
	ok ->
	    file:delete(previous_log_file()),
	    mnesia2_lib:incr_counter(trans_log_writes_prev, Updates),
	    dumped;
	{error, Reason} ->
	    {error, Reason}
    end.
...
prepare_decision_log_dump(false, Prev) ->
    ...
    case mnesia2_monitor:reopen_log(decision_log, Prev, Head) of
	ok ->
...
confirm_decision_log_dump() ->
    case mnesia2_monitor:close_log(previous_decision_log) of
	ok ->
...
save_decision_tab(Decisions) ->
    Log = decision_tab,
    Tmp = mnesia2_lib:dir("DECISION_TAB.TMP"),
    file:delete(Tmp),
...
view_file(C, Log) ->
	...
	{C2, Terms, _BadBytes} ->
	    ...
	    lists:foreach(fun(X) -> mnesia2_lib:show("~p~n", [X]) end,
			  Terms),
	    view_file(C2, Log);
	{C2, Terms} ->
	    lists:foreach(fun(X) -> mnesia2_lib:show("~p~n", [X]) end,
...
backup(Opaque, Args) when is_list(Args) ->
    ...
    case mnesia2_checkpoint:activate(CpArgs) of
	{ok, Name, _Nodes} ->
...
backup_checkpoint(Name, Opaque, Args) when is_list(Args) ->
    DefaultMod = mnesia2_monitor:get_env(backup_module),
...
do_backup_master(B) ->
    ...
    case mnesia2_checkpoint:tables_and_cookie(Name) of
	{ok, AllTabs, Cookie} ->
...
select_tables(AllTabs, B) ->
	...
	local ->
	    Name = B#backup_args.name,
	    [T || T <- Tabs, mnesia2_checkpoint:most_local_node(Name, T) == {ok, node()}]
    end.
...
backup_schema(B, Tabs) ->
	...
	true ->
	    backup_tab(schema, B);
	false ->
	    Defs = [{schema, T, mnesia2_schema:get_create_list(T)} || T <- Tabs],
	    safe_write(B, Defs)
    end.
...
backup_tab(Tab, B) ->
    Name = B#backup_args.name,
    case mnesia2_checkpoint:most_local_node(Name, Tab) of
...
select_source(Tab, Name, PrevName) ->
	...
	true ->
	    ...
	    case mnesia2_checkpoint:most_local_node(PrevName, Tab) of
		{ok, Node} when Node == node() ->
...
handle_more(Pid, B, Tab, FirstName, FirstSource, Name) ->
    Acc = {0, B},
    case {mnesia2_checkpoint:really_retain(Name, Tab),
...
iterate(B, Name, Tab, Pid, Source, Age, Pass, Acc) ->
    ...
    case mnesia2_checkpoint:iterate(Name, Tab, Fun, Acc, Source, Age) of
	{ok, Acc2} ->
	    Acc2;
	{error, Reason} ->
...
rec_filter(B, schema, _RecName, Recs) ->
    try mnesia2_bup:refresh_cookie(Recs, B#backup_args.cookie)
...
ets2dcd(Tab, Ftype) ->
    ...
    TmpF = mnesia2_lib:tab2tmp(Tab),
    file:delete(TmpF),
    Log  = open_log({Tab, ets2dcd}, dcd_log_header(), TmpF, false),
    mnesia2_lib:db_fixtable(ram_copies, Tab, true),
    ok   = ets2dcd(mnesia2_lib:db_init_chunk(ram_copies, Tab, 1000), Tab, Log),
    mnesia2_lib:db_fixtable(ram_copies, Tab, false),
    close_log(Log),
...
ets2dcd({Recs, Cont}, Tab, Log) ->
    ...
    ets2dcd(mnesia2_lib:db_chunk(ram_copies, Cont), Tab, Log).

dcd2ets(Tab) ->
    dcd2ets(Tab, mnesia2_monitor:get_env(auto_repair)).
...
dcd2ets(Tab, Rep) ->
	...
	false -> %% Handle old dets files, and conversion from disc_only to disc.
	    ...
	    case mnesia2_lib:dets_to_ets(Tab, Tab, Fname, Type, Rep, yes) of
		loaded ->
		    ets2dcd(Tab),
		    file:delete(Fname),
		    0;
