...
do_check_overload(S) ->
    ...
    Pid when is_pid(Pid) ->
        ...
        Len > Threshold, Prev > Threshold ->
            ...
            mnesia2_lib:report_system_event({mnesia2_overload, What}),
            mnesia2_lib:overload_set(mnesia2_tm, true),
            S#state{tm_queue_len = 0};
        Len > Threshold ->
            S#state{tm_queue_len = Len};
        true ->
            mnesia2_lib:overload_set(mnesia2_tm, false),
            S#state{tm_queue_len = 0}
        end;
    undefined ->
        S
...
note_up(Node, _Date, _Time) ->
    ?ets_delete(mnesia2_decision, Node).
    
note_down(Node, Date, Time) ->
    ?ets_insert(mnesia2_decision, {mnesia2_down, Node, Date, Time}).
    
note_master_nodes(Tab, []) ->
    ?ets_delete(mnesia2_decision, Tab);
note_master_nodes(Tab, Nodes) when is_list(Nodes) ->
    Master = {master_nodes, Tab, Nodes},
    ?ets_insert(mnesia2_decision, Master).
...
%%    ?DBG("~w: note_tmp_decision: ~w~n", [node(), D]),
    ...
    ?ets_delete(mnesia2_decision, D#decision.tid);
note_outcome(D) when D#decision.disc_nodes /= [] ->
%%    ?DBG("~w: note_decision: ~w~n", [node(), D]),
    ?ets_insert(mnesia2_decision, D).
...
log_mnesia2_up(Node) ->
    call({log_mnesia2_up, Node}).

log_mnesia2_down(Node) ->
    call({log_mnesia2_down, Node}).

get_mnesia2_downs() ->
    Tab = mnesia2_decision,
    Pat = {mnesia2_down, '_', '_', '_'},
    Downs = ?ets_match_object(Tab, Pat),
    [Node || {mnesia2_down, Node, _Date, _Time} <- Downs].

%% Check if we have got a mnesia2_down from Node
has_mnesia2_down(Node) ->
    case ?ets_lookup(mnesia2_decision, Node) of
...
mnesia2_down(Node) ->
	...
	{'EXIT', _} ->
	    ...
	    ignore;
	_ ->
	    mnesia2_lib:del(recover_nodes, Node),
	    cast({mnesia2_down, Node})
    end.
...
log_master_nodes(Args, UseDir, IsRunning) ->
	...
	true ->
	    Name = latest_log,
	    Fname = mnesia2_log:latest_log_file(),
	    Exists = mnesia2_lib:exists(Fname),
	    Repair = mnesia2:system_info(auto_repair),
		...
		{repaired, Name, {recovered,  _R}, {badbytes, _B}}
		  when Exists == false ->
		    mnesia2_log:write_trans_log_header(),
		    log_master_nodes2(Args, UseDir, IsRunning, ok);
		{error, Reason} ->
		    {error, Reason}
	    end
...
log_master_nodes2([{Tab, Nodes} | Tail], UseDir, IsRunning, WorstRes) ->
	...
	case IsRunning of
		...
		mnesia2_controller:master_nodes_updated(Tab, Nodes),
		R;
	    _ ->
...
get_master_node_info() ->
    ...
    try mnesia2_lib:db_match_object(ram_copies,Tab, Pat)
    catch error:_ -> []
    end.
...
get_master_nodes(Tab) ->
    try ?ets_lookup_element(mnesia2_decision, Tab, 3)
    catch error:_ -> []
    end.
...
load_decision_tab() ->
    Cont = mnesia2_log:open_decision_tab(),
    load_decision_tab(Cont, load_decision_tab),
    mnesia2_log:close_decision_tab().

load_decision_tab(Cont, InitBy) ->
    case mnesia2_log:chunk_decision_tab(Cont) of
	{Cont2, Decisions} ->
...
convert_old() ->
    ...
    case HasOldStuff of
	true ->
	    mnesia2_log:open_decision_log(),
	    dump_decision_log(startup),
	    dump_decision_log(startup),
	    mnesia2_log:close_decision_log(),
	    Latest = mnesia2_log:decision_log_file(),
...
confirm_decision_log_dump() ->
    dump_decision_tab(),
    mnesia2_log:confirm_decision_log_dump().

dump_decision_tab() ->
    Tab = mnesia2_decision,
    All = mnesia2_lib:db_match_object(ram_copies,Tab, '_'),
    mnesia2_log:save_decision_tab({decision_list, All}).
...
note_log_decision(H, _InitBy) when H#log_header.log_kind == decision_log ->
    V = mnesia2_log:decision_log_version(),
    if
...
note_log_decision(H, _InitBy) when H#log_header.log_kind == decision_tab ->
    V = mnesia2_log:decision_tab_version(),
    if
...
trans_tid_serial() ->
    ?ets_lookup_element(mnesia2_decision, serial, 3).

set_trans_tid_serial(Val) ->
    ?ets_insert(mnesia2_decision, {trans_tid, serial, Val}).

incr_trans_tid_serial() ->
    ?ets_update_counter(mnesia2_decision, serial, 1).
...
init([Parent]) ->
    ...
    mnesia2_lib:verbose("~p starting: ~p~n", [?MODULE, self()]),
...
create_transient_decision() ->
    ?ets_new_table(mnesia2_transient_decision, [{keypos, 2}, set, public]).
...
handle_call(init, From, State) when State#state.initiated == false ->
    ...
    case mnesia2_monitor:use_dir() of
	true ->
	    ?ets_new_table(mnesia2_decision, Args),
	    set_trans_tid_serial(0),
	    TabFile = mnesia2_log:decision_tab_file(),
	    case mnesia2_lib:exists(TabFile) of
		true ->
		    load_decision_tab();
		false ->
		    ignore
	    ...
	    convert_old(),
	    mnesia2_dumper:opt_dump_log(scan_decisions);
	false ->
	    ?ets_new_table(mnesia2_decision, Args),
	    set_trans_tid_serial(0)
    end,
...
handle_call({disconnect, Node}, _From, State) ->
    mnesia2_monitor:disconnect(Node),
    mnesia2_lib:del(recover_nodes, Node),
    {reply, ok, State};
...
handle_call({connect_nodes, Ns}, From, State) ->
    ...
    {_, Nodes} = mnesia2_lib:search_delete(node(), Ns),
    Check = Nodes -- AlreadyConnected,
    case mnesia2_monitor:negotiate_protocol(Check) of
	busy -> 
...
handle_call({wait_for_decision, D}, From, State) ->
    ...
    AliveRam = (mnesia2_lib:intersect(D#decision.ram_nodes, Recov) -- [node()]),
	...
	true ->
	    ...
	    AliveDisc = mnesia2_lib:intersect(RemoteDisc, Recov),
...
do_log_mnesia2_up(Node) ->
    Yoyo = {mnesia2_up, Node, Date = date(), Time = time()},
    case mnesia2_monitor:use_dir() of
	true ->
	    mnesia2_log:append(latest_log, Yoyo),
...
do_log_mnesia2_down(Node) ->
    Yoyo = {mnesia2_down, Node, Date = date(), Time = time()},
    case mnesia2_monitor:use_dir() of
	true ->
	    mnesia2_log:append(latest_log, Yoyo),
	    disk_log:sync(latest_log);
...
do_log_master_nodes(Tab, Nodes, UseDir, IsRunning) ->
	...
	case UseDir of
	    true ->
		LogRes = mnesia2_log:append(latest_log, Master),
		disk_log:sync(latest_log),
		LogRes;
	    false  ->
		ok
...
handle_cast({decisions, Node, Decisions}, State) ->
    mnesia2_lib:add(recover_nodes, Node),
...
handle_cast({log_dump_overload, Flag}, State) when is_boolean(Flag) ->
    ...
    mnesia2_lib:overload_set(mnesia2_dump_log, Overload),
...
handle_info({'EXIT', Pid, R}, State) when Pid == State#state.supervisor ->
    mnesia2_lib:dbg_out("~p was ~p~n",[?MODULE, R]),
    {stop, shutdown, State};
...
terminate(Reason, State) ->
    mnesia2_monitor:terminate_proc(?MODULE, Reason, State).
...
tabs() ->
    ...
    [Curr, mnesia2_decision | Prev].           % Ordered by hit probability
...
%%			       [Node, NewD, D, OldD]),
		...
		NewD#decision.outcome == aborted ->
		    ...
		    mnesia2_lib:report_system_event(Msg),
