...
create_table([], Mnesia2Tab, _Storage) ->
    mnesia2:abort({badarg, Mnesia2Tab, {snmp, empty_snmpstruct}});

create_table([{key, Us}], Mnesia2Tab, Storage) ->
    Tree = b_new(Mnesia2Tab, Us),
    mnesia2_lib:db_fixtable(Storage, Mnesia2Tab, true),
    First = mnesia2_lib:db_first(Storage, Mnesia2Tab),
    build_table(First, Mnesia2Tab, Tree, Us, Storage),
    mnesia2_lib:db_fixtable(Storage, Mnesia2Tab, false),
    Tree.
    
build_table(Mnesia2Key, Mnesia2Tab, Tree, Us, Storage)
  when Mnesia2Key /= '$end_of_table' ->
    %%update(write, Tree, Mnesia2Key, Mnesia2Key),
    SnmpKey = key_to_oid_i(Mnesia2Key, Us),
    b_insert(Tree, SnmpKey, Mnesia2Key),
    Next = mnesia2_lib:db_next_key(Storage, Mnesia2Tab, Mnesia2Key), 
    build_table(Next, Mnesia2Tab, Tree, Us, Storage);
...
update({clear_table, Mnesia2Tab}) ->
    Tree = mnesia2_lib:val({Mnesia2Tab, {index, snmp}}),
    b_clear(Tree),
    ok;
...
oid_to_key(Oid, Tab) ->
    [{key, Types}] = mnesia2_lib:val({Tab,snmp}),
...
get_row(Name, RowIndex) ->
    Tree = mnesia2_lib:val({Name, {index, snmp}}),
    case b_lookup(Tree, RowIndex) of
	{ok, {_RowIndex, Key}} ->
	    [Row] = mnesia2:dirty_read({Name, Key}),
	    {ok, Row};
	_ ->
	    undefined
    end.
...
b_new(_Tab, _Us) ->
    mnesia2_monitor:unsafe_mktab(?MODULE, [public, ordered_set]).

b_delete_tree(Tree) ->
    ets:delete(Tree).  %% Close via mnesia2_monitor ?

b_clear(Tree) ->
    ets:delete_all_objects(Tree).

b_insert(Tree, SnmpKey, Mnesia2Key) ->
    ets:insert(Tree, {SnmpKey, Mnesia2Key}).
