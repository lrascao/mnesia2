...
lock(ActivityId, Opaque, {table , Tab}, LockKind) ->
	...
	[Tab] ->
	    mnesia2:lock(ActivityId, Opaque, {table, Tab}, LockKind);	    
	Frags ->
	    DeepNs = [mnesia2:lock(ActivityId, Opaque, {table, F}, LockKind) ||
			 F <- Frags],
	    mnesia2_lib:uniq(lists:append(DeepNs))
    end;

lock(ActivityId, Opaque, LockItem, LockKind) ->
    mnesia2:lock(ActivityId, Opaque, LockItem, LockKind).

write(ActivityId, Opaque, Tab, Rec, LockKind) ->
    Frag = record_to_frag_name(Tab, Rec),
    mnesia2:write(ActivityId, Opaque, Frag, Rec, LockKind).

delete(ActivityId, Opaque, Tab, Key, LockKind) ->
    Frag = key_to_frag_name(Tab, Key),
    mnesia2:delete(ActivityId, Opaque, Frag, Key, LockKind).

delete_object(ActivityId, Opaque, Tab, Rec, LockKind) ->
    Frag = record_to_frag_name(Tab, Rec),
    mnesia2:delete_object(ActivityId, Opaque, Frag, Rec, LockKind).

read(ActivityId, Opaque, Tab, Key, LockKind) ->
    Frag = key_to_frag_name(Tab, Key),
    mnesia2:read(ActivityId, Opaque, Frag, Key, LockKind).
...
all_keys(ActivityId, Opaque, Tab, LockKind) ->
    Match = [mnesia2:all_keys(ActivityId, Opaque, Frag, LockKind)
	     || Frag <- frag_names(Tab)],
    lists:append(Match).

clear_table(ActivityId, Opaque, Tab, Obj) ->
    [mnesia2:clear_table(ActivityId, Opaque, Frag, Obj)  || Frag <- frag_names(Tab)],
    ok.

index_match_object(ActivityId, Opaque, Tab, Pat, Attr, LockKind) ->
    Match =
	[mnesia2:index_match_object(ActivityId, Opaque, Frag, Pat, Attr, LockKind)
	 || Frag <- frag_names(Tab)],
    lists:append(Match).

index_read(ActivityId, Opaque, Tab, Key, Attr, LockKind) ->
    Match =
	[mnesia2:index_read(ActivityId, Opaque, Frag, Key, Attr, LockKind)
	     || Frag <- frag_names(Tab)],
    lists:append(Match).

foldl(ActivityId, Opaque, Fun, Acc, Tab, LockKind) ->
    Fun2 = fun(Frag, A) ->
		   mnesia2:foldl(ActivityId, Opaque, Fun, A, Frag, LockKind)
	   end,
    lists:foldl(Fun2, Acc, frag_names(Tab)).

foldr(ActivityId, Opaque, Fun, Acc, Tab, LockKind) ->
    Fun2 = fun(Frag, A) ->
		   mnesia2:foldr(ActivityId, Opaque, Fun, A, Frag, LockKind)
	   end,
...
table_info2(ActivityId, Opaque, Tab, Frag, Item) ->
	...
	_ ->
	    mnesia2:table_info(ActivityId, Opaque, Frag, Item)
    end.

first(ActivityId, Opaque, Tab) ->
    case ?catch_val({Tab, frag_hash}) of
	{'EXIT', _} ->
	    mnesia2:first(ActivityId, Opaque, Tab);
	FH ->
	    FirstFrag = Tab,
	    case mnesia2:first(ActivityId, Opaque, FirstFrag) of
		'$end_of_table' ->
...
search_first(ActivityId, Opaque, Tab, N, FH) when N < FH#frag_state.n_fragments ->
    ...
    case mnesia2:first(ActivityId, Opaque, NextFrag) of
	'$end_of_table' ->
...
last(ActivityId, Opaque, Tab) ->
    case ?catch_val({Tab, frag_hash}) of
	{'EXIT', _} ->
	    mnesia2:last(ActivityId, Opaque, Tab);
	FH ->
...
search_last(ActivityId, Opaque, Tab, N, FH) when N >= 1 ->
    ...
    case mnesia2:last(ActivityId, Opaque, Frag) of
	'$end_of_table' ->
	    PrevN = N - 1,
...
prev(ActivityId, Opaque, Tab, Key) ->
    case ?catch_val({Tab, frag_hash}) of
	{'EXIT', _} ->
	    mnesia2:prev(ActivityId, Opaque, Tab, Key);
	FH ->
	    N = key_to_n(FH, Key),
	    Frag = n_to_frag_name(Tab, N),
	    case mnesia2:prev(ActivityId, Opaque, Frag, Key) of
		'$end_of_table' ->
...
search_prev(ActivityId, Opaque, Tab, N) when N > 1 ->
    ...
    case mnesia2:last(ActivityId, Opaque, PrevFrag) of
	'$end_of_table' ->
...
next(ActivityId, Opaque, Tab, Key) ->
    case ?catch_val({Tab, frag_hash}) of
	{'EXIT', _} ->
	    mnesia2:next(ActivityId, Opaque, Tab, Key);
	FH ->
	    N = key_to_n(FH, Key),
	    Frag = n_to_frag_name(Tab, N),
	    case mnesia2:next(ActivityId, Opaque, Frag, Key) of
		'$end_of_table' ->
...
remote_table_info(ActivityId, Opaque, Tab, Item) ->
    ...
    case rpc:call(N, mnesia2, table_info, [ActivityId, Opaque, Tab, Item]) of
	{badrpc, _} ->
	    mnesia2:abort({no_exists, Tab, Item});
	Info ->
	    Info
    end.
...
init_select(Tid,Opaque,Tab,Pat,Limit,LockKind) ->
    case ?catch_val({Tab, frag_hash}) of
	{'EXIT', _} ->
	    mnesia2:select(Tid, Opaque, Tab, Pat, Limit,LockKind);
	FH ->
	    FragNumbers = verify_numbers(FH,Pat), 
	    Fun = fun(Num) ->
			  Name = n_to_frag_name(Tab, Num),
			  Node = val({Name, where_to_read}),
			  Storage = mnesia2_lib:storage_type_at_node(Node, Name),
			  mnesia2:lock(Tid, Opaque, {table, Name}, LockKind),
			  {Name, Node, Storage}
		  end,
	    [{FTab,Node,Type}|NameNodes] = lists:map(Fun, FragNumbers),
	    InitFun = fun(FixedSpec) -> mnesia2:dirty_sel_init(Node,FTab,FixedSpec,Limit,Type) end,
	    Res = mnesia2:fun_select(Tid,Opaque,FTab,Pat,LockKind,FTab,InitFun,Limit,Node,Type),
...
select_cont(Tid,Ts,{frag_cont, '$end_of_table', [{Tab,Node,Type}|Rest],Args}) -> 
    ...
    InitFun = fun(FixedSpec) -> mnesia2:dirty_sel_init(Node,Tab,FixedSpec,Limit,Type) end,
    Res = mnesia2:fun_select(Tid,Ts,Tab,Spec,LockKind,Tab,InitFun,Limit,Node,Type),
    frag_sel_cont(Res, Rest, Args);
select_cont(Tid,Ts,{frag_cont, Cont, TabL, Args}) -> 
    frag_sel_cont(mnesia2:select_cont(Tid,Ts,Cont),TabL,Args);
select_cont(Tid,Ts,Else) ->  %% Not a fragmented table
    mnesia2:select_cont(Tid,Ts,Else).
...
do_select(ActivityId, Opaque, Tab, MatchSpec, LockKind) ->
    case ?catch_val({Tab, frag_hash}) of
	{'EXIT', _} ->
	    mnesia2:select(ActivityId, Opaque, Tab, MatchSpec, LockKind);
	FH ->
	    FragNumbers = verify_numbers(FH,MatchSpec), 
	    Fun = fun(Num) ->
			  Name = n_to_frag_name(Tab, Num),
			  Node = val({Name, where_to_read}),
			  mnesia2:lock(ActivityId, Opaque, {table, Name}, LockKind),
			  {Name, Node}
		  end,
	    NameNodes = lists:map(Fun, FragNumbers),
	    SelectAllFun =
		fun(PatchedMatchSpec) ->
			Match = [mnesia2:dirty_select(Name, PatchedMatchSpec)
		...
		[] ->
		    ...
		    mnesia2:fun_select(ActivityId, Opaque, Tab, MatchSpec, none, '_', SelectAllFun);
		RemoteNameNodes ->
			...
			fun(PatchedMatchSpec) ->
				...
				LocalMatch0 = [mnesia2:dirty_select(Name, PatchedMatchSpec)
		    ...
		    mnesia2:fun_select(ActivityId, Opaque, Tab, MatchSpec, none, '_', SelectFun)
	    end
    end.
...
verify_numbers(FH,MatchSpec) ->
	...
	FragNumbers
	    ...
	    mnesia2:abort({"match_spec_to_frag_numbers: Fragment numbers out of range",
			  BadFrags, {range, 1, N}})
    end.

local_select(ReplyTo, Ref, RemoteNameNodes, MatchSpec) ->
    RemoteNodes = mnesia2_lib:uniq([Node || {_Name, Node} <- RemoteNameNodes]),
    Args = [ReplyTo, Ref, RemoteNameNodes, MatchSpec],
    {Replies, BadNodes} = rpc:multicall(RemoteNodes, ?MODULE, remote_select, Args),
    case mnesia2_lib:uniq(Replies) -- [ok] of
	[] when BadNodes == [] ->
...
do_remote_select(ReplyTo, Ref, [{Name, Node} | NameNodes], MatchSpec) ->
    if
	Node == node() ->
	    Res = ?CATCH({ok, mnesia2:dirty_select(Name, MatchSpec)}),
...
expand_cstruct(Cs, Mode) ->
    ...
    mnesia2_schema:verify({alt, [nil, list]}, mnesia2_lib:etype(Props),
			 {badarg, Tab, Props}), 
    %% Verify keys
    ValidKeys = [foreign_key, n_fragments, node_pool,
		 n_ram_copies, n_disc_copies, n_disc_only_copies,
		 hash_module, hash_state],
    Keys = mnesia2_schema:check_keys(Tab, Props, ValidKeys),
    mnesia2_schema:check_duplicates(Tab, Keys),

    %% Pick fragmentation props
    ForeignKey = mnesia2_schema:pick(Tab, foreign_key, Props, undefined),
    {ForeignKey2, N, Pool, DefaultNR, DefaultND, DefaultNDO} =
	pick_props(Tab, Cs, ForeignKey),

    %% Verify node_pool
    BadPool = {bad_type, Tab, {node_pool, Pool}},
    mnesia2_schema:verify(list, mnesia2_lib:etype(Pool), BadPool),
    NotAtom = fun(A) when is_atom(A) -> false;
		 (_A) -> true
	      end,
    mnesia2_schema:verify([], [P || P <- Pool, NotAtom(P)], BadPool),

    NR  = mnesia2_schema:pick(Tab, n_ram_copies, Props, 0),
    ND  = mnesia2_schema:pick(Tab, n_disc_copies, Props, 0),
    NDO = mnesia2_schema:pick(Tab, n_disc_only_copies, Props, 0),
    
    PosInt = fun(I) when is_integer(I), I >= 0 -> true;
		(_I) -> false
	     end,
    mnesia2_schema:verify(true, PosInt(NR),
			 {bad_type, Tab, {n_ram_copies, NR}}),
    mnesia2_schema:verify(true, PosInt(ND),
			 {bad_type, Tab, {n_disc_copies, ND}}),
    mnesia2_schema:verify(true, PosInt(NDO),
    ...
    HashMod = mnesia2_schema:pick(Tab, hash_module, Props, ?DEFAULT_HASH_MOD),
    HashState = mnesia2_schema:pick(Tab, hash_state, Props, undefined),
...
do_expand_cstruct(Cs, FH, N, Pool, NR, ND, NDO, Mode) ->
    ...
    mnesia2_schema:verify(false, LC,
			 {combine_error, Tab, {local_content, LC}}),

    Snmp = Cs#cstruct.snmp,
    mnesia2_schema:verify([], Snmp,
...
verify_n_fragments(N, Cs, Mode) when is_integer(N), N >= 1 ->
	...
	activate  ->
	    ...
	    mnesia2_schema:verify(1, N, Reason),
	    Cs
    end;
verify_n_fragments(N, Cs, _Mode) ->
    mnesia2:abort({bad_type, Cs#cstruct.name, {n_fragments, N}}).

pick_props(Tab, Cs, {ForeignTab, Attr}) ->
    mnesia2_schema:verify(true, ForeignTab /= Tab,
    ...
    N           = mnesia2_schema:pick(Tab, n_fragments, Props,  ForeignN),
    Pool        = mnesia2_schema:pick(Tab, node_pool, Props, ForeignPool),
    
    mnesia2_schema:verify(ForeignN, N, 
			 {combine_error, Tab, {n_fragments, N},
			  ForeignTab, {n_fragments, ForeignN}}),

    mnesia2_schema:verify(ForeignPool, Pool, 
			 {combine_error, Tab, {node_pool, Pool},
			  ForeignTab, {node_pool, ForeignPool}}),

    mnesia2_schema:verify(undefined, ForeignKey,
			 {combine_error, Tab,
			  "Multiple levels of foreign_key dependencies",
			  {ForeignTab, Attr}, ForeignKey}),

    Key = {ForeignTab, mnesia2_schema:attr_to_pos(Attr, Attrs)},
...
pick_props(Tab, Cs, undefined) ->
    Props = Cs#cstruct.frag_properties,
    DefaultN = 1,
    DefaultPool = mnesia2:system_info(db_nodes),
    N    = mnesia2_schema:pick(Tab, n_fragments, Props,  DefaultN),
    Pool = mnesia2_schema:pick(Tab, node_pool, Props, DefaultPool),
    DefaultNR = 1,
    DefaultND = 0,
    DefaultNDO = 0,
    {undefined, N, Pool, DefaultNR, DefaultND, DefaultNDO};
pick_props(Tab, _Cs, BadKey) ->
    mnesia2:abort({bad_type, Tab, {foreign_key, BadKey}}).
...
set_frag_nodes(_, _, _, Cs, [], _) ->
    mnesia2:abort({combine_error,  Cs#cstruct.name, "Too few nodes in node_pool"}).
...
set_frag_node(Cs, Pos, Head) ->
	...
	case Head of
		...
		{N, 1};
	    BadNode ->
		mnesia2:abort({bad_type, Cs#cstruct.name, BadNode})
	end,
    mnesia2_schema:verify(true, 
...
change_table_frag(Tab,  Change) ->
    mnesia2:abort({bad_type, Tab, Change}).
...
make_activate(Tab, Props) ->
    Cs = mnesia2_schema:incr_version(val({Tab, cstruct})),
    mnesia2_schema:ensure_active(Cs),
    case Cs#cstruct.frag_properties of
	[] ->
	    Cs2 = Cs#cstruct{frag_properties = Props},
	    [Cs3] = expand_cstruct(Cs2, activate),
	    TabDef = mnesia2_schema:vsn_cs2list(Cs3),
	    Op = {op, change_table_frag, activate, TabDef},
	    [[Op]];
	BadProps ->
	    mnesia2:abort({already_exists, Tab, {frag_properties, BadProps}})
    end.
...
make_deactivate(Tab) ->
    ...
    if
	BaseTab /= Tab ->
	    mnesia2:abort({combine_error, Tab, "Not a base table"});
	Foreigners /= [] ->
	    mnesia2:abort({combine_error, Tab, "Too many foreigners", Foreigners});
	FH#frag_state.n_fragments > 1 ->
	    mnesia2:abort({combine_error, Tab, "Too many fragments"});
	true ->
	    Cs2 = Cs#cstruct{frag_properties = []},
	    TabDef = mnesia2_schema:vsn_cs2list(Cs2),
...
make_multi_add_frag(Tab, SortedNs) ->
    mnesia2:abort({bad_type, Tab, SortedNs}).
...
make_frag_names_and_acquire_locks(Tab, N, FragIndecies, DoNotLockN) ->
    mnesia2_schema:get_tid_ts_and_lock(Tab, write),
    Fun = fun(Index, FN) ->
		  if
		      ...
		      true ->
			  Name = n_to_frag_name(Tab, Index),
			  mnesia2_schema:get_tid_ts_and_lock(Name, write),
			  setelement(Index , FN, Name)
		  end
	  end,
...
make_add_frag(Tab, SortedNs) ->
    ...
    [NewOp] = mnesia2_schema:make_create_table(NewCs2),
...
adjust_before_split(FH) ->
	...
	{FH2, FromFrags2, UnionFrags}
	    ...
	    mnesia2:abort({"add_frag: Fragment numbers out of range",
			  BadFrags, {range, 1, N}})
    end.

split(Tab, FH, [SplitN | SplitNs], FragNames, Ops) ->
    SplitFrag = element(SplitN, FragNames),
    Pat = mnesia2:table_info(SplitFrag, wild_pattern),
    {_Mod, Tid, Ts} = mnesia2_schema:get_tid_ts_and_lock(Tab, none),
    Recs = mnesia2:match_object(Tid, Ts, SplitFrag, Pat, read),
...
do_split(FH, OldN, FragNames, [Rec | Recs], Ops) ->
	...
	NewN ->
		...
		_NewFrag ->
		    ...
		    mnesia2:abort({"add_frag: Fragment not locked", NewN})
	    end
    end;
...
make_del_frag(Tab) ->
	...
	N when N > 1 ->
	    ...
	    [LastOp] = mnesia2_schema:make_delete_table(LastFrag, single_frag),
	    Cs2 = replace_frag_hash(Cs, FH2),
	    TabDef = mnesia2_schema:vsn_cs2list(Cs2),
...
adjust_before_merge(FH) ->
	...
	case lists:member(N, FromFrags2) of
		...
		{FH2, FromFrags2, UnionFrags};
		false ->
		mnesia2:abort({"del_frag: Last fragment number not included", N})
	end
    catch error:BadFrags ->
	    mnesia2:abort({"del_frag: Fragment numbers out of range",
			  BadFrags, {range, 1, N}})
    end.

merge(Tab, FH, [FromN | FromNs], FragNames, Ops) ->
    FromFrag = element(FromN, FragNames),
    Pat = mnesia2:table_info(FromFrag, wild_pattern),
    {_Mod, Tid, Ts} = mnesia2_schema:get_tid_ts_and_lock(Tab, none),
    Recs = mnesia2:match_object(Tid, Ts, FromFrag, Pat, read),
...
do_merge(FH, OldN, FragNames, [Rec | Recs], Ops) ->
	...
	NewN when NewN == LastN ->
	    ...
	    mnesia2:abort({"del_frag: Fragment number out of range",
			  NewN, {range, 1, LastN}});
	NewN when NewN == OldN ->
	    %% Keep record in the same fragment. No need to move it.
	...
	NewN when OldN == LastN ->
		...
		_NewFrag ->
		    %% Tried to move record to fragment that not is locked
		    mnesia2:abort({"del_frag: Fragment not locked", NewN})
	    end;
	NewN ->
	    case element(NewN, FragNames) of
		NewFrag when NewFrag /= undefined ->
...
make_add_node(Tab, Node) ->
    mnesia2:abort({bad_type, Tab, Node}).
...
make_del_node(Tab, Node) when is_atom(Node) ->
	...
	true ->
	    ...
	    [Op];
	false ->
	    mnesia2:abort({no_exists, Tab, Node})
    end;
...
set_frag_hash(Tab, Props) ->
	...
	FH when is_record(FH, frag_state) ->
	    mnesia2_lib:set({Tab, frag_hash}, FH);
	no_hash ->
	    mnesia2_lib:unset({Tab, frag_hash})
    end.

props_to_frag_hash(_Tab, []) ->
    no_hash;
props_to_frag_hash(Tab, Props) ->
    case mnesia2_schema:pick(Tab, base_table, Props, undefined) of
	T when T == Tab ->
	    Foreign = mnesia2_schema:pick(Tab, foreign_key, Props, must),
	    N = mnesia2_schema:pick(Tab, n_fragments, Props, must),
	    
	    case mnesia2_schema:pick(Tab, hash_module, Props, undefined) of
		undefined ->
		    Split = mnesia2_schema:pick(Tab, next_n_to_split, Props, must),
		    Doubles = mnesia2_schema:pick(Tab, n_doubles, Props, must),
		...
		HashMod ->
		    HashState = mnesia2_schema:pick(Tab, hash_state, Props, must),
		    #frag_state{foreign_key = Foreign,
				n_fragments = N,
				hash_module = HashMod,
				hash_state  = HashState}
...
lookup_prop(Tab, Prop) ->
	...
	{value, {Prop, Val}} ->
	    Val;
	false ->
	    mnesia2:abort({no_exists, Tab, Prop, {frag_properties, Props}})
    end.
...
lookup_frag_hash(Tab) ->
	...
	{frag_hash, K, N, _S, _D} = FH ->
	    ...
	    #frag_state{foreign_key = K, 
			...
			hash_state  = HashState};    
	{'EXIT', _} ->
	    mnesia2:abort({no_exists, Tab, frag_properties, frag_hash})
    end.
...
lookup_foreigners(Tab) ->
    ...
    [T || [T] <- ?ets_match(mnesia2_gvar, {{'$1', frag_hash}, HashPat})].
...
key_to_frag_number({Tab, ForeignKey}, _Key) ->
	...
	undefined ->
	    mnesia2:abort({combine_error, Tab, frag_properties,
...
key_to_n(FH, Key) ->
	...
	is_integer(N), N >= 1, N =< FH#frag_state.n_fragments ->
	    N;
	true ->
	    mnesia2:abort({"key_to_frag_number: Fragment number out of range",
