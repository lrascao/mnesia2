...
get_log_writes() ->
    Max = mnesia2_monitor:get_env(dump_log_write_threshold),
    Prev = mnesia2_lib:read_counter(trans_log_writes),
    Left = mnesia2_lib:read_counter(trans_log_writes_left),
    Diff = Max - Left,
    Prev + Diff.

incr_log_writes() ->
    Left = mnesia2_lib:incr_counter(trans_log_writes_left, -1),
    if
	Left > 0 ->
	    ignore;
	true ->
...
adjust_log_writes(DoCast) ->
    Token = {mnesia2_adjust_log_writes, self()},
	...
	true ->
		...
		false ->
		    ignore;
		true ->
		    mnesia2_controller:async_dump_log(write_threshold)
	    end,
	    Max = mnesia2_monitor:get_env(dump_log_write_threshold),
	    Left = mnesia2_lib:read_counter(trans_log_writes_left),
	    %% Don't care if we lost a few writes
	    mnesia2_lib:set_counter(trans_log_writes_left, Max),
	    Diff = Max - Left,
	    _ = mnesia2_lib:incr_counter(trans_log_writes, Diff),
...
snapshot_dcd(Tables) ->
    lists:foreach(
      fun(Tab) ->
	      case mnesia2_lib:storage_type_at_node(node(), Tab) of
		  disc_copies ->
		      mnesia2_log:ets2dcd(Tab);
		  _ ->
...
perform_dump(InitBy, Regulator) when InitBy == scan_decisions ->
    ...
    scan_decisions(mnesia2_log:previous_log_file(), InitBy, Regulator),
    scan_decisions(mnesia2_log:latest_log_file(), InitBy, Regulator);

%% Propagate the log into the DAT-files
perform_dump(InitBy, Regulator) ->
    ?eval_debug_fun({?MODULE, perform_dump}, [InitBy]),
    LogState = mnesia2_log:prepare_log_dump(InitBy),
    dbg_out("Transaction log dump initiated by ~w: ~w~n",
	    [InitBy, LogState]),
    adjust_log_writes(false),
    case LogState of
	already_dumped ->
	    mnesia2_recover:allow_garb(),
	    dumped;
	{needs_dump, Diff} ->
	    U = mnesia2_monitor:get_env(dump_log_update_in_place),
	    Cont = mnesia2_log:init_log_dump(),
	    mnesia2_recover:sync(),
...
scan_decisions(Fname, InitBy, Regulator) ->
    Exists = mnesia2_lib:exists(Fname),
    case Exists of
	false ->
	    ok;
	true ->
	    Header = mnesia2_log:trans_log_header(),
	    Name = previous_log,
	    mnesia2_log:open_log(Name, Header, Fname, Exists,
		...
		do_perform_dump(Cont, false, InitBy, Regulator, undefined)
		    ...
		    {error, Reason}
	    after mnesia2_log:close_log(Name)
	    end
    end.

do_perform_dump(Cont, InPlace, InitBy, Regulator, OldVersion) ->
    case mnesia2_log:chunk_log(Cont) of
	{C2, Recs} ->
...
insert_rec(Rec, _InPlace, scan_decisions, _LogV) ->
	...
	is_record(Rec, log_header) ->
	    ignore;
	true ->
	    mnesia2_recover:note_log_decision(Rec, scan_decisions)
    end;
insert_rec(Rec, InPlace, InitBy, LogV) when is_record(Rec, commit) ->
    ...
    case mnesia2_recover:wait_for_decision(D, InitBy) of
	{Tid, committed} ->
	    do_insert_rec(Tid, Rec, InPlace, InitBy, LogV);
	{Tid, aborted} ->
	    case InitBy of
		startup ->
		    mnesia2_schema:undo_prepare_commit(Tid, Rec);
		_ ->
		    ok
	    end
    end;
insert_rec(H, _InPlace, _InitBy, _LogV) when is_record(H, log_header) ->
    CurrentVersion = mnesia2_log:version(),
    if
...
update(Tid, SchemaOps, DumperMode) ->
    ...
    mnesia2_controller:release_schema_commit_lock(),
    Res.
...
perform_update(Tid, SchemaOps, _DumperMode, _UseDir) ->
    ...
    InPlace = mnesia2_monitor:get_env(dump_log_update_in_place),
...
disc_insert(_Tid, Storage, Tab, Key, Val, Op, InPlace, InitBy) ->
	...
	true ->
		...
		disc_copies when Tab /= schema ->
		    mnesia2_log:append({?MODULE,Tab}, {{Tab, Key}, Val, Op}),
		    ok;
		_ ->
...
insert(Tid, Storage, Tab, Key, Val, Op, InPlace, InitBy) ->
	...
	_ when Storage == ram_copies ->
	    mnesia2_tm:do_update_op(Tid, Storage, Item),
	    Snmp = mnesia2_tm:prepare_snmp(Tab, Key, [Item]),
	    mnesia2_tm:do_snmp(Tid, Snmp);
...
disc_delete_table(Tab, Storage) ->
	...
	true ->
		...
		Storage == disc_only_copies; Tab == schema ->
		    ...
		    ok;
		true ->
		    DclFile = mnesia2_lib:tab2dcl(Tab),
		    case get({?MODULE,Tab}) of
			{opened_dumper, dcl} ->
			    del_opened_tab(Tab),
			    mnesia2_log:unsafe_close_log(Tab);
			_ ->
			    ok
		    end,
		    file:delete(DclFile),
...
disc_delete_indecies(Tab, Cs, disc_only_copies) ->
    ...
    mnesia2_index:del_transient(Tab, Indecies, disc_only_copies).
...
insert_op(Tid, _, {op, change_table_copy_type, N, FromS, ToS, TabDef}, InPlace, InitBy) ->
    Cs = mnesia2_schema:list2cs(TabDef),
    Val = mnesia2_schema:insert_cstruct(Tid, Cs, true), % Update ram only
    {schema, Tab, _} = Val,
    case lists:member(N, val({current, db_nodes})) of
	true when InitBy /= startup  ->
	    mnesia2_controller:add_active_replica(Tab, N, Cs);
	_ ->
	    ignore
    end,
    if
	N == node() ->
	    ...
	    Dat  = mnesia2_lib:tab2dat(Tab),
	    Dcd  = mnesia2_lib:tab2dcd(Tab),
	    Dcl  = mnesia2_lib:tab2dcl(Tab),
	    case {FromS, ToS} of
		...
		{ram_copies, disc_only_copies} ->
		    ...
		    mnesia2_schema:ram_delete_table(Tab, FromS),
		    PosList = Cs#cstruct.index,
		    mnesia2_index:init_indecies(Tab, disc_only_copies, PosList);
		{disc_only_copies, ram_copies} ->
			...
			startup ->
			    ignore;
			_ ->
			    mnesia2_controller:get_disc_copy(Tab),
			    ok
		    end,
...
insert_op(Tid, _, {op, restore_recreate, TabDef}, InPlace, InitBy) ->
	...
	_ ->
		...
		{'EXIT', _} -> ignore;
		_ ->
		    mnesia2_schema:ram_delete_table(Tab, Storage),
	...
	Storage == ram_copies ->
	    ...
	    mnesia2_monitor:mktab(Tab, Args);
	Storage == disc_copies ->
	    EtsProps = proplists:get_value(ets, StorageProps, []),
	    Args = [{keypos, 2}, public, named_table, Type | EtsProps],
	    mnesia2_monitor:mktab(Tab, Args),
	    ...
	    {ok, Log} = mnesia2_monitor:open_log(FArg),
	...
	Storage == disc_only_copies ->
	    ...
	    Args = [{file, mnesia2_lib:tab2dat(Tab)},
		    ...
		    | DetsProps ],
	    mnesia2_monitor:open_dets(Tab, Args)
    end,
...
insert_op(Tid, _, {op, create_table, TabDef}, InPlace, InitBy) ->
	...
	startup ->
		...
		disc_copies ->
		    Dcd = mnesia2_lib:tab2dcd(Tab),
		    case mnesia2_lib:exists(Dcd) of
			true -> ignore;
			false ->
			    mnesia2_log:open_log(temp,
						mnesia2_log:dcd_log_header(),
						Dcd,
						false,
						false,
						read_write),
			    mnesia2_log:unsafe_close_log(temp)
		    end;
		_ ->
		    DetsProps = proplists:get_value(dets, StorageProps, []),
		    ...
		    Args = [{file, mnesia2_lib:tab2dat(Tab)},
			    ...
			    {repair, mnesia2_monitor:get_env(auto_repair)} 
			    | DetsProps ],
		    case mnesia2_monitor:open_dets(Tab, Args) of
			{ok, _} ->
			    mnesia2_monitor:unsafe_close_dets(Tab);
			{error, Error} ->
	...
	_ ->
	    ...
	    Active = mnesia2_lib:intersect(Copies, val({current, db_nodes})),
	    [mnesia2_controller:add_active_replica(Tab, N, Cs) || N <- Active],
		...
		_ ->
		    case Cs#cstruct.local_content of
			true ->
			    mnesia2_lib:set_local_content_whereabouts(Tab);
			false ->
		    ...
		    mnesia2_controller:create_table(Tab),
		    mnesia2_lib:unset({Tab, create_table})
	    end
    end;
	...
	schema ->
	    ...
	    Update = fun(NS = {Node,Storage}) ->
				 ...
				 disc_copies when Node == node() ->
				     Dir = mnesia2_lib:dir(),
				     ok = mnesia2_schema:opt_create_dir(true, Dir),
				     mnesia2_schema:purge_dir(Dir, []),
				     mnesia2_log:purge_all_logs(),

				     mnesia2_lib:set(use_dir, true),
				     mnesia2_log:init(),
	    ...
	    if W2C == W2C0 -> ignore;
	       true -> mnesia2_lib:set({schema, where_to_commit}, W2C)
	    end;
	_ ->
	    ignore
    end,
...
insert_op(Tid, _, {op, del_table_copy, Storage, Node, TabDef}, InPlace, InitBy) ->
	...
	Tab == schema, Storage == ram_copies ->
	    insert_cstruct(Tid, Cs, true, InPlace, InitBy);
        Tab /= schema ->
	    mnesia2_controller:del_active_replica(Tab, Node),
	    mnesia2_lib:del({Tab, Storage}, Node),
	    if
		Node == node() ->
		    case Cs#cstruct.local_content of
			true -> mnesia2_lib:set({Tab, where_to_read}, nowhere);
			false -> mnesia2_lib:set_remote_where_to_read(Tab)
		    end,
		    mnesia2_lib:del({schema, local_tables}, Tab),
		    mnesia2_lib:set({Tab, storage_type}, unknown),
		    insert_cstruct(Tid, Cs, true, InPlace, InitBy),
		    disc_delete_table(Tab, Storage),
		    disc_delete_indecies(Tab, Cs, Storage),
		    mnesia2_schema:ram_delete_table(Tab, Storage),
...
insert_op(Tid, _, {op, add_index, Pos, TabDef}, InPlace, InitBy) ->
	...
	startup when Storage == disc_only_copies ->
	    ...
	    mnesia2_index:init_indecies(Tab, Storage, [Pos]);
	startup ->
	    ignore;
	_  ->
	    case val({Tab,where_to_read}) of
...
insert_op(Tid, _, {op, del_index, Pos, TabDef}, InPlace, InitBy) ->
    ...
    case InitBy of
	startup when Storage == disc_only_copies ->
	    mnesia2_index:del_index_table(Tab, Storage, Pos);
	startup ->
	    ignore;
	_ ->
	    mnesia2_index:del_index_table(Tab, Storage, Pos)
...
insert_op(Tid, _, {op, change_table_access_mode,TabDef, _OldAccess, _Access}, InPlace, InitBy) ->
    ...
    case InitBy of
	startup -> ignore;
	_ -> mnesia2_controller:change_table_access_mode(Cs)
    end,
    insert_cstruct(Tid, Cs, true, InPlace, InitBy);
...
insert_op(Tid, _, {op, change_table_majority,TabDef, _OldAccess, _Access}, InPlace, InitBy) ->
    ...
    case InitBy of
	startup -> ignore;
	_ -> mnesia2_controller:change_table_majority(Cs)
    end,
    insert_cstruct(Tid, Cs, true, InPlace, InitBy);
...
insert_op(Tid, _, {op, delete_property, TabDef, PropKey}, InPlace, InitBy) ->
    Cs = mnesia2_schema:list2cs(TabDef),
    Tab = Cs#cstruct.name,
    mnesia2_lib:unset({Tab, user_property, PropKey}),
    insert_cstruct(Tid, Cs, true, InPlace, InitBy);
		...
		Type ->
			...
			_ ->
			    ...
			    Args = [{file, Fname},
				    ...
				    {type, mnesia2_lib:disk_type(Tab, Type)} 
				    | DetsProps],
			    {ok, _} = mnesia2_monitor:open_dets(Tab, Args),
...
open_disc_copies(Tab, InitBy) ->
	...
	DumpEts == false; InitBy == startup ->
            DclF = mnesia2_lib:tab2dcl(Tab),
	    mnesia2_log:open_log({?MODULE,Tab},
...
needs_dump_ets(Tab) ->
    ...
    case file:read_file_info(DclF) of
        {error, enoent} ->
            false;
        {ok, DclInfo} ->
            DcdF =  mnesia2_lib:tab2dcd(Tab),
            case file:read_file_info(DcdF) of
                {error, Reason} ->
                    mnesia2_lib:dbg_out("File ~p info_error ~p ~n",
                                       [DcdF, Reason]),
                    true;
                {ok, DcdInfo} ->
...
prepare_open(Tab, UpdateInPlace) ->
	...
	true ->
	    Dat;
	false ->
	    Tmp = mnesia2_lib:tab2tmp(Tab),
	    try ok = mnesia2_lib:copy_file(Dat, Tmp)
	    catch error:Error ->
...
close_files(InPlace, Outcome, InitBy, [{{?MODULE, Tab}, {opened_dumper, Type}} | Tail]) ->
	...
	disc_only_copies when InitBy /= startup ->
	    ignore;
	disc_copies when Tab /= schema ->
	    mnesia2_log:close_log({?MODULE,Tab});
	Storage ->
...
do_close(_, _, Tab, dcl, unknown) ->
    mnesia2_log:close_log({?MODULE,Tab}),
    file:delete(mnesia2_lib:tab2dcl(Tab));
...
do_close(InPlace, Outcome, Tab, dat, Storage) ->
	...
	Outcome == ok, Storage /= unknown ->
	    ...
	    TabDat = mnesia2_lib:tab2dat(Tab),
	    ok = file:rename(mnesia2_lib:tab2tmp(Tab), TabDat);
	true ->
	    file:delete(mnesia2_lib:tab2tmp(Tab))
    end.


ensure_rename(From, To) ->
    case mnesia2_lib:exists(From) of
	true ->
	    file:rename(From, To);
	false ->
	    case mnesia2_lib:exists(To) of
		true ->
		    ok;
		false ->
...
insert_cstruct(Tid, Cs, KeepWhereabouts, InPlace, InitBy) ->
    Val = mnesia2_schema:insert_cstruct(Tid, Cs, KeepWhereabouts),
    {schema, Tab, _} = Val,
    S = val({schema, storage_type}),
    disc_insert(Tid, S, schema, Tab, Val, write, InPlace, InitBy),
    Tab.
...
delete_cstruct(Tid, Cs, InPlace, InitBy) ->
    Val = mnesia2_schema:delete_cstruct(Tid, Cs),
    {schema, Tab, _} = Val,
    S = val({schema, storage_type}),
...
temp_set_master_nodes() ->
    ...
    mnesia2_recover:log_master_nodes(Masters, false, yes),
    ok.
...
raw_named_dump_table(Tab, Ftype) ->
    case mnesia2_monitor:use_dir() of
	true ->
	    mnesia2_lib:lock_table(Tab),
	    TmpFname = mnesia2_lib:tab2tmp(Tab),
	    Fname =
		case Ftype of
		    dat -> mnesia2_lib:tab2dat(Tab);
		    dmp -> mnesia2_lib:tab2dmp(Tab)
	    ...
	    TabRef = Tab,
	    DiskType = mnesia2_lib:disk_type(Tab),
	    Args = [{file, TmpFname},
		    {keypos, 2},
		    %%		    {ram_file, true},
		    ...
		    {repair, mnesia2_monitor:get_env(auto_repair)},
		    {type, DiskType}],
	    case mnesia2_lib:dets_sync_open(TabRef, Args) of
		{ok, TabRef} ->
		    Storage = ram_copies,
			...
			mnesia2_lib:dets_sync_close(Tab),
			mnesia2_lib:unlock_table(Tab)
		    end;
		{error, Reason} ->
		    mnesia2_lib:unlock_table(Tab),
...
start_regulator() ->
    case mnesia2_monitor:get_env(dump_log_load_regulation) of
	false ->
	    nopid;
	true ->
	    N = ?REGULATOR_NAME,
	    case mnesia2_monitor:start_proc(N, ?MODULE, regulator_init, [self()]) of
		{ok, Pid} ->
		    Pid;
		{error, Reason} ->
