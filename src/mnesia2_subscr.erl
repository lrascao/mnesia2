...
set_debug_level(Level) ->
    OldEnv = application:get_env(mnesia2, debug),
    case mnesia2_monitor:patch_env(debug, Level) of
	{error, Reason} ->
	    {error, Reason};
	NewLevel ->
...
set_debug_level(Level, OldEnv) ->
	...
	no ->
	    ...
	    E;
	_ ->
	    Old = mnesia2_lib:val(debug),
	    Local = mnesia2:system_info(local_tables),
	    E = whereis(mnesia2_event),
...
subscribers() ->
    [whereis(mnesia2_event) | mnesia2_lib:val(subscribers)].
...
report_activity(Tid) ->
	...
	{'EXIT', _} -> ok;
	Subscribers ->
	    deliver(Subscribers, {mnesia2_activity_event, {complete, Tid}})
	end.
...
what(Tab, Tid, {RecName, Key}, delete, undefined) ->
    try mnesia2_lib:db_get(Tab, Key) of
	Old -> %% Op only allowed for set table.
	    {mnesia2_table_event, {delete, Tab, {RecName, Key}, Old, Tid}}
    catch error:_ ->
...
what(Tab, Tid, Obj, delete, Old) ->
    {mnesia2_table_event, {delete, Tab, Obj, Old, Tid}};
what(Tab, Tid, Obj, delete_object, _Old) ->
    {mnesia2_table_event, {delete, Tab, Obj, [Obj], Tid}};
what(Tab, Tid, Obj, write, undefined) ->
    try	mnesia2_lib:db_get(Tab, element(2, Obj)) of
	Old ->
	    {mnesia2_table_event, {write, Tab, Obj, Old, Tid}}
    catch error:_ ->
	    ignore
    end;
...
init([Parent]) ->
    ...
    ClientPid = whereis(mnesia2_event),
    link(ClientPid),
    mnesia2_lib:verbose("~p starting: ~p~n", [?MODULE, self()]),
    Tab = ?ets_new_table(mnesia2_subscr, [duplicate_bag, private]),
...
do_change({activate, ClientPid, activity}, SubscrTab) when is_pid(ClientPid) ->
    ...
    activate(ClientPid, activity, Var, mnesia2_lib:val(Var), SubscrTab);
do_change({activate, ClientPid, {table, Tab, How}}, SubscrTab) when is_pid(ClientPid) ->
    case ?catch_val({Tab, where_to_read}) of
	Node when Node == node() ->
	    Var = {Tab, commit_work},
	    activate(ClientPid, {table, Tab, How}, Var, mnesia2_lib:val(Var), SubscrTab);
	{'EXIT', _} ->
	    {error, {no_exists, Tab}};
	_Node ->
...
add_subscr(subscribers, _What, Pid) ->
    mnesia2_lib:add(subscribers, Pid),
    {ok, node()};
add_subscr(activity_subscribers, _What, Pid) ->
    mnesia2_lib:add(activity_subscribers, Pid),
    {ok, node()};
add_subscr({Tab, commit_work}, What, Pid) ->
    Commit = mnesia2_lib:val({Tab, commit_work}),
    case lists:keysearch(subscribers, 1, Commit) of
	false ->
	    Subscr = 
		case What of 
	    ...
	    mnesia2_lib:add({Tab, subscribers}, Pid),
...
del_subscr(subscribers, _What, Pid) ->
    mnesia2_lib:del(subscribers, Pid);
del_subscr(activity_subscribers, _What, Pid) ->
    mnesia2_lib:del(activity_subscribers, Pid);
del_subscr({Tab, commit_work}, What, Pid) ->
	...
	{value, Old} ->
		...
		{subscribers, [], []} ->
		    ...
		    mnesia2_lib:del({Tab, subscribers}, Pid),
		    mnesia2_lib:set({Tab, commit_work}, 
				   mnesia2_lib:sort_commit(NewC));
		_ ->
		    NewC = lists:keyreplace(subscribers, 1, Commit, Subscr),
...
prepare_stop(SubscrTab) ->
    mnesia2_lib:report_system_event({mnesia2_down, node()}),
