...
dump_to_textfile(File) ->
    dump_to_textfile(mnesia2_lib:is_running(), file:open(File, [write])).
dump_to_textfile(yes, {ok, F}) ->
    Tabs = lists:delete(schema, mnesia2_lib:local_active_tables()),
    Defs = lists:map(fun(T) -> {T, [{record_name, mnesia2_lib:val({T, record_name})},
...
dump_tab(F, T) ->
    W = mnesia2_lib:val({T, wild_pattern}),
    {atomic,All} = mnesia2:transaction(fun() -> mnesia2:match_object(T, W, read) end),
...
ensure_started() ->
	...
	yes ->
	    yes;
	no ->
	    case mnesia2_lib:exists(mnesia2_lib:dir("schema.DAT")) of
		true ->
		    mnesia2:start();
		false ->
		    mnesia2:create_schema([node()]),
		    mnesia2:start()
	    end
    end.
...
load_data(L) ->
    mnesia2:transaction(fun() ->
			       F = fun(X) -> 
					   Tab = element(1, X),
					   RN = mnesia2:table_info(Tab, record_name),
					   Rec = setelement(1, X, RN),
					   mnesia2:write(Tab, Rec, write) end,
			       lists:foreach(F, L)
		       end).
