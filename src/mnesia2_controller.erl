...
max_loaders() ->
	...
	{'EXIT', _} ->
	    mnesia2_lib:set(no_table_loaders,1),
	    1;
	Val -> Val
    end.
...
snapshot_dcd(Tables) when is_list(Tables) ->
    case [T || T <- Tables,
	       mnesia2_lib:storage_type_at_node(node(), T) =/= disc_copies] of
	[] ->
...
reply_wait(Tabs) ->
    try mnesia2_lib:active_tables() of
...
get_cstructs() ->
    ...
    {cstructs, mnesia2_schema:normalize_cs(Cstructs, Node), Running}.
...
mnesia2_down(Node) ->
	...
	undefined -> mnesia2_monitor:mnesia2_down(?SERVER_NAME, Node);
	Pid -> gen_server:cast(Pid, {mnesia2_down, Node})
    end.
...
create_table(Tab) ->
    {loaded, ok} = mnesia2_loader:disc_load_table(Tab, {dumper,create_table}).
...
do_force_load_table(Tab) ->
	...
	unknown ->
	    set({Tab, load_by_force}, true),
	    mnesia2_late_loader:async_late_disc_load(node(), [Tab], forced_by_user),
	    wait_for_tables([Tab], infinity);
	{'EXIT', _} ->
	    set({Tab, load_by_force}, true),
	    mnesia2_late_loader:async_late_disc_load(node(), [Tab], forced_by_user),
...
try_schedule_late_disc_load(Tabs, Reason, MsgTag) ->
    GetIntents =
	fun() ->
		Item = mnesia2_late_disc_load,
		Nodes = val({current, db_nodes}),
		mnesia2:lock({global, Item, Nodes}, write),
    ...
    case mnesia2:transaction(GetIntents) of
	{atomic, done} ->
	    done;
...
connect_nodes(Ns, UserFun) ->
	...
	yes ->
		...
		{?MODULE, Pid, Res, New} ->
		    case Res of
			ok ->
			    mnesia2_lib:add_list(extra_db_nodes, New),
			    {ok, New};
...
connect_nodes2(Father, Ns, UserFun) ->
    ...
    {NewC, OldC} = mnesia2_recover:connect_nodes(Ns),
    Connected = NewC ++OldC,
    New1 = mnesia2_lib:intersect(Ns, Connected),
    New = New1 -- Current,
    process_flag(trap_exit, true),
    ...
    Father ! {?MODULE, self(), Res, mnesia2_lib:intersect(Ns,After)},
    unlink(Father),
    ok.
...
merge_schema() ->
    AllNodes = mnesia2_lib:all_nodes(),
...
try_merge_schema(Nodes, Told0, UserFun) ->
	...
	{atomic, {merged, OldFriends, NewFriends}} ->
	    ...
	    Diff = mnesia2_lib:all_nodes() -- [node() | Nodes],
	    mnesia2_recover:connect_nodes(Diff),
...
next_async_dump_log() ->
    Interval = mnesia2_monitor:get_env(dump_log_time_threshold),
...
init([Parent]) ->
    process_flag(trap_exit, true),
    mnesia2_lib:verbose("~p starting: ~p~n", [?SERVER_NAME, self()]),
    ...
    mnesia2_lib:unset(original_nodes),
    mnesia2_recover:connect_nodes(Diff),

    Ref = next_async_dump_log(),
    mnesia2_dumper:start_regulator(),
...
handle_call({snapshot_dcd, Tables}, From, State) ->
    Worker = #dump_log{initiated_by = user,
		       opt_reply_to = From,
		       operation = fun() ->
					   mnesia2_dumper:snapshot_dcd(Tables)
				   end},
    State2 = add_worker(Worker, State),
    noreply(State2);
...
handle_call(disc_load_intents,From,State = #state{loader_queue=LQ,late_loader_queue=LLQ}) ->
    ...
    ActiveTabs = lists:sort(mnesia2_lib:local_active_tables()),
...
handle_call({update_where_to_write, [add, Tab, AddNode], _From}, _Dummy, State) ->
	...
	case lists:member(AddNode, Current) and
	    ...
	    true ->
		mnesia2_lib:add_lsort({Tab, where_to_write}, AddNode),
...
late_disc_load(TabsR, Reason, RemoteLoaders, From,
    ...
    LocalTabs = gb_sets:from_ordset(lists:sort(mnesia2_lib:val({schema,local_tables}))),
...
late_load_filter([RL | RemoteLoaders], Tab, Nodes, Acc) ->
	...
	StillActive == true,
	RemoteIntent == true ->
	    Masters = mnesia2_recover:get_master_nodes(Tab),
...
handle_cast({mnesia2_down, Node}, State) ->
    maybe_log_mnesia2_down(Node),
    mnesia2_lib:del({current, db_nodes}, Node),
    mnesia2_lib:unset({node_up, Node}),
    mnesia2_checkpoint:tm_mnesia2_down(Node),
    Alltabs = val({schema, tables}),
    reconfigure_tables(Node, Alltabs),
    %% Done from (external point of view)
    mnesia2_monitor:mnesia2_down(?SERVER_NAME, Node),
...
handle_cast({merging_schema, Node}, State) ->
	...
	false ->
		...
		andalso (mnesia2_lib:val({schema, local_tables}) == [schema]),
...
handle_cast({im_running, Node, NewFriends}, State) ->
    LocalTabs = mnesia2_lib:local_active_tables() -- [schema],
    RemoveLocalOnly = fun(Tab) -> not val({Tab, local_content}) end,
    Tabs = lists:filter(RemoveLocalOnly, LocalTabs),
    Nodes = mnesia2_lib:union([Node],val({current, db_nodes})),
    Ns = mnesia2_lib:intersect(NewFriends, Nodes),
...
handle_cast({master_nodes_updated, Tab, Masters}, State) ->
	...
	case val({Tab, load_by_force}) of
		...
		if
			...
			Active;
		    true ->
			mnesia2_lib:intersect(Masters, Active)
		end
	end,
    case Valid of
	[] ->
...
handle_cast({adopt_orphans, Node, Tabs}, State) ->
	...
	_ ->
	    ...
	    mnesia2_recover:log_mnesia2_up(Node),
	    verbose("Logging mnesia2_up ~w~n",[Node]),
	    mnesia2_lib:report_system_event({mnesia2_up, Node}),
	    mnesia2_tm:mnesia2_up(Node),
	    %% Load orphan tables
	    ...
	    mnesia2_late_loader:async_late_disc_load(node(), LocalOrphans, Reason),
		...
		fun(N) ->
			RemoteOrphans =
			    [Tab || {Tab, Ns} <- RemoteMasters,
				    lists:member(N, Ns)],
			mnesia2_late_loader:maybe_async_late_disc_load(N, RemoteOrphans, Reason)
		end,
...
handle_info({'EXIT', Pid, R}, State) when Pid == State#state.supervisor ->
    ?SAFE(set(mnesia2_status, stopping)),
...
terminate(Reason, State) ->
    mnesia2_monitor:terminate_proc(?SERVER_NAME, Reason, State).
...
maybe_log_mnesia2_down(N) ->
	...
	yes ->
	    verbose("Logging mnesia2_down ~w~n", [N]),
	    mnesia2_recover:log_mnesia2_down(N),
	    ok;
	_ ->
	    Filter = fun(Tab) ->
...
inactive_copy_holders(Tab, Node) ->
	...
	unknown ->
	    false;
	_Storage ->
	    mnesia2_lib:not_active_here(Tab)
    end.

orphan_tables([Tab | Tabs], Node, Ns, Local, Remote) ->
    Cs = val({Tab, cstruct}),
    CopyHolders = mnesia2_lib:copy_holders(Cs),
    ...
    RamCopyHoldersOnDiscNodes = mnesia2_lib:intersect(RamCopyHolders, DiscNodes),
    Active = val({Tab, active_replicas}),
...
update_whereabouts(Tab, Node, State) ->
	...
	Storage == unknown ->
	    ...
	    add_active_replica(Tab, Node),
	    NodeST = mnesia2_lib:storage_type_at_node(Node, Tab),
	    ReadST = mnesia2_lib:storage_type_at_node(Read, Tab),
...
initial_safe_loads() ->
	...
	disc_copies ->
	    Downs = mnesia2_recover:get_mnesia2_downs(),
	    dbg_out("mnesia2_downs = ~p~n", [Downs]),
...
last_consistent_replica(Tab, Downs) ->
    ...
    BetterCopies0 = mnesia2_lib:remote_copy_holders(Cs) -- Downs,
...
reconfigure_tables(N, [Tab |Tail]) ->
    ...
    case val({Tab, where_to_read}) of
	N ->  mnesia2_lib:set_remote_where_to_read(Tab);
	_ ->  ignore
    end,
...
add_active_replica(Tab, Node, Storage, AccessMode) ->
	...
	read_write ->
	    ...
	    mnesia2_lib:add_lsort({Tab, where_to_write}, Node);
	read_only ->
	    set(Var, mark_blocked_tab(Blocked, Del)),
	    mnesia2_lib:del({Tab, where_to_write}, Node)
    end,
...
del_active_replica(Tab, Node) ->
    ...
    New = lists:sort(Del),
    set(Var, mark_blocked_tab(Blocked, New)),      % where_to_commit
    mnesia2_lib:del({Tab, active_replicas}, Node),
    mnesia2_lib:del({Tab, where_to_write}, Node),
    update_where_to_wlock(Tab).
...
user_sync_tab(Tab) ->
    case val(debug) of
	trace ->
	    mnesia2_subscr:subscribe(whereis(mnesia2_event), {table, Tab});
	_ ->
	    ignore
    end,
...
info() ->
    Tabs = mnesia2_lib:local_active_tables(),
...
info_format(Tab, Size, Mem, Media) ->
    StrT = mnesia2_lib:pad_name(atom_to_list(Tab), 15, []),
    StrS = mnesia2_lib:pad_name(integer_to_list(Size), 8, []),
    StrM = mnesia2_lib:pad_name(integer_to_list(Mem), 8, []),
...
add_worker(Worker = #dump_log{}, State) ->
    ...
    {Status, Queue2} = if Overload ->
                 ...
                 Event = {mnesia2_overload, DetectedBy},
                 mnesia2_lib:report_system_event(Event),
                 {true, Queue};
             true ->
                 {false, Queue ++ [Worker]}
              end,
    mnesia2_recover:log_dump_overload(Status),
...
dump_and_reply(ReplyTo, Worker) ->
    ...
    Res = case Worker#dump_log.operation of
	      dump_log ->
		  mnesia2_dumper:opt_dump_log(Worker#dump_log.initiated_by);
...
send_and_reply(ReplyTo, Worker) ->
    %% No trap_exit, die intentionally instead
    Res = mnesia2_loader:send_table(Worker#send_table.receiver_pid,
...
load_table_fun(#net_load{cstruct=Cs, table=Tab, reason=Reason, opt_reply_to=ReplyTo}) ->
	...
	LocalC == true ->
	    fun() ->
		    Res = mnesia2_loader:disc_load_table(Tab, load_local_content),
	...
	true ->
	    fun() ->
		    ...
		    %% two nodes
		    Res = mnesia2_loader:net_load_table(Tab, Reason, Active, Cs),
		    case Res of
			{loaded, ok} ->
...
load_table_fun(#disc_load{table=Tab, reason=Reason, opt_reply_to=ReplyTo}) ->
	...
	ReadNode == nowhere ->
	    ...
	    Cs = val({Tab, cstruct}),
	    fun() ->
		    case mnesia2_loader:net_load_table(Tab, Reason, Active, Cs) of
			{loaded, ok} ->
			    Done#loader_done{needs_sync = true};
...
disc_load_table(Tab, Reason, ReplyTo) ->
    ...
    Res = mnesia2_loader:disc_load_table(Tab, Reason),
    if
	Res == {loaded, ok} ->
	    Done#loader_done{needs_announce = true,
			     needs_sync = true,
...
filter_active(Tab) ->
	...
	case LS of
	    disc_only_copies ->
		G = mnesia2_lib:intersect(Ns, DOC),
		{G,Ns--G};
	    _ ->
		G = Ns -- DOC,
		{G,Ns--G}
...
do_filter_active(false, Active, Masters) ->
    mnesia2_lib:intersect(Active, Masters).
