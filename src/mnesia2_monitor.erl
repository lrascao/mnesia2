...
negotiate_protocol_impl(Nodes, Requester) ->
    Version    = mnesia2:system_info(version),
...
start_proc(Who, Mod, Fun, Args) ->
    ...
    proc_lib:start_link(mnesia2_sp, init_proc, Args2, infinity).
...
terminate_proc(Who, Reason, _State) ->
    mnesia2_lib:verbose("~p terminated: ~p~n", [Who, Reason]),
    ok.
...
init([Parent]) ->
    ...
    ?ets_new_table(mnesia2_gvar, [set, public, named_table]),
    ?ets_new_table(mnesia2_stats, [set, public, named_table]),
    set(subscribers, []),
	...
	ok ->
	    mnesia2_lib:set({'$$$_report', current_pos}, 0),
	    Level = mnesia2_lib:val(debug),
	    mnesia2_lib:verbose("Mnesia2 debug level set to ~p\n", [Level]),
	    set(mnesia2_status, starting), %%  set start status
	    set({current, db_nodes}, [node()]),
	    set(use_dir, use_dir()),
	    mnesia2_lib:create_counter(trans_aborts),
	    mnesia2_lib:create_counter(trans_commits),
	    mnesia2_lib:create_counter(trans_log_writes),
	    Left = get_env(dump_log_write_threshold),
	    mnesia2_lib:set_counter(trans_log_writes_left, Left),
	    mnesia2_lib:create_counter(trans_log_writes_prev),
	    mnesia2_lib:create_counter(trans_restarts),
	    mnesia2_lib:create_counter(trans_failures),
	    set(checkpoints, []),
	    ...
	    mnesia2_lib:report_fatal("Bad configuration: ~p~n", [Reason]),
...
non_empty_dir() ->
    ...
    mnesia2_lib:exists(mnesia2_lib:tab2dmp(schema)) or
...
handle_call({close_dets, Tab}, _From, State) ->
    ok = mnesia2_lib:dets_sync_close(Tab),
    {reply, ok, State};
...
handle_call({negotiate_protocol, Mon, Version, Protocols}, From, State)
  when node(Mon) /= node() ->
    ...
    MyVersion = mnesia2:system_info(version),
    case lists:member(Protocol, Protocols) of
	true ->
...
handle_call({negotiate_protocol, Nodes}, From, State) ->
    case mnesia2_lib:intersect(State#state.going_down, Nodes) of
	[] ->
...
handle_cast({mnesia2_down, mnesia2_controller, Node}, State) ->
    mnesia2_tm:mnesia2_down(Node),
    {noreply, State};

handle_cast({mnesia2_down, mnesia2_tm, Node}, State) ->
    Down = {mnesia2_down, Node},
    mnesia2_lib:report_system_event(Down),
...
handle_info({'EXIT', Pid, fatal}, State) when node(Pid) == node() ->
    ...
    ?SAFE(exit(whereis(mnesia2_locker), kill)),
    {noreply, State};
...
handle_info(Msg = {'EXIT',Pid,_}, State) ->
	...
	Node /= node(), State#state.connecting == undefined ->
	    ...
	    mnesia2_recover:mnesia2_down(Node),
	    mnesia2_controller:mnesia2_down(Node),
...
handle_info({check_nodeup, Node}, State) ->
    State2 = check_mnesia2_down(Node, State),
    {noreply, State2};

handle_info({nodeup, Node}, State) ->
    State2 = remote_node_status(Node, up, State),
    State3 = check_mnesia2_down(Node, State2),
    {noreply, State3};
...
handle_info({disk_log, _Node, Log, Info}, State) ->
	...
	{truncated, _No} ->
	    ok;
	_ ->
	    mnesia2_lib:important("Warning Log file ~p error reason ~s~n",
...
set_env(E,Val) ->
    mnesia2_lib:set(E, check_type(E,Val)),
    ok.

get_env(E) ->
    case ?catch_val(E) of
	{'EXIT', _} ->
	    case application:get_env(mnesia2, E) of
		{ok, Val} ->
		    check_type(E, Val);
		undefined ->
...
env() ->
    [
     ...
     check_overload_period,
     overload_tm_queue_len_threshold,
     async_dirty_n_tm,
     async_dirty_n_sender,
...
default_env(debug) ->
    none;
default_env(dir) ->
    Name = lists:concat(["Mnesia2.", node()]),
...
default_env(send_max_transfer_size) ->
    7500;
default_env(async_dirty_n_tm) ->
    32;
default_env(async_dirty_n_sender) ->
    3;
...
default_env(async_dirty_buffer_drained_cutoff) ->
    5;
default_env(check_overload_period) ->
    10;
default_env(overload_tm_queue_len_threshold) ->
    100.
...
do_check_type(check_overload_period, L) when is_integer(L), L > 0 -> L;
do_check_type(overload_tm_queue_len_threshold, L) when is_integer(L), L > 0 -> L;
do_check_type(async_dirty_n_tm, L) when is_integer(L), L > 0 -> L;
do_check_type(async_dirty_n_sender, L) when is_integer(L), L > 0 -> L;
...
patch_env(Env, Val) ->
	...
	NewVal ->
	    application_controller:set_env(mnesia2, Env, NewVal),
	    NewVal
    catch error:_ ->
...
detect_inconcistency(Nodes, Context) ->
    Downs = [N || N <- Nodes, mnesia2_recover:has_mnesia2_down(N)],
    {Replies, _BadNodes} =
...
has_remote_mnesia2_down(Node) ->
    HasDown = mnesia2_recover:has_mnesia2_down(Node),
    Master  = mnesia2_recover:get_master_nodes(schema),
    if
	HasDown == true, Master == [] ->
	    {true, node()};
	true ->
...
remote_node_status(Node, Status, State) ->
    {ok, Nodes} = mnesia2_schema:read_nodes(),
...
check_mnesia2_down(Node, State = #state{remote_node_status = RNodeS}) ->
    ...
    HasDown   = mnesia2_recover:has_mnesia2_down(Node),
    ImRunning = mnesia2_lib:is_running(),
    if
