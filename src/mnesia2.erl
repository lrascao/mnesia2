...
start() ->
	...
	{error, {already_started, mnesia2}} ->
	    verbose("Mnesia2 already started, ~p seconds~n",[ Secs]),
	    ok;
	{error, R} ->
	    verbose("Mnesia2 failed to start, ~p seconds: ~p~n",[ Secs, R]),
	    {error, R}
    end.
...
patched_start([{Env, Val} | Tail]) when is_atom(Env) ->
    case mnesia2_monitor:patch_env(Env, Val) of
	{error, Reason} ->
	    {error, Reason};
	_NewVal ->
	    patched_start(Tail)
...
set_debug_level(Level) ->
    mnesia2_subscr:set_debug_level(Level).

lkill() ->
    mnesia2_sup:kill().

kill() ->
    rpc:multicall(mnesia2_sup, kill, []).
...
transaction(Fun) ->
    transaction(get(mnesia2_activity_state), Fun, [], infinity, ?DEFAULT_ACCESS, async).
transaction(Fun, Retries) when is_integer(Retries), Retries >= 0 ->
    transaction(get(mnesia2_activity_state), Fun, [], Retries, ?DEFAULT_ACCESS, async);
transaction(Fun, Retries) when Retries == infinity ->
    transaction(get(mnesia2_activity_state), Fun, [], Retries, ?DEFAULT_ACCESS, async);
transaction(Fun, Args) ->
    transaction(get(mnesia2_activity_state), Fun, Args, infinity, ?DEFAULT_ACCESS, async).
transaction(Fun, Args, Retries) ->
    transaction(get(mnesia2_activity_state), Fun, Args, Retries, ?DEFAULT_ACCESS, async).

sync_transaction(Fun) ->
    transaction(get(mnesia2_activity_state), Fun, [], infinity, ?DEFAULT_ACCESS, sync).
sync_transaction(Fun, Retries) when is_integer(Retries), Retries >= 0 ->
    transaction(get(mnesia2_activity_state), Fun, [], Retries, ?DEFAULT_ACCESS, sync);
sync_transaction(Fun, Retries) when Retries == infinity ->
    transaction(get(mnesia2_activity_state), Fun, [], Retries, ?DEFAULT_ACCESS, sync);
sync_transaction(Fun, Args) ->
    transaction(get(mnesia2_activity_state), Fun, Args, infinity, ?DEFAULT_ACCESS, sync).
sync_transaction(Fun, Args, Retries) ->
    transaction(get(mnesia2_activity_state), Fun, Args, Retries, ?DEFAULT_ACCESS, sync).


transaction(State, Fun, Args, Retries, Mod, Kind)
  when is_function(Fun), is_list(Args), Retries == infinity, is_atom(Mod) ->
    mnesia2_tm:transaction(State, Fun, Args, Retries, Mod, Kind);
transaction(State, Fun, Args, Retries, Mod, Kind)
...
non_transaction(State, Fun, Args, ActivityKind, Mod)
  when is_function(Fun), is_list(Args), is_atom(Mod) ->
    mnesia2_tm:non_transaction(State, Fun, Args, ActivityKind, Mod);
...
sync_dirty(Fun) ->
    sync_dirty(Fun, []).
sync_dirty(Fun, Args) ->
    non_transaction(get(mnesia2_activity_state), Fun, Args, sync_dirty, ?DEFAULT_ACCESS).

ets(Fun) ->
    ets(Fun, []).
ets(Fun, Args) ->
    non_transaction(get(mnesia2_activity_state), Fun, Args, ets, ?DEFAULT_ACCESS).

activity(Kind, Fun) ->
    activity(Kind, Fun, []).
activity(Kind, Fun, Args) when is_list(Args) ->
    activity(Kind, Fun, Args, mnesia2_monitor:get_env(access_module));
activity(Kind, Fun, Mod) ->
    activity(Kind, Fun, [], Mod).

activity(Kind, Fun, Args, Mod) ->
    State = get(mnesia2_activity_state),
    case Kind of
...
lock_record(Tid, Ts, Tab, Key, LockKind) when is_atom(Tab) ->
    ...
    case LockKind of
	read ->
	    mnesia2_locker:rlock(Tid, Store, Oid);
	write ->
	    mnesia2_locker:wlock(Tid, Store, Oid);
	sticky_write ->
	    mnesia2_locker:sticky_wlock(Tid, Store, Oid);
	none ->
	    [];
	_ ->
...
lock_table(Tid, Ts, Tab, LockKind) when is_atom(Tab) ->
    ...
    case LockKind of
	read ->
	    mnesia2_locker:rlock_table(Tid, Store, Tab);
	write ->
	    mnesia2_locker:wlock_table(Tid, Store, Tab);
	load ->
	    mnesia2_locker:load_lock_table(Tid, Store, Tab);
	sticky_write ->
	    mnesia2_locker:sticky_wlock_table(Tid, Store, Tab);
	none ->
	    [];
	_ ->
	    abort({bad_type, Tab, LockKind})
...
global_lock(Tid, Ts, Item, Kind, Nodes) when is_list(Nodes) ->
	...
	tid ->
		...
		Kind /= read, Kind /= write ->
		    abort({bad_type, Kind});
		true ->
		    mnesia2_locker:global_lock(Tid, Store, Item, Kind, GoodNs)
	    end;
	_Protocol ->
	    []
    end;
...
good_global_nodes(Nodes) ->
    ...
    mnesia2_lib:intersect(Nodes, Recover).
  ...
  when is_atom(Tab), Tab /= schema ->
	...
	tid ->
		...
		case LockKind of
			...
			mnesia2_locker:rlock(Tid, Store, Oid);
		    write ->
			mnesia2_locker:rwlock(Tid, Store, Oid);
		    sticky_write ->
			mnesia2_locker:sticky_rwlock(Tid, Store, Oid);
		    _ ->
			abort({bad_type, Tab, LockKind})
		end,
...
do_foldl(A, O, Tab, Key, Fun, Acc, ordered_set, [H | Stored]) when H == Key ->
    ...
    {_, Tid, Ts} = get(mnesia2_activity_state),
...
init_iteration(ActivityId, Opaque, Tab, LockKind) ->
	...
	St == unknown ->
	    ignore;
	true ->
	    mnesia2_lib:db_fixtable(St, Tab, true)
    end,
    {Type, Previous}.
...
close_iteration(Res, Tab) ->
	...
	unknown ->
	    ignore;
	St ->
	    mnesia2_lib:db_fixtable(St, Tab, false)
    end,
    case Res of
	{'EXIT', {aborted, What}} ->
	   abort(What);
...
match_object(Tid, Ts, Tab, Pat, LockKind)
  when is_atom(Tab), Tab /= schema, is_tuple(Pat), tuple_size(Pat) > 2 ->
    case element(1, Tid) of
	ets ->
	    mnesia2_lib:db_match_object(ram_copies, Tab, Pat);
	tid ->
	    Key = element(2, Pat),
	    case has_var(Key) of
...
fun_select(Tid, Ts, Tab, Spec, LockKind, TabPat, SelectFun) ->
    case element(1, Tid) of
	ets ->
	    mnesia2_lib:db_select(ram_copies, Tab, Spec);
	tid ->
...
select(Tid, Ts, Tab, Spec, NObjects, LockKind) ->
    ...
    Type = mnesia2_lib:storage_type_at_node(Where,Tab),
...
fun_select(Tid, Ts, Tab, Spec, LockKind, TabPat, Init, NObjects, Node, Storage) ->
    Def = #mnesia2_select{tid=Tid,node=Node,storage=Storage,tab=Tab,orig=Spec},
    case element(1, Tid) of
	ets ->
	    select_state(mnesia2_lib:db_select_init(ram_copies,Tab,Spec,NObjects),Def);
	tid ->
	    select_lock(Tid,Ts,LockKind,Spec,Tab),
	    Store = Ts#tidstore.store,
...
select_cont(Tid,_Ts,State=#mnesia2_select{tid=Tid,cont=Cont, orig=Ms})
  when element(1,Tid) == ets ->
    case Cont of
	'$end_of_table' -> '$end_of_table';
	_ -> select_state(mnesia2_lib:db_select_cont(ram_copies,Cont,Ms),State)
    end;
...
select_state({Matches, Cont}, MS) ->
    {Matches, MS#mnesia2_select{cont=Cont}};
...
index_match_object(Tid, Ts, Tab, Pat, Attr, LockKind)
  when is_atom(Tab), Tab /= schema, is_tuple(Pat), tuple_size(Pat) > 2 ->
	...
	tid ->
	    case mnesia2_schema:attr_tab_to_pos(Tab, Attr) of
...
index_read(Tid, Ts, Tab, Key, Attr, LockKind)
  when is_atom(Tab), Tab /= schema ->
	...
	tid ->
	    Pos = mnesia2_schema:attr_tab_to_pos(Tab, Attr),
	    case LockKind of
		read ->
		    case has_var(Key) of
			false ->
			    Store = Ts#tidstore.store,
			    Objs = mnesia2_index:read(Tid, Store, Tab, Key, Pos),
...
do_dirty_delete(SyncMode, Tab, Key) when is_atom(Tab), Tab /= schema  ->
    Oid = {Tab, Key},
    mnesia2_tm:dirty(SyncMode, {Oid, Oid, delete});
...
do_dirty_delete_object(SyncMode, Tab, Val)
    ...
    case has_var(Val) of
	false ->
	    mnesia2_tm:dirty(SyncMode, {Oid, Val, delete_object});
	true ->
	    abort({bad_type, Tab, Val})
    end;
...
do_dirty_update_counter(SyncMode, Tab, Key, Incr)
  when is_atom(Tab), Tab /= schema, is_integer(Incr) ->
	...
	{RecName, 3, set} ->
	    Oid = {Tab, Key},
	    mnesia2_tm:dirty(SyncMode, {Oid, {RecName, Incr}, update_counter});
	_ ->
...
dirty_read(Tab, Key)
  when is_atom(Tab), Tab /= schema ->
    dirty_rpc(Tab, mnesia2_lib, db_get, [Tab, Key]);
dirty_read(Tab, _Key) ->
    abort({bad_type, Tab}).
...
remote_dirty_match_object(Tab, Pat) ->
    ...
    case has_var(Key) of
	false ->
	    mnesia2_lib:db_match_object(Tab, Pat);
	true ->
...
remote_dirty_match_object(Tab, Pat, [Pos | Tail]) when Pos =< tuple_size(Pat) ->
	...
	false ->
	    mnesia2_index:dirty_match_object(Tab, Pat, Pos);
	true ->
...
remote_dirty_select(Tab, Spec) ->
	...
	[{HeadPat, _, _}] when is_tuple(HeadPat), tuple_size(HeadPat) > 2 ->
	    ...
	    case has_var(Key) of
		false ->
		    mnesia2_lib:db_select(Tab, Spec);
		true  ->
		    PosList = val({Tab, index}),
...
remote_dirty_select(Tab, [{HeadPat,_, _}] = Spec, [Pos | Tail])
  when is_tuple(HeadPat), tuple_size(HeadPat) > 2, Pos =< tuple_size(HeadPat) ->
    ...
    case has_var(Key) of
	false ->
	    Recs = mnesia2_index:dirty_select(Tab, HeadPat, Pos),
...
dirty_sel_init(Node,Tab,Spec,NObjects,Type) ->
    do_dirty_rpc(Tab,Node,mnesia2_lib,db_select_init,[Type,Tab,Spec,NObjects]).

dirty_sel_cont(#mnesia2_select{cont='$end_of_table'}) -> '$end_of_table';
dirty_sel_cont(#mnesia2_select{node=Node,tab=Tab,storage=Type,cont=Cont,orig=Ms}) ->
    do_dirty_rpc(Tab,Node,mnesia2_lib,db_select_cont,[Type,Cont,Ms]).
  ...
  when is_atom(Tab), Tab /= schema, is_tuple(Pat), tuple_size(Pat) > 2 ->
	...
	Pos when Pos =< tuple_size(Pat) ->
		...
		true ->
			...
			false ->
			    dirty_rpc(Tab, mnesia2_index, dirty_match_object,
				      [Tab, Pat, Pos]);
			true ->
...
dirty_index_read(Tab, Key, Attr) when is_atom(Tab), Tab /= schema ->
    ...
    case has_var(Key) of
	false ->
	    mnesia2_index:dirty_read(Tab, Key, Pos);
	true ->
	    abort({bad_type, Tab, Attr, Key})
    end;
...
dirty_slot(Tab, Slot) when is_atom(Tab), Tab /= schema, is_integer(Slot)  ->
    dirty_rpc(Tab, mnesia2_lib, db_slot, [Tab, Slot]);
dirty_slot(Tab, Slot) ->
    abort({bad_type, Tab, Slot}).

dirty_first(Tab) when is_atom(Tab), Tab /= schema ->
    dirty_rpc(Tab, mnesia2_lib, db_first, [Tab]);
dirty_first(Tab) ->
    abort({bad_type, Tab}).

dirty_last(Tab) when is_atom(Tab), Tab /= schema ->
    dirty_rpc(Tab, mnesia2_lib, db_last, [Tab]);
dirty_last(Tab) ->
    abort({bad_type, Tab}).

dirty_next(Tab, Key) when is_atom(Tab), Tab /= schema ->
    dirty_rpc(Tab, mnesia2_lib, db_next_key, [Tab, Key]);
dirty_next(Tab, _Key) ->
    abort({bad_type, Tab}).

dirty_prev(Tab, Key) when is_atom(Tab), Tab /= schema ->
    dirty_rpc(Tab, mnesia2_lib, db_prev_key, [Tab, Key]);
dirty_prev(Tab, _Key) ->
    abort({bad_type, Tab}).
...
do_dirty_rpc(_Tab, nowhere, _, _, Args) ->
    mnesia2:abort({no_exists, Args});
do_dirty_rpc(Tab, Node, M, F, Args) ->
    case rpc:call(Node, M, F, Args) of
	{badrpc, Reason} ->
	    timer:sleep(20), %% Do not be too eager, and can't use yield on SMP
	    %% Sync with mnesia2_monitor
	    _ = try sys:get_status(mnesia2_monitor) catch _:_ -> ok end,
	    case mnesia2_controller:call({check_w2r, Node, Tab}) of % Sync
		NewNode when NewNode =:= Node ->
		    ErrorTag = mnesia2_lib:dirty_rpc_error_tag(Reason),
		    mnesia2:abort({ErrorTag, Args});
		NewNode ->
		    case get(mnesia2_activity_state) of
			{_Mod, Tid, _Ts} when is_record(Tid, tid) ->
			    ...
			    mnesia2:abort({node_not_running, Node});
...
any_table_info(Tab, Item) when is_atom(Tab) ->
    case Item of
	master_nodes ->
	    mnesia2_recover:get_master_nodes(Tab);
%	checkpoints ->
...
%	    end;
	...
	all ->
	    case mnesia2_schema:get_table_properties(Tab) of
		[] ->
		    abort({no_exists, Tab, Item});
		Props ->
...
raw_table_info(Tab, Item) ->
    try
	case ?ets_lookup_element(mnesia2_gvar, {Tab, storage_type}, 2) of
	    ram_copies ->
...
schema(Tab) ->
    mnesia2_schema:info(Tab).

error_description(Err) ->
    mnesia2_lib:error_desc(Err).

info() ->
    case mnesia2_lib:is_running() of
	yes ->
	    TmInfo = mnesia2_tm:get_info(10000),
	    ...
	    mnesia2_tm:display_info(group_leader(), TmInfo),

	    Pat = {'_', unclear, '_'},
	    Uncertain = ets:match_object(mnesia2_decision, Pat),
...
display_tab_info() ->
    MasterTabs = mnesia2_recover:get_master_node_tables(),
...
system_info2(all) ->
    Items = system_info_items(mnesia2_lib:is_running()),
...
system_info2(db_nodes) ->
	...
	is_list(DiscNs), is_list(RamNs) ->
	    DiscNs ++ RamNs;
	true ->
	    case mnesia2_schema:read_nodes() of
		{ok, Nodes} -> Nodes;
...
system_info2(extra_db_nodes) ->
	...
	{'EXIT',_} ->
	    ...
	    %% application_controller to get into deadlock
	    load_mnesia2_or_abort(),
	    mnesia2_monitor:get_env(extra_db_nodes);
	Other ->
	    Other
    end;
...
system_info2(schema_location) ->
	...
	{'EXIT',_} ->
	    ...
	    %% application_controller to get into deadlock
	    load_mnesia2_or_abort(),
	    mnesia2_monitor:get_env(schema_location);
	Other ->
	    Other
    end;
...
system_info2(version) ->
	...
	{'EXIT', _} ->
		...
		false ->
		    ...
		    {mnesia2_not_loaded, node(), mnesia2_time:timestamp()}
	    end;
	Version ->
	    Version
    end;

system_info2(access_module) -> mnesia2_monitor:get_env(access_module);
system_info2(auto_repair) -> mnesia2_monitor:get_env(auto_repair);
system_info2(is_running) -> mnesia2_lib:is_running();
system_info2(backup_module) -> mnesia2_monitor:get_env(backup_module);
system_info2(event_module) -> mnesia2_monitor:get_env(event_module);
system_info2(debug) -> mnesia2_monitor:get_env(debug);
system_info2(dump_log_load_regulation) -> mnesia2_monitor:get_env(dump_log_load_regulation);
system_info2(dump_log_write_threshold) -> mnesia2_monitor:get_env(dump_log_write_threshold);
system_info2(dump_log_time_threshold) -> mnesia2_monitor:get_env(dump_log_time_threshold);
system_info2(dump_log_update_in_place) ->
    mnesia2_monitor:get_env(dump_log_update_in_place);
system_info2(max_wait_for_decision) -> mnesia2_monitor:get_env(max_wait_for_decision);
system_info2(ignore_fallback_at_startup) -> mnesia2_monitor:get_env(ignore_fallback_at_startup);
system_info2(fallback_error_function) ->  mnesia2_monitor:get_env(fallback_error_function);
system_info2(log_version) -> mnesia2_log:version();
system_info2(protocol_version) -> mnesia2_monitor:protocol_version();
system_info2(schema_version) -> mnesia2_schema:version(); %backward compatibility
system_info2(tables) -> val({schema, tables});
system_info2(local_tables) -> val({schema, local_tables});
system_info2(master_node_tables) -> mnesia2_recover:get_master_node_tables();
system_info2(subscribers) -> mnesia2_subscr:subscribers();
system_info2(checkpoints) -> mnesia2_checkpoint:checkpoints();
system_info2(held_locks) -> mnesia2_locker:get_held_locks();
system_info2(lock_queue) -> mnesia2_locker:get_lock_queue();
system_info2(transactions) -> mnesia2_tm:get_transactions();
system_info2(transaction_failures) -> mnesia2_lib:read_counter(trans_failures);
system_info2(transaction_commits) -> mnesia2_lib:read_counter(trans_commits);
system_info2(transaction_restarts) -> mnesia2_lib:read_counter(trans_restarts);
system_info2(transaction_log_writes) -> mnesia2_dumper:get_log_writes();
system_info2(core_dir) ->  mnesia2_monitor:get_env(core_dir);
...
system_info() ->
    IsRunning = mnesia2_lib:is_running(),
    case IsRunning of
	yes ->
	    TmInfo = mnesia2_tm:get_info(10000),
	    Held = system_info(held_locks),
...
create_schema(Ns) ->
    mnesia2_bup:create_schema(Ns).

delete_schema(Ns) ->
    mnesia2_schema:delete_schema(Ns).

backup(Opaque) ->
    mnesia2_log:backup(Opaque).

backup(Opaque, Mod) ->
    mnesia2_log:backup(Opaque, Mod).

traverse_backup(S, T, Fun, Acc) ->
    mnesia2_bup:traverse_backup(S, T, Fun, Acc).

traverse_backup(S, SM, T, TM, F, A) ->
    mnesia2_bup:traverse_backup(S, SM, T, TM, F, A).

install_fallback(Opaque) ->
    mnesia2_bup:install_fallback(Opaque).

install_fallback(Opaque, Mod) ->
    mnesia2_bup:install_fallback(Opaque, Mod).

uninstall_fallback() ->
    mnesia2_bup:uninstall_fallback().

uninstall_fallback(Args) ->
    mnesia2_bup:uninstall_fallback(Args).

activate_checkpoint(Args) ->
    mnesia2_checkpoint:activate(Args).

deactivate_checkpoint(Name) ->
    mnesia2_checkpoint:deactivate(Name).

backup_checkpoint(Name, Opaque) ->
    mnesia2_log:backup_checkpoint(Name, Opaque).

backup_checkpoint(Name, Opaque, Mod) ->
    mnesia2_log:backup_checkpoint(Name, Opaque, Mod).

restore(Opaque, Args) ->
    mnesia2_schema:restore(Opaque, Args).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Table mgt

create_table(Arg) ->
    mnesia2_schema:create_table(Arg).
create_table(Name, Arg) when is_list(Arg) ->
    mnesia2_schema:create_table([{name, Name}| Arg]);
create_table(Name, Arg) ->
    {aborted, badarg, Name, Arg}.

delete_table(Tab) ->
    mnesia2_schema:delete_table(Tab).

add_table_copy(Tab, N, S) ->
    mnesia2_schema:add_table_copy(Tab, N, S).
del_table_copy(Tab, N) ->
    mnesia2_schema:del_table_copy(Tab, N).

move_table_copy(Tab, From, To) ->
    mnesia2_schema:move_table(Tab, From, To).

add_table_index(Tab, Ix) ->
    mnesia2_schema:add_table_index(Tab, Ix).
del_table_index(Tab, Ix) ->
    mnesia2_schema:del_table_index(Tab, Ix).

transform_table(Tab, Fun, NewA) ->
    try val({Tab, record_name}) of
	OldRN -> mnesia2_schema:transform_table(Tab, Fun, NewA, OldRN)
    catch exit:Reason ->
	    mnesia2:abort(Reason)
    end.

transform_table(Tab, Fun, NewA, NewRN) ->
    mnesia2_schema:transform_table(Tab, Fun, NewA, NewRN).

change_table_copy_type(T, N, S) ->
    mnesia2_schema:change_table_copy_type(T, N, S).
...
clear_table(Tab) ->
	...
	_ -> %% Not allowed for clear_table
	    mnesia2:abort({aborted, nested_transaction})
    end.
...
write_table_property(Tab, Prop) ->
    mnesia2_schema:write_table_property(Tab, Prop).

delete_table_property(Tab, PropKey) ->
    mnesia2_schema:delete_table_property(Tab, PropKey).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Table mgt - user properties

change_table_frag(Tab, FragProp) ->
    mnesia2_schema:change_table_frag(Tab, FragProp).
...
dump_tables(Tabs) ->
    mnesia2_schema:dump_tables(Tabs).

%% allow the user to wait for some tables to be loaded
wait_for_tables(Tabs, Timeout) ->
    mnesia2_controller:wait_for_tables(Tabs, Timeout).

force_load_table(Tab) ->
    case mnesia2_controller:force_load_table(Tab) of
	ok -> yes; % Backwards compatibility
	Other -> Other
    end.

change_table_access_mode(T, Access) ->
    mnesia2_schema:change_table_access_mode(T, Access).

change_table_load_order(T, O) ->
    mnesia2_schema:change_table_load_order(T, O).

change_table_majority(T, M) ->
    mnesia2_schema:change_table_majority(T, M).
...
set_master_nodes(Nodes) when is_list(Nodes) ->
	...
	yes ->
	    CsPat = {{'_', cstruct}, '_'},
	    Cstructs0 = ?ets_match_object(mnesia2_gvar, CsPat),
...
log_valid_master_nodes(Cstructs, Nodes, UseDir, IsRunning) ->
    Fun = fun(Cs) ->
		  Copies = mnesia2_lib:copy_holders(Cs),
		  Valid = mnesia2_lib:intersect(Nodes, Copies),
		  {Cs#cstruct.name, Valid}
	  end,
    Args = lists:map(Fun, Cstructs),
    mnesia2_recover:log_master_nodes(Args, UseDir, IsRunning).
...
set_master_nodes(Tab, Nodes) when is_list(Nodes) ->
	...
	yes ->
		...
		{'EXIT', _} ->
		    {error, {no_exists, Tab}};
		Cs ->
		    case Nodes -- mnesia2_lib:copy_holders(Cs) of
			[] ->
			    Args = [{Tab , Nodes}],
			    mnesia2_recover:log_master_nodes(Args, UseDir, IsRunning);
			BadNodes ->
...
dump_log() ->
    mnesia2_controller:sync_dump_log(user).

sync_log() ->
    mnesia2_monitor:sync_log(latest_log).

subscribe(What) ->
    mnesia2_subscr:subscribe(self(), What).

unsubscribe(What) ->
    mnesia2_subscr:unsubscribe(self(), What).

report_event(Event) ->
    mnesia2_lib:report_system_event({mnesia2_user, Event}).

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Snmp

snmp_open_table(Tab, Us) ->
    mnesia2_schema:add_snmp(Tab, Us).

snmp_close_table(Tab) ->
    mnesia2_schema:del_snmp(Tab).
...
snmp_get_row(Tab, RowIndex) when is_atom(Tab), Tab /= schema, is_list(RowIndex) ->
	...
	_ ->
 	    dirty_rpc(Tab, mnesia2_snmp_hook, get_row, [Tab, RowIndex])
    end;
...
snmp_get_next_index(Tab, RowIndex) when is_atom(Tab), Tab /= schema, is_list(RowIndex) ->
    {Next,OrigKey} = dirty_rpc(Tab, mnesia2_snmp_hook, get_next_index, [Tab, RowIndex]),
    case get(mnesia2_activity_state) of
...
snmp_order_keys(Store,Tab,RowIndex,Def) ->
    All = ?ets_match(Store, {{Tab,'$1'},'_','$2'}),
    SnmpType = val({Tab,snmp}),
    Keys0 = [mnesia2_snmp_hook:key_to_oid(Tab,Key,SnmpType) ||
...
snmp_get_mnesia2_key(Tab, RowIndex) when is_atom(Tab), Tab /= schema, is_list(RowIndex) ->
    case get(mnesia2_activity_state) of
 	{_Mod, Tid, Ts} when element(1, Tid) =:= tid ->
	    Res = dirty_rpc(Tab,mnesia2_snmp_hook,get_mnesia2_key,[Tab,RowIndex]),
	    snmp_filter_key(Res, RowIndex, Tab, Ts#tidstore.store);
	_ ->
	    dirty_rpc(Tab, mnesia2_snmp_hook, get_mnesia2_key, [Tab, RowIndex])
    end;
snmp_get_mnesia2_key(Tab, _RowIndex) ->
    abort({bad_type, Tab}).

snmp_oid_to_mnesia2_key(RowIndex, Tab) ->
    case mnesia2_snmp_hook:oid_to_key(RowIndex, Tab) of
	unknown ->  %% Contains fix_string needs lookup
	    case dirty_rpc(Tab,mnesia2_snmp_hook,get_mnesia2_key,[Tab,RowIndex]) of
		{ok, Mnesia2Key} -> Mnesia2Key;
		undefined -> unknown
	    end;
	Mnesia2Key ->
	    Mnesia2Key
...
load_textfile(F) ->
    mnesia2_text:load_textfile(F).
dump_to_textfile(F) ->
    mnesia2_text:dump_to_textfile(F).
...
table(Tab,Opts) ->
    ...
    ParentFun = fun() ->
			{mnesia2_activity, mnesia2:get_activity_id()}
		end,
    Lookup =
	case Trav of
	    {select, _} -> [];
...
pre_qlc(Opts, Tab) ->
	...
	_ ->
		...
		ordered_set ->   ok;
		_ ->
		    dirty_rpc(Tab, mnesia2_tm, fixtable, [Tab,true,self()]),
		    ok
	    end
    end.
...
post_qlc(Tab) ->
	...
	_ ->
		...
		ordered_set ->
		    ok;
		_ ->
		    dirty_rpc(Tab, mnesia2_tm, fixtable, [Tab,false,self()]),
		    ok
	    end
    end.
...
do_fixtable(Tab, Store) ->
	...
	_ ->
		...
		[] ->
		    Node = dirty_rpc(Tab, mnesia2_tm, fixtable, [Tab,true,self()]),
...
get_activity_id() ->
    get(mnesia2_activity_state).

put_activity_id(Activity) ->
    mnesia2_tm:put_activity_id(Activity).
put_activity_id(Activity,Fun) ->
    mnesia2_tm:put_activity_id(Activity,Fun).
